...
### (1-5) Double DQN
▣ 정의 : Double DQN은 행동 선택과 행동 평가를 분리하여 DQN의 과대 추정 편향 문제를 해결합니다.
▣ 장점 : 과대 추정을 줄입니다. 정책의 안정성을 향상시킵니다.
▣ 단점 : 계산 부담이 증가합니다. 하이퍼파라미터 튜닝이 필요합니다.
▣ 적용 분야 : 복잡한 의사결정 작업.

<br>

### (1-6) Dueling DQN
▣ 정의 : Dueling DQN은 𝑄 값을 상태-가치 𝑉(𝑠)와 이점 𝐴(𝑠,𝑎)로 분리하여 학습합니다:
▣ 장점 : 학습 효율성을 향상시킵니다. 의사결정에 중요한 상태에 집중합니다.
▣ 단점 : 네트워크 복잡도가 증가합니다. 모든 환경에서 이점이 있는 것은 아닙니다.
▣ 적용 분야 : 희소 보상 환경.

<br>

### (1-7) DRQN(Deep Recurrent Q-Network)
▣ 정의 : DRQN은 DQN에 순환 신경망(RNN)을 추가하여 부분 관찰 가능 환경에서 학습을 가능하게 합니다.
▣ 장점 : 순차적 데이터 또는 부분 관찰 데이터를 처리할 수 있습니다. 비마르코프 설정에서도 정책을 개선합니다.
▣ 단점 : 학습 시간이 증가합니다. 학습이 더 어려워질 수 있습니다.
▣ 적용 분야 : 주식 거래. 텍스트 기반 게임.

<br>

### (1-8) C51
▣ 정의 : C51은 단일 기대값 대신 보상의 범주형 분포를 추정하는 알고리즘입니다.
▣ 장점 : 보상의 불확실성을 모델링할 수 있습니다. 더 풍부한 표현을 제공합니다.
▣ 단점 : 분포를 관리해야 하므로 복잡도가 증가합니다. 계산 비용이 더 큽니다.
▣ 적용 분야 : 위험 민감 의사결정.

<br>

### (1-9) IQN(Implicit Quantile Networks)
▣ 정의 : IQN은 보상 분포의 분위수를 예측하여 보상의 변동성을 더 잘 모델링합니다.
▣ 장점 : 다양한 보상 분포에 적응할 수 있습니다. C51보다 표현력이 뛰어납니다.
▣ 단점 : 학습이 복잡합니다. 많은 계산 자원이 필요합니다.
▣ 적용 분야 : 위험 인지 AI. 전략적 계획.

<br>

### (1-10) Rainbow
▣ 정의 : Rainbow는 DQN, Double DQN, PER, Dueling DQN, C51 등을 결합한 포괄적인 RL 알고리즘입니다.
▣ 장점 : 최신 기술을 종합한 최고의 성능. 기존 알고리즘들의 장점을 모두 활용.
▣ 단점 : 구현이 복잡합니다. 계산 비용이 높습니다.
▣ 적용 분야 : 범용 강화 학습 작업.

<br>

### (1-11) SQL
▣ 정의 : SQL은 정책의 엔트로피를 극대화하여 탐색을 촉진합니다.
▣ 장점 : 다양한 행동 선택을 장려합니다. 조기 수렴을 방지합니다.
▣ 단점 : 결정론적 정책에는 비효율적입니다.
▣ 적용 분야 : 로봇 공학. 탐색이 중요한 작업.

<br>

### (1-12) PER
▣ 정의 : PER은 학습 중 중요한 경험에 우선순위를 부여합니다.
▣ 장점 : 더 빠른 수렴. 학습 효율성 향상.
▣ 단점 : 추가 계산 비용이 필요합니다.
▣ 적용 분야 : 게임. 로봇 공학.
...
### (1-13) HER
▣ 정의 : HER(Hindsight Experience Replay)은 실제로 달성한 목표와 다른 가상의 목표를 설정하여 학습 데이터를 수정하는 방식으로, 희소한 보상 환경에서 효율적으로 학습할 수 있도록 돕는 알고리즘입니다.
▣ 장점 : 희소 보상 환경에서 샘플 효율성을 크게 향상시킵니다. 목표 중심 환경에서 성능이 우수합니다. 기존 경험을 효과적으로 활용합니다.
▣ 단점 : 목표가 명확히 정의된 환경에서만 사용할 수 있습니다. 구현이 복잡할 수 있습니다.
▣ 적용 분야 : 로봇 공학(예: 물체 조작 및 경로 계획). 목표 지향적 학습 환경.

<br>

### (1-14) NoisyNet
▣ 정의 : NoisyNet은 신경망 가중치에 학습 가능한 노이즈를 추가하여 탐색(exploration)을 자동화하는 알고리즘입니다. 기존의 𝜖-탐욕적 정책 대신 학습 과정에서 노이즈를 조절하여 더 나은 탐색과 학습 균형을 제공합니다.
▣ 장점 : 탐색-활용(trade-off) 문제를 자동으로 해결합니다. 𝜖-탐욕적 정책에 대한 추가 튜닝이 필요하지 않습니다. 학습 과정에서 더 효과적인 탐색을 가능하게 합니다.
▣ 단점 : 추가 학습 가능한 노이즈 매개변수를 관리해야 하므로 학습 시간이 증가합니다. 환경에 따라 노이즈 설정이 비효율적일 수 있습니다.
▣ 적용 분야 : 연속적인 행동 공간을 가진 환경. 다양한 보상을 가진 게임 환경.
...
### (2-1) Policy Gradient
▣ 정의 : Policy Gradient는 정책(Policy) 자체를 직접 학습하는 강화 학습 알고리즘으로, 행동-가치 함수 𝑄(𝑠,𝑎)를 통해 정책의 성능을 측정하고 정책을 업데이트합니다. 목표는 파라미터화된 정책 𝜋𝜃(𝑎∣𝑠)를 최대화하는 것입니다.
▣ 장점 : 연속적인 행동 공간을 처리할 수 있습니다. 확률적 정책을 통해 다양한 행동 선택 가능. 탐색 문제를 효과적으로 해결.
▣ 단점 : 수렴 속도가 느릴 수 있습니다. 높은 분산(Variance)으로 인해 불안정할 수 있습니다. 충분한 샘플 데이터가 필요.
▣ 적용 분야 : 로봇 제어. 연속적인 제어 문제. 자율 주행.

<br>

### (2-2) A2C/A3C
▣ 정의 : A2C와 A3C는 Actor-Critic 구조 기반으로, Actor는 정책을 업데이트하고 Critic은  𝑄-값 또는 𝑉-값을 평가합니다. A3C는 비동기적(Asynchronous)으로 여러 에이전트가 동시에 학습하며, A2C는 동기적(Synchronous)으로 학습합니다.
▣ 장점 : 학습 속도 향상(A3C). 정책과 가치 함수 학습을 분리하여 안정성 제공. 멀티 쓰레드로 효율적인 샘플 수집 가능.
...
### (2-3) PPO
▣ 정의
PPO는 Policy Gradient를 안정적으로 학습하기 위해 손실 함수에 클리핑을 도입한 알고리즘입니다. 기존 정책과 새로운 정책 간의 차이를 제한하여 학습 안정성을 확보합니다.
▣ 장점

구현이 간단하고 안정적입니다.
Hyperparameter에 덜 민감합니다.
높은 성능.
▣ 단점

일부 환경에서 클리핑이 비효율적일 수 있습니다.
계산 비용이 높습니다.
▣ 적용 분야
...
### (2-4) TRPO
▣ 정의
TRPO는 정책 업데이트 시 큰 변화를 방지하기 위해 신뢰 영역(Trust Region)을 유지하며 학습합니다. Kullback-Leibler (KL) 발산을 사용하여 정책 간 차이를 제한합니다.
...
### (2-5) Actor-Critic
▣ 정의
Actor-Critic은 정책(Actor)과 가치 함수(Critic)를 동시에 학습하는 구조로, Actor는 정책을 학습하고 Critic은 가치 함수로 피드백을 제공합니다.
...
### (2-6) NAC
▣ 정의
NAC는 자연 그라디언트(Natural Gradient)를 사용하여 정책 업데이트를 안정화합니다. 자연 그라디언트는 기존 그라디언트보다 빠르게 수렴하며 정책 변화의 효과를 극대화합니다.
...
### (2-7) DDPG
▣ 정의
DDPG는 Actor-Critic 기반으로, 연속적인 행동 공간에서 작동하도록 설계된 모델입니다. Actor는 행동을 생성하고, Critic은 행동의 
𝑄
Q-값을 평가합니다.
...
### (2-8) TD3
▣ 정의
TD3는 DDPG의 개선된 버전으로, 행동 가치 평가의 과대 추정을 방지하기 위해 두 개의 Critic 네트워크를 사용합니다. 또한, 행동 업데이트를 지연시켜 안정성을 강화합니다.
...
### (2-9) SAC
▣ 정의
SAC는 정책 엔트로피를 최대화하여 탐색 성능을 높이고 안정성을 강화한 Actor-Critic 알고리즘입니다.
...
### (2-10) BC
▣ 정의
BC는 주어진 행동 데이터를 사용해 정책을 모방 학습(Imitation Learning)합니다. 지도 학습 접근법으로 주로 사용됩니다.
...
### (2-11) DDPGfD
▣ 정의
DDPGfD는 DDPG(Deep Deterministic Policy Gradient)에 전문가 시연 데이터를 추가로 활용하여 초기 학습을 강화하는 알고리즘입니다. 시연 데이터를 우선적으로 활용하여 학습 효율성을 높이는 경험 재생(Experience Replay) 기법을 포함합니다.

▣ 장점

학습 초기 성능이 향상됩니다.
샘플 효율성을 개선합니다.
희소 보상 환경에서도 학습이 용이합니다.
▣ 단점

전문가 시연 데이터가 필수적입니다.
추가 메모리 및 계산 비용이 필요합니다.
▣ 적용 분야

로봇 공학(시연 데이터를 활용한 로봇 작업).
자율 주행 및 복잡한 제어 작업.
<br>

### (2-12) REINFORCE
▣ 정의
REINFORCE는 Policy Gradient의 기본 형태로, 매번 에피소드가 끝난 후 정책의 파라미터를 업데이트합니다. 에피소드별로 누적 보상을 기반으로 정책을 학습합니다. 여기서 𝐺𝑡는 시간 𝑡에서의 누적 보상입니다.

▣ 장점

단순하고 구현이 용이합니다.
정책 학습의 직관적 접근 방식을 제공합니다.
▣ 단점

높은 분산(Variance)으로 인해 학습이 불안정할 수 있습니다.
샘플 효율성이 낮습니다.
▣ 적용 분야
...
### (2-13) ACER
▣ 정의
ACER는 Actor-Critic에 경험 재생(Experience Replay)을 추가하여 샘플 효율성을 높인 알고리즘입니다. PPO의 클리핑 방식과 유사하게 정책 업데이트를 안정화하는 기법도 포함됩니다.

▣ 장점

샘플 효율성 향상.
Actor-Critic 구조의 안정성과 효율성을 제공.
▣ 단점

구현이 복잡합니다.
추가적인 메모리와 계산 비용이 요구됩니다.
▣ 적용 분야
...
### (2-14) IMPALA
▣ 정의
IMPALA는 분산 학습 환경에서 효과적인 Actor-Critic 알고리즘으로, 여러 Actor가 데이터를 수집하고 Learner가 이 데이터를 학습합니다. 중요도 가중치(Importance Weighting)를 사용해 Actor와 Learner 간의 시간차를 보상합니다.
...
### (2-15) Off-PAC
▣ 정의
Off-PAC는 Off-Policy 데이터(정책 외부의 데이터)를 활용하여 Actor-Critic 학습을 수행하는 알고리즘입니다. 행동 정책(Behavior Policy)과 학습 정책(Target Policy)을 분리하여 샘플 효율성을 개선합니다.
...
### (3-3) MuZero
▣ 정의 : MuZero는 AlphaZero의 확장 버전으로, 환경 모델을 사전에 제공받는 대신 학습 과정에서 환경 모델을 스스로 학습하는 알고리즘입니다. 모델 기반 학습과 모델 프리 학습을 결합해 환경 모델이 없는 경우에도 높은 성능을 발휘합니다.
▣ 장점 : 환경 모델 불필요: AlphaZero와 달리 환경 모델이 없어도 동작 가능. 일반화 가능성: 규칙 기반 게임뿐 아니라 다양한 환경에 적용 가능. 효율적 학습: 모델 기반 예측을 통해 데이터 효율성을 극대화.
▣ 단점 : 복잡한 구현: 환경 모델 학습과 RL을 결합해야 하므로 구현이 어렵습니다. 높은 계산 비용: 모델 학습 및 MCTS 사용으로 인해 연산 자원이 많이 요구됩니다.
...
Autoencoder): 관측 데이터를 잠재 공간으로 압축. RNN (Recurrent Neural Network): 상태 전이를 모델링. Controller: 잠재 공간에서 행동을 선택<br>
▣ 장점 : 환경 모델이 학습되면 실제 환경과 상호작용하지 않고도 학습 가능. 데이터 효율성이 높아짐. 환경의 복잡한 동작을 압축 표현으로 학습<br>
▣ 단점 : 복잡한 환경에서는 모델 학습이 어렵고 부정확할 수 있음. 환경 모델이 불완전하면 성능이 저하됨<br>
▣ 적용분야 : 게임과 같은 시뮬레이션 환경. 로봇 제어. 데이터가 제한된 환경에서 강화 학습<br>

<br>

### (4-2) I2A
▣ 정의 : I2A는 상상 기반 학습을 도입한 알고리즘으로, 환경 모델을 학습하고 이를 사용해 행동의 결과를 상상(imagine)합니다. 행동의 상상된 결과를 정책 학습에 활용하여 샘플 효율성을 높입니다.<br>
▣ 장점 : 실제 상호작용 없이 상상된 데이터를 사용해 학습 가능. 모델 기반 접근법과 모델 프리 접근법의 장점을 결합.<br>
▣ 단점 : 상상 모델의 품질에 따라 성능이 크게 달라짐. 계산 비용이 증가.<br>
▣ 적용분야 : 게임 환경. 복잡한 의사결정 문제. 희소 보상 환경.<br>

<br>

### (4-3) MBMF
▣ 정의 : MBMF는 환경 모델(Model-Based)과 모델 프리 학습(Model-Free)을 결합하여 보완적인 방식으로 작동하는 알고리즘입니다. 환경 모델을 통해 빠르게 초기 정책을 학습하고, 이후 모델 프리 방식으로 정책을 미세 조정합니다.<br>
▣ 장점 : 샘플 효율성을 높이며, 안정적인 정책 학습 가능. 초기 학습 속도가 빠름.<br>
▣ 단점 : 환경 모델의 품질에 따라 초기 성능이 영향을 받음. 모델 기반 및 모델 프리 학습 모두 구현해야 하므로 복잡도 증가.<br>
▣ 적용분야 : 로봇 제어. 대규모 시뮬레이션 환경.<br>

<br>

### (4-4) MBVE
▣ 정의 : MBVE는 환경 모델을 사용하여 짧은 시간 동안 예측된 전이(Transition)를 통해 가치 함수(Value Function)를 계산합니다. 이 예측된 값은 정책 학습에 사용됩니다.<br>
▣ 장점 : 샘플 효율성이 높으며, 가치 함수의 품질이 개선됨. 단기 예측을 통해 모델 학습의 불확실성을 감소.<br>
▣ 단점 : 환경 모델의 품질에 따라 성능이 좌우됨. 단기 예측에만 적합하며, 장기적 의사결정에는 한계.<br>
▣ 적용분야 : 게임 AI. 가치 함수 학습이 중요한 환경.<br>

<br>

### (4-5) Dreamer
▣ 정의 : Dreamer는 환경 모델을 학습하여 잠재 공간에서 정책을 최적화하는 알고리즘입니다. PlaNet의 확장 버전으로, 단기 및 장기 보상을 고려하여 정책을 학습합니다.<br>
▣ 장점 : 잠재 공간에서 학습하여 데이터 효율성을 극대화. 샘플 효율성이 높으며, 복잡한 환경에서도 잘 작동.<br>
▣ 단점 : 환경 모델 학습의 복잡도가 높음. 모델 학습에 필요한 계산 자원이 큼.<br>
▣ 적용분야 : 시뮬레이션 및 물리 기반 환경. 연속적인 제어 문제.<br>

<br>

### (4-6) PlaNet
▣ 정의 : PlaNet은 잠재 공간 계획(Latent Space Planning)을 수행하는 강화 학습 알고리즘입니다. 관측 데이터를 잠재 공간으로 변환하고, 이 공간에서 환경 모델을 학습하여 행동을 계획합니다.<br>
▣ 장점 : 환경 모델을 잠재 공간에서 학습하여 계산 효율성이 높음. 장기 계획이 가능한 정책 학습.<br>
▣ 단점 : 복잡한 환경에서는 잠재 공간 모델링이 어렵고 부정확할 수 있음. 계산 자원 요구가 큼.<br>
▣ 적용분야 : 로봇 제어. 물리 시뮬레이션.<br>

<br>

### (4-7) PETs
▣ 정의 : PETs는 여러 환경 모델의 앙상블을 사용하여 샘플링 기반 예측을 수행하는 알고리즘입니다. 모델 불확실성을 감소시키기 위해 앙상블 접근법을 사용하며, 예측 결과를 정책 학습에 활용합니다.<br>
▣ 장점 : 모델 불확실성을 효과적으로 처리. 샘플 효율성과 안정성이 높음.<br>
▣ 단점 : 여러 모델을 학습해야 하므로 계산 비용이 큼. 모델 앙상블 관리가 복잡.<br>
