...
### (3-3) MuZero
▣ 정의 : MuZero는 AlphaZero의 확장 버전으로, 환경 모델을 사전에 제공받는 대신 학습 과정에서 환경 모델을 스스로 학습하는 알고리즘. 모델 기반 학습과 모델 프리 학습을 결합해 환경 모델이 없는 경우에도 높은 성능을 발휘<br>
▣ 장점 : 환경 모델 불필요: AlphaZero와 달리 환경 모델이 없어도 동작 가능. 일반화 가능성: 규칙 기반 게임뿐 아니라 다양한 환경에 적용 가능. 효율적 학습: 모델 기반 예측을 통해 데이터 효율성을 극대화.<br>
