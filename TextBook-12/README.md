...
▣ 주요 알고리즘 : Q값을 0으로 초기화후 현재 상태에서 가능한 행동을 선택 (탐험/탐색 균형), 보상을 받고 다음 상태로 이동, Q값을 업데이트. 종료 상태에 도달할 때까지 반복.<br>
▣ python 예제 : 
...
▣ 주요 알고리즘 : 초기화: 개체군을 무작위로 생성. 적응도 계산: 각 개체의 적응도를 계산. 선택: 적응도에 따라 부모 개체를 선택. 교차 및 돌연변이: 자손을 생성. 적응도 평가 후, 최적화될 때까지 반복.<br>
▣ python 예제 : 
