...
## [1-7] 데이터 중복 제거(Data Deduplication)
▣ 정의 : 데이터 중복 제거는 동일하거나 유사한 데이터를 탐지하고 제거하여 데이터셋의 일관성과 정확성을 높이는 과정<br>
...
## [1-8] 데이터 변환(Data Transformation)
▣ 정의:
데이터 변환은 데이터를 모델에 적합한 형식으로 조정하거나, 성능 최적화를 위해 데이터를 변형하는 과정입니다.

▣ 필요성:
데이터 분포를 조정하여 학습 알고리즘의 성능 최적화.
입력 데이터가 알고리즘의 요구사항에 맞도록 준비.
이상치, 불균형 데이터 등의 영향을 최소화.
▣ 주요 기법:
수치형 변환:
로그 변환: 비대칭 분포를 정규 분포로 조정.
스케일링: Min-Max, 표준화(Standardization).
범주형 변환:
원-핫 인코딩: 범주 데이터를 이진 변수로 변환.
라벨 인코딩: 범주를 정수로 매핑.
차원 축소:
PCA, t-SNE.
▣ 장점:
데이터의 분포를 정규화하여 학습 효과 증가.
다양한 알고리즘에서 안정적인 성능 확보.
해석 가능성을 높여 데이터 이해도 향상.
▣ 단점:
적절한 변환 기법 선택이 어려울 수 있음.
원본 데이터의 의미가 왜곡될 가능성.
고차원 데이터에서는 변환 비용 증가.
▣ 적용대상 알고리즘:
회귀 모델, 딥러닝, PCA, SVM 등.
...
## [1-9] 특성 엔지니어링(Feature Engineering)
▣ 정의:
특성 엔지니어링은 모델 성능을 최적화하기 위해 데이터를 변형하거나 새로운 특성을 생성하는 과정입니다. 특성 생성, 변환, 선택 등의 작업이 포함됩니다.

▣ 필요성:
고품질 특성은 모델의 성능을 크게 향상.
데이터의 의미를 반영하여 복잡한 패턴을 학습할 수 있도록 도움.
특성 중요도를 평가하여 불필요한 변수 제거 가능.
▣ 주요 기법:
특성 생성:
수학적 조합, 도메인 지식을 활용한 파생 특성 생성.
특성 변환:
로그 변환, 다항식 변환.
특성 선택:
모델 기반 선택 (Lasso, XGBoost).
중요도 평가 (LIME, SHAP).
차원 축소:
PCA, t-SNE.
▣ 장점:
모델 성능을 큰 폭으로 개선 가능.
도메인 지식을 반영하여 더 나은 해석 가능.
중요하지 않은 특성을 제거해 학습 속도 향상.
▣ 단점:
높은 도메인 지식 요구.
시간과 자원 소모.
잘못된 특성 생성은 모델 성능 저하.
▣ 적용대상 알고리즘:
선형 모델, 의사결정 나무, 랜덤 포레스트, 딥러닝 등 대부분의 알고리즘.
<br>
