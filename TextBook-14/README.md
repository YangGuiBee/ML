...
#  14 : CASE Study
# "머신러닝 모델에 대한" 또는 "머신러닝 모델을 사용한" 논문 1편을 찾아서 정리 및 발표

---

▣ 논문 검색 사이트 : https://www.dbpia.co.kr/  등
▣ 논문 검색 조건 : 최근(2025~2026년), 석박사 학위논문, 키워드(머신러닝+본인이 관심있는 모델 또는 주제)

### 보고서
아래 항목을 포함하여 hwp나 doc 파일로 3페이지 이내로 작성(세로)
보고서 파일명 : 14장(보고서)_이름(학번).hwp/doc
(1) 제목 : 검색한 논문의 제목, 저자, 연도, 발행처, 검색한 조건
(2) 개요 : 해당 논문의 연구 필요성, 목적, 수행절차
(3) 데이터 : 해당 논문에서 사용한 데이터 종류, 건수, 수집방법, 전처리 방법
(4) 모델 : 해당 논문에 포함된 머신러닝 모델
(5) 결과 : 해당 논문의 연구 결과 및 본인 생각한 해당 논문의 특징/평가

### 발표파일
위 보고서 내용 중 (5)번의 내용을 집중적으로 ppt파일로 1페이지로 작성하여 발표(가로)
발표자료 파일명 : 14장(발표)_이름(학번).ppt
