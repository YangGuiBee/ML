...
|모델명|이론 개요|특징|
|---|---|---|
|[1-1] 선형 판별 분석 (LDA)|다변량 정규분포 기반 판별 이론<br>→ 모든 클래스가 동일한 공분산을 가진다는 가정 하에 사후확률로 분류|기본 생성 모델을 사용<br>→ 클래스 간 공분산을 공유하여 결정 경계가 선형으로 제한됨|
|[1-2] 이차 판별 분석 (QDA)|다변량 정규분포 기반 판별 이론<br>→ 클래스별로 서로 다른 공분산을 허용하여 사후확률로 분류|LDA를 기본 모델로 사용<br>→ 클래스별 공분산을 허용하여 비선형(이차) 결정 경계 형성|
...
|[1-2] 이차 판별 분석 (QDA)|$d_k(x)=(x-\mu_k)^T A_k (x-\mu_k)+b_k$|$d_k(x)$: 입력 $x$가 클래스 $k$에 속할 판별함수<br>$x$: 입력 특성 벡터 $(x \in \mathbb{R}^d)$<br>$\mu_k$: 클래스 $k$의 평균 벡터<br>$A_k$: 클래스 $k$의 공분산 행렬의 역행렬에 비례하는 대칭 행렬<br>$b_k$: 클래스 $k$에 대한 상수 항(사전확률 포함 가능)<br>$T$: 전치연산(벡터의 행·열을 바꿈)|이차 결정 경계 (타원·포물면 등)|의료 영상 분류<br>패턴 인식|
