...
| 알고리즘                                                           | 분류/회귀 | 적용 분야                                                 |
| -------------------------------------------------------------- | ----- | ----------------------------------------------------- |
| **[1-1] LDA (Linear Discriminant Analysis)**                   | 분류    | - 얼굴 인식, 질병 분류 등 (문제에 따라 다르지만, 주로 클래스가 두 개 이상인 분류 문제) |
|                                                                |       | - 텍스트 분류 (문서 분류, 스팸 이메일 탐지 등)                         |
|                                                                |       | - 음성 인식 및 패턴 인식                                       |
| **[1-2] QDA (Quadratic Discriminant Analysis)**                | 분류    | - 얼굴 인식, 의학적 질병 예측 (LDA보다 비선형적인 데이터 구조에 적합)           |
|                                                                |       | - 재무 리스크 평가 (분포가 비선형인 경우)                             |
| **[2-1] 결정 트리 (Decision Tree)**                                | 분류/회귀 | - 고객 이탈 예측, 마케팅 분석                                    |
|                                                                |       | - 의사결정 지원 시스템                                         |
|                                                                |       | - 의료 진단 및 의학적 예측                                      |
|                                                                |       | - 금융 및 투자 분석 (신용카드 사기 탐지 등)                           |
| **[2-2] 랜덤 포레스트 (Random Forest)**                              | 분류/회귀 | - 이미지 분석 (객체 인식, 얼굴 인식 등)                             |
|                                                                |       | - 유전자 데이터 분석                                          |
|                                                                |       | - 금융 분야 (리스크 분석, 주식 예측)                               |
|                                                                |       | - 대규모 데이터셋에서의 예측 (과적합 방지)                             |
| **[3-1] k-최근접 이웃 (k-Nearest Neighbors, K-NN)**                 | 분류/회귀 | - 추천 시스템 (콘텐츠 추천, 상품 추천 등)                            |
|                                                                |       | - 질병 예측 및 진단 (의료 데이터를 기반으로)                           |
|                                                                |       | - 이미지 분류 및 영상 처리                                      |
|                                                                |       | - 음성 인식 (음성 데이터를 특징으로 분류)                             |
| **[3-2] 서포트 벡터 머신 (Support Vector Machine, SVM)**              | 분류/회귀 | - 텍스트 분류 (문서 분류, 스팸 탐지 등)                             |
|                                                                |       | - 얼굴 인식, 음성 인식                                        |
|                                                                |       | - 이미지 분류, 물체 인식                                       |
|                                                                |       | - 의료 분야 (질병 분류 및 예측, 유전자 분석)                          |
| **[4-1] PCR (Principal Component Regression)**                 | 회귀    | - 경제학 및 재무 데이터 분석 (변수 간의 상관 관계가 큰 경우)                 |
|                                                                |       | - 화학적 분석 (분석 데이터 차원이 큰 경우)                            |
|                                                                |       | - 기후 예측 및 환경 데이터 분석                                   |
| **[4-2] PLS (Partial Least Squares)**                          | 회귀    | - 화학 및 생물학적 데이터 분석 (예: 스펙트럼 분석, 유전자 데이터 분석)           |
|                                                                |       | - 고객 행동 분석 (고객의 특성과 구매 행동을 예측)                        |
|                                                                |       | - 경제학 및 환경 모델링                                        |
| **[4-3] PLS-DA (Partial Least Squares Discriminant Analysis)** | 분류    | - 유전자 데이터 분석, 분자 생물학 연구                               |
|                                                                |       | - 화학 분석, 약물 반응 예측                                     |
|                                                                |       | - 마케팅 분석 (고객 세분화 및 타겟 마케팅)                            |
| **[4-4] Supervised PCA**                                       | 회귀/분류 | - 데이터 차원 축소 후 회귀 분석을 통한 예측 (예: 고차원 데이터에서 성능 향상)       |
|                                                                |       | - 이미지 및 음성 데이터 분석 (차원 축소 후 분석)                        |
|                                                                |       | - 금융 데이터 분석, 의료 데이터 분석 등 차원 축소가 필요한 다양한 분야            |
