...
| 개요 | 손실함수의 gradient를 이용해 결정트리를 순차적으로 학습하는 기본 부스팅 모델 | 정규화·2차 미분·병렬화 등을 도입해 실무 성능을 극대화한 GBM | 히스토그램·Leaf-wise 성장 전략으로 속도와 확장성에 특화 |
| 트리 성장 방식 | Level-wise (깊이 기준 분할) | Level-wise (깊이 기준 분할) | **Leaf-wise (손실 감소 최대 leaf부터 분할)** |
| 핵심 특징 | 구조 단순, 이론적 해석 용이 | L1/L2 정규화, Hessian 활용, 병렬 처리, 결측치 자동 처리 | Histogram 기반, GOSS, EFB, Leaf-wise 성장 |
| 정규화 | 약함 | **강력 (L1/L2)** | 중간 |
| 학습 속도 | 느림 | 빠름 | **매우 빠름** |
| 대규모 데이터 적합성 | 부적합 | 적합 | **매우 우수** |
| 장점 | • 원리 이해 용이<br>• 통계적 해석 명확<br>• 소규모 데이터 안정적 | • 높은 예측 성능<br>• 과적합 억제 우수<br>• 실무·대회 표준 | • 학습 속도 최상<br>• 메모리 효율 우수<br>• 초대형 데이터 처리 가능 |
| 단점 | • 속도 느림<br>• 과적합 위험<br>• 확장성 부족 | • 하이퍼파라미터 많음<br>• 메모리 사용량 큼 | • 과적합 위험 (Leaf-wise)<br>• 소규모 데이터 불안정<br>• 해석 어려움 |
| 모델 해석 용이성 | 높음 | 중간 | 낮음 |
| 주요 사용 맥락 | • 교육용 모델<br>• 개념 설명<br>• 소규모 실험 연구 | • 정형 데이터 예측<br>• Kaggle/실무 모델<br>• 성능 중심 연구 | • 산업용 대규모 ML<br>• 로그/추천/CTR 예측 |
| 대표 라이브러리 | sklearn | xgboost | lightgbm |
...
|모델명|이론 개요|특징|
|---|---|---|
|[3-1] k-최근접 이웃(K-NN)|사례 기반 학습 이론<br>→입력 데이터와 가장 가까운 k개의 이웃을 기준으로 예측|기본 모델은 최근접 이웃(1-NN)<br>→단일 이웃의 불안정성을 k개 다수결로 완화한 비모수 모델|
|[3-2] 서포트 벡터 머신(SVM)|구조적 위험 최소화 이론<br>→마진을 최대화하는 결정 초평면을 학습|기본 모델은 선형 분류기<br>→마진 개념과 서포트 벡터만으로 일반화 성능을 강화|

|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|[3-1] k-최근접 이웃(K-NN)|$d(x,x_i)=(x-x_i)^T(x-x_i)$|$x$: 입력 특성 벡터<br>$x_i$: 학습 데이터의 i번째 샘플<br>$T$: 벡터의 전치(transpose)<br>$d(x,x_i)$: 두 샘플 간 유클리드 거리의 제곱|데이터 분포에 따라 결정 경계가 국소적으로 형성됨|추천 시스템, 패턴 인식, 소규모 의료 진단 데이터|
|[3-2] 서포트 벡터 머신(SVM)|$f(x)=w^T x+b$|$x$: 입력 특성 벡터<br>$w$: 가중치 벡터<br>$T$: 벡터의 전치(transpose)<br>$w^T x$: 두 벡터의 내적(inner product)<br>$b$: 편향(bias)<br>$f(x)$: 결정 함수|초평면과 마진, 경계에 위치한 서포트 벡터 중심 구조|텍스트 분류, 고차원 바이오 데이터, 이상 탐지|
...
|모델명|sk-learn 사용 예제 소스|최적의 데이터셋|
|---|---|---|
|[3-1] k-최근접 이웃(K-NN)|KNeighborsClassifier / knn.fit() / accuracy_score|Iris Dataset (UCI ML Repository)|
...
PCR: 반응변수 무관하게 PCA만 수행 → 정보 손실 가능<br>
PLS (연속형 회귀): 반응변수와의 관계를 고려하여 잠재변수 선택 → PCR 보다 예측력 높음(반응변수와 설명변수 간 공분산 최대화)<br>
PLS-DA(범주형 분류): 반응변수와의 관계를 고려하여 잠재변수 선택 → PCR 보다 예측력 높음(클래스 간 분리를 최대화하도록 잠재변수 추출)<br>
Supervised PCA: PCA와 유사하지만 반응변수 정보를 명시적으로 포함<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-11/images/PCRs.png'> 
<br>


|모델명|이론 개요|특징|
|---|---|---|
|[4-1] 주성분 회귀(PCR)|차원 축소 이론(PCA)<br>→주성분으로 회귀를 수행하는 2단계 회귀 모델|기본 모델은 선형 회귀<br>→설명변수 분산만 고려하여 주성분을 추출한 뒤 회귀|
|[4-2] 부분 최소제곱 회귀(PLS)|공분산 최대화 이론<br>→설명변수와 반응변수를 동시에 고려한 잠재변수 회귀|PCR 대비 PCA를 수정<br>→반응변수와의 공분산을 최대화하는 방향으로 성분 추출|
|[4-3] 부분 최소제곱 판별 분석(PLS‑DA)|PLS + 분류 이론<br>→PLS를 분류 문제에 적용한 판별 모델|기본 모델은 PLS 회귀<br>→연속 반응변수를 범주형 반응으로 변형하여 분류에 적용|
|[4-4] 감독 주성분 분석(Supervised PCA)|감독 학습 기반 PCA 이론<br>→반응변수와 관련된 변수만으로 PCA 수행|기본 모델은 PCA<br>→반응변수와 무관한 변수를 사전 제거 후 차원 축소|

|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|[4-1] 주성분 회귀(PCR)|$z_k=X w_k$|$X$: 설명변수 행렬<br>$w_k$: k번째 주성분 방향<br>$z_k$: 주성분 점수|설명변수 분산 기준의 직교 성분 축|다중공선성이 강한 화학·공정 데이터|
|[4-2] 부분 최소제곱 회귀(PLS)|$t_k=X r_k$|$X$: 설명변수<br>$r_k$: 공분산 기반 가중치<br>$t_k$: 잠재변수|X–Y 공분산을 반영한 잠재공간|스펙트럼 분석, 공정 예측|
|[4-3] 부분 최소제곱 판별 분석(PLS‑DA)|$y=t c$|$t$: 잠재변수<br>$c$: 클래스 계수<br>$y$: 클래스 반응값|클래스 분리를 강조한 잠재 공간|오믹스 데이터 분류, 생물정보학|
|[4-4] 감독 주성분 분석(SupervisedPCA)|$z=X_s w$|$X_s$: 선택된 변수 행렬<br>$w$: 주성분 방향<br>$z$: 감독 주성분|반응변수 관련 축만 남긴 PCA 구조|고차원 유전자 데이터 탐색|
...
|모델명|sk-learn 사용 예제 소스|최적의 데이터셋|
|---|---|---|
|[4-1] 주성분 회귀(PCR)|PCA, LinearRegression / model.fit() / r2_score|Hitters Dataset (ISLR)|
|[4-2] 부분 최소제곱 회귀(PLS)|PLSRegression / pls.fit() / r2_score|Wine Quality Dataset (UCI)|
|[4-3] 부분 최소제곱 판별 분석(PLS‑DA)|PLSRegression, LabelBinarizer / pls.fit() / accuracy_score|Wine Dataset (UCI)|
|[4-4] 감독 주성분 분석(Supervised PCA)|SelectKBest, PCA, LogisticRegression / model.fit() / accuracy_score|Golub Leukemia Dataset|
...
▣ 단점 : 주성분 선택 시 ‘변동성(variance)’ 큰 주성분이 반드시 예측력(종속변수 설명력)이 높은 것은 아니라는 점에서, 중요한 정보가 사라질 가능성. 비지도 방식의 PCA를 먼저 수행하므로, 종속변수 𝑦정보가 주성분 선정에 반영되지 않아 예측력이 떨어질 가능성<br>
...
|[5-2] 합성곱(CNN)/순환(RNN) 신경망|국소 연결·순차 모델 이론<br>→공간·시간 구조를 반영한 신경망|기본 모델은 MLP<br>→CNN은 지역 패턴, RNN은 시계열 의존성 반영|
|[5-3] 트랜스포머(Transformer)|자기주의 메커니즘 이론<br>→전역 의존성을 병렬적으로 학습하는 모델|기본 모델은 RNN<br>→순차 계산 제거, 장기 의존성 학습 강화|
|[5-4] 오토인코더(AE)|표현 학습 이론<br>→입력을 재구성하며 잠재 표현을 학습|기본 모델은 MLP<br>→목표변수 없이 차원 축소와 특징 학습 수행|

|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|[5-1] 다층 퍼셉트론(MLP)|$y=f(Wx+b)$|$x$: 입력<br>$W$: 가중치<br>$b$: 편향<br>$f$: 활성함수<br>$y$: 출력|완전연결 계층이 순차적으로 연결됨|일반적인 분류·회귀 문제|
|[5-2] 합성곱(CNN)/순환(RNN) 신경망|$h=f(Wx)$|$x$: 입력<br>$W$: 필터 또는 순환 가중치<br>$h$: 은닉상태|CNN은 격자형, RNN은 순환 구조|영상 인식, 음성·시계열 분석|
|[5-3] 트랜스포머(Transformer)|$z=xW$|$x$: 입력 토큰 표현<br>$W$: 변환 가중치<br>$z$: 변환된 표현|자기주의 기반 병렬 연결 구조|자연어 처리, 대규모 언어 모델|
|[5-4] 오토인코더(AE)|$\hat{x}=g(f(x))$|$x$: 입력<br>$f$: 인코더<br>$g$: 디코더<br>$\hat{x}$: 재구성 입력|인코더–디코더 대칭 구조|차원 축소, 이상 탐지, 표현 학습|
...
|모델명|sk-learn 사용 예제 소스|최적의 데이터셋|
|---|---|---|
|[5-1] 다층 퍼셉트론(MLP)|MLPClassifier / mlp.fit() / accuracy_score|MNIST (OpenML)|
|[5-2] 합성곱(CNN)/순환(RNN) 신경망|MLPClassifier(대체) / model.fit() / accuracy_score|Fashion-MNIST (OpenML)|
|[5-3] 트랜스포머(Transformer)|SGDClassifier(대체) / model.fit() / accuracy_score|20 Newsgroups (sklearn)|
|[5-4] 오토인코더(AE)|MLPRegressor / ae.fit() / mean_squared_error|MNIST (OpenML)|
