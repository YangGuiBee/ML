...
| **[1-2] QDA (Quadratic Discriminant Analysis)**                | ( \delta_k(x)=-\tfrac12 \log\lvert \Sigma_k\rvert -\tfrac12 (x-\mu_k)^\top \Sigma_k^{-1}(x-\mu_k)+\log \pi_k ) | 공분산이 클래스별로 다른 분류, 생물정보, 금융 리스크     |
| **[2-1] PCR (Principal Component Regression)**                 | ( Z = X W_{\text{PCA}},\ \ \hat{y}=Z\hat{\beta} )                                                              | 다중공선성 완화 회귀, 스펙트럼 분석, 공정 데이터 예측    |
| **[2-2] PLS (Partial Least Squares)**                          | ( X=TP^\top+E,\ Y=UQ^\top+F,\ \ \max \operatorname{Cov}(T,U) )                                                 | X–Y 상관이 높은 예측, 화학계량학, 공정 모니터링      |
| **[2-3] PLS-DA (Partial Least Squares Discriminant Analysis)** | ( Y\in{0,1,\dots},\ \ T=XW,\ \ \max \operatorname{Cov}(T,Y) )                                                  | 다중 클래스 분류, 오믹스 분석, 품질 검사, 바이오마커 탐색 |
| **[2-4] Supervised PCA**                                       | ( \max_{w:\ \lVert w\rVert=1}\ \operatorname{Corr}(Xw,\ y) ) (라벨 연관성 가중 후 PCA 수행)                              | 라벨 정보 활용 차원축소, 이미지/텍스트 분류 전처리      |
| **[3-1] 결정 트리 (Decision Tree)**                                | ( I(t)= -\sum_i p_i(t)\log p_i(t) ) (엔트로피 또는 지니지수 기준 분할)                                                       | 분류·회귀, 변수 중요도 분석, 해석 용이한 모델        |
