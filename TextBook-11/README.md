...
|---|---|---|---|---|
|[1-1] 선형 판별 분석 (LDA)|$d_k(x)=x^T w_k+c_k$|$d_k(x)$: 입력 $x$가 클래스 $k$에 속할 판별함수<br>$x$: 입력 특성 벡터 $(x \in \mathbb{R}^d)$<br>$w_k$: 클래스 $k$에 대한 판별 가중치 벡터<br>$c_k$: 클래스 $k$의 절편(bias) 항<br>$T$: 전치(transpose) 연산으로, 벡터의 행·열을 바꿈|선형 결정 경계 (초평면)|텍스트 분류<br>고차원·소표본 문제|
|[1-2] 이차 판별 분석 (QDA)|$d_k(x)=(x-\mu_k)^T A_k (x-\mu_k)+b_k$|$d_k(x)$: 입력 $x$가 클래스 $k$에 속할 판별함수<br>$x$: 입력 특성 벡터 $(x \in \mathbb{R}^d)$<br>$\mu_k$: 클래스 $k$의 평균 벡터<br>$A_k$: 클래스 $k$의 공분산 행렬의 역행렬에 비례하는 대칭 행렬<br>$b_k$: 클래스 $k$에 대한 상수 항(사전확률 포함 가능)<br>$T$: 전치(transpose) 연산|이차 결정 경계 (타원·포물면 등)|의료 영상 분류<br>패턴 인식|
