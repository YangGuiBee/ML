...
|[4-4] 감독 주성분 분석(PCA)|감독 학습 기반 PCA 이론<br>→반응변수와 관련된 변수만으로 PCA 수행|기본 모델은 PCA<br>→반응변수와 무관한 변수를 사전 제거 후 차원 축소|
...
|[4-4] 감독 주성분 분석(PCA)|$z=X_s w$|$X_s$: 선택된 변수 행렬<br>$w$: 주성분 방향<br>$z$: 감독 주성분|반응변수 관련 축만 남긴 PCA 구조|고차원 유전자 데이터 탐색|
...
|[4-4] 감독 주성분 분석(PCA)|SelectKBest, PCA, LogisticRegression / model.fit() / accuracy_score|Golub Leukemia Dataset|
