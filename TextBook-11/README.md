...
데이터가 여러 그룹으로 나뉘어 있을 때, 각 그룹의 확률 분포(특히 평균과 공분산)를 이용해서 “이 점은 어떤 그룹에서 나올 가능성이 가장 높을까?”를 계산하는 방식(가정 : 각 클래스의 공분산이 다를 수 있다. 모양이 다른 2차식 곡선)<br>
...
| :------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------- | -------------------------------------------------------- | ------------------------------ |
| **[1-1] LDA (Linear Discriminant Analysis)**                   | ![lda](https://latex.codecogs.com/svg.image?\delta_k\(x\)=x^\top\Sigma^{-1}\mu_k-\tfrac12\mu_k^\top\Sigma^{-1}\mu_k+\log\pi_k)                                                                                                                                                                                                                        | 다중 클래스 분류, 얼굴 인식, 문서 분류, 의료 데이터    |                                                          |                                |
| **[1-2] QDA (Quadratic Discriminant Analysis)**                | ![qda]([https://latex.codecogs.com/svg.image?\delta_k(x)=-\tfrac12\log](https://latex.codecogs.com/svg.image?\delta_k%28x%29=-\tfrac12\log)                                                                                                                                                                                                           | \Sigma_k                           | -\tfrac12(x-\mu_k)^\top\Sigma_k^{-1}(x-\mu_k)+\log\pi_k) | 공분산이 클래스별로 다른 분류, 생물정보, 금융 리스크 |
| **[2-1] PCR (Principal Component Regression)**                 | ![pcr]([https://latex.codecogs.com/svg.image?Z=XW_{PCA},\](https://latex.codecogs.com/svg.image?Z=XW_{PCA},\) \hat{y}=Z\hat{\beta})                                                                                                                                                                                                                   | 다중공선성 완화 회귀, 스펙트럼 분석, 공정 데이터 예측    |                                                          |                                |
| **[2-2] PLS (Partial Least Squares)**                          | ![pls]([https://latex.codecogs.com/svg.image?X=TP^\top+E,\](https://latex.codecogs.com/svg.image?X=TP^\top+E,\) Y=UQ^\top+F,\ \max\operatorname{Cov}(T,U))                                                                                                                                                                                            | X–Y 상관이 높은 예측, 화학계량학, 공정 모니터링      |                                                          |                                |
| **[2-3] PLS-DA (Partial Least Squares Discriminant Analysis)** | ![plsda]([https://latex.codecogs.com/svg.image?Y\in\{0,1,\dots\},\](https://latex.codecogs.com/svg.image?Y\in\{0,1,\dots\},\) T=XW,\ \max\operatorname{Cov}(T,Y))                                                                                                                                                                                     | 다중 클래스 분류, 오믹스 분석, 품질 검사, 바이오마커 탐색 |                                                          |                                |
| **[2-4] Supervised PCA**                                       | ![spca]([https://latex.codecogs.com/svg.image?\max_{w:\|w\|=1}\operatorname{Corr}(Xw,\](https://latex.codecogs.com/svg.image?\max_{w:\|w\|=1}\operatorname{Corr}%28Xw,\) y)) (라벨 연관성 가중 후 PCA 수행)                                                                                                                                                     | 라벨 정보 활용 차원축소, 이미지/텍스트 분류 전처리      |                                                          |                                |
| **[3-1] 결정 트리 (Decision Tree)**                                | ![tree]([https://latex.codecogs.com/svg.image?I(t)=-\sum_i](https://latex.codecogs.com/svg.image?I%28t%29=-\sum_i) p_i(t)\log p_i(t)) (엔트로피 또는 지니지수 기준 분할)                                                                                                                                                                                            | 분류·회귀, 변수 중요도 분석, 해석 용이한 모델        |                                                          |                                |
| **[3-2] 랜덤 포레스트 (Random Forest)**                              | ![rf](https://latex.codecogs.com/svg.image?\hat{y}\(x\)=\tfrac1B\sum_{b=1}^Bh_b\(x\))                                                                                                                                                                                                                                                                 | 대규모 데이터 분류·회귀, 변수 중요도 평가, 이상 탐지    |                                                          |                                |
| **[4-1] K-NN (k-Nearest Neighbors)**                           | 분류: ![knn1]([https://latex.codecogs.com/svg.image?\hat{y}=\operatorname{mode}\{y_i:x_i\in](https://latex.codecogs.com/svg.image?\hat{y}=\operatorname{mode}\{y_i:x_i\in) N_k(x)}) <br> 회귀: ![knn2]([https://latex.codecogs.com/svg.image?\hat{y}=\tfrac1k\sum_{x_i\in](https://latex.codecogs.com/svg.image?\hat{y}=\tfrac1k\sum_{x_i\in) N_k(x)}y_i) | 패턴 인식, 추천 시스템, 비모수 근접 기반 예측        |                                                          |                                |
| **[4-2] SVM (Support Vector Machine)**                         | ![svm]([https://latex.codecogs.com/svg.image?\min_{w,b}\tfrac12\|w\|^2\text{s.t](https://latex.codecogs.com/svg.image?\min_{w,b}\tfrac12\|w\|^2\text{s.t). }y_i(w^\top x_i+b)\ge1)                                                                                                                                                                    | 이진/다중 분류, 고차원 텍스트/이미지, 생체 신호 분석    |                                                          |                                |
