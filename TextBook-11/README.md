...
|[1-2] 이차 판별 분석|$d_k(x)=-\frac{1}{2}(x-\mu_k)^T S_k^{-1}(x-\mu_k)-\frac{1}{2}\log|S_k|+\log\pi_k$|$x$: 입력 특성 벡터<br>$\mu_k$: 클래스 $k$의 평균 벡터<br>$S_k$: 클래스 $k$의 공분산 행렬<br>$\pi_k$: 클래스 $k$의 사전확률|이차 결정 경계|의료 영상 분류<br>패턴·생체 신호 인식|
