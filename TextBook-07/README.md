...
| **레버리지 (Leverage)**       | 0에서 멀수록 강함(양수=양의 연관)             | **> 0.05**: 강함 / **0.01–0.05**: 유의미 / **≈ 0**: 거의 무관                       | 절대적 “추가 동시발생 비율”. **빈도 민감도**가 낮아 lift 보완에 유용. 표본 커질수록 작은 값도 의미. |
| **확신도 (Conviction)**      | 1보다 클수록 좋음(규칙 위반률 감소)            | **> 2.0**: 강함 / **1.2–2.0**: 유의미 / **≈1.0**: 독립                            | **confidence=1**이고 **P(B)<1**이면 **∞**. 후건이 드문 규칙에서 왜곡 가능.       |
| **상관계수 (Correlation, φ)** | **∣φ∣**(=abs(φ)) 클수록 강함 · 부호로 방향 | **∣φ∣ ≥ 0.5**: 강함 / **0.3–0.49**: 중간 / **0.1–0.29**: 약함 / **< 0.1**: 매우 약함 | 대칭 지표(순서 무관). **음수(φ<0)**는 상호 배타 성향 의미. 빈도·희소성 영향은 중간 정도.       |
