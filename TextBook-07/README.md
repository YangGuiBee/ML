...
| 지표 | 수식 | 설명 |
|---|---|---|
| **[5.1] Inception Score (IS)** | $IS=\exp(\sum_y p(y\mid x)\log\frac{p(y\mid x)}{p(y)})$ | 생성 이미지의 품질(명확성)과 다양성을 동시에 평가 |
| **[5.2] Fréchet Inception Distance (FID)** | $FID=\|\mu_r-\mu_g\|^2+\mathrm{Tr}(\Sigma_r+\Sigma_g-2(\Sigma_r\Sigma_g)^{1/2})$ | 실제 분포와 생성 분포 간 거리 |
| **[5.3] Kernel Inception Distance (KID)** | $KID=\mathrm{MMD}^2=\mathbb{E}[k(x,x')]+\mathbb{E}[k(y,y')]-2\mathbb{E}[k(x,y)]$ | MMD 기반 분포 거리 |
| **[5.4] Precision & Recall (Distributions)** | $Precision=\frac{\|G\cap R\|}{\|G\|}$, $Recall=\frac{\|G\cap R\|}{\|R\|}$ | Precision=품질, Recall=다양성 |
| **[5.5] SSIM** | $SSIM(x,y)=\frac{(2\mu_x\mu_y+C_1)(2\sigma_{xy}+C_2)}{(\mu_x^2+\mu_y^2+C_1)(\sigma_x^2+\sigma_y^2+C_2)}$ | 구조적 유사도 |
| **[5.6] PSNR** | $PSNR=10\log_{10}\frac{MAX^2}{MSE}$ | 재구성 품질 평가 |
| **[5.7] LPIPS** | $LPIPS=\sum_l\|\phi_l(x)-\phi_l(y)\|^2$ | 지각적 유사도 |
| **[5.8] ELBO** | $ELBO=\log p(x)-D_{KL}(q(z\mid x)\parallel p(z))$ | VAE 학습 목적함수 |
| **[5.9] Reconstruction Loss** | $\mathcal{L}_{rec}=\|x-\hat{x}\|^2$ | 재구성 오차 |
| **[5.10] Mode Score** | $MS=\exp(D_{KL}(p(y\mid x)\parallel p(y))-D_{KL}(p(y)\parallel p_{data}(y)))$ | IS의 mode collapse 보완 |
| **[5.11] Coverage** | $Coverage=\frac{\|R_{covered}\|}{\|R\|}$ | 실제 데이터 모드 포함 비율 |
| **[5.12] Perplexity** | $Perplexity=\exp(-\frac{1}{N}\sum_{i=1}^N\log p(x_i))$ | 확률모형 불확실성 |
| **[5.13] Linear Probe Accuracy** | $Acc=\frac{1}{N}\sum_{i=1}^N\mathbb{I}(\hat{y}_i=y_i)$ | 표현 품질 평가 |
| **[5.14] k-NN Classification Accuracy** | $Acc_{kNN}=\frac{1}{N}\sum_{i=1}^N\mathbb{I}(y_i=\mathrm{mode}(N_k(x_i)))$ | 임베딩 공간 구조 평가 |
