...
▣ 베이지안 최적화(Bayesian Optimization) 단계 : 미지의 함수가 반환하는 값의 최소 또는 최대값을 만드는 최적의 해를 짧은 반복을 통해 찾아내는 최적화 방식<br> 
 (1단계) 최초에는 랜덤하게 하이퍼파라미터들을 샘플링하여 성능결과를 관측<br> 
 ![](./images/step1.png)
 
 <br>
 (2단계) 관측된 값을 기반으로 대체모델의 최적함수를 추정<br> 
 ![](./images/step2.png)

 <br>
 (3단계) 획득함수에서 다음으로 관측할 하이퍼파라미터를 추출<br> 
 ![](./images/step3.png)
 
 <br>
 (4단계) 해당 하이퍼파라미터로 관측된 값을 기반으로 다시 대체모델의 최적함수 추정<br>
 ![](./images/step4.png)
