...
| 항목 | 내용 |
|------|------|
| **구성요소** | 데이터 공간을 격자(grid) 형태로 변환한 후, 웨이블릿 변환(wavelet transform)을 적용하여 밀도 변화 패턴 탐색 |
| **거리함수** | 명시적 거리함수 대신, 웨이블릿 필터를 통해 격자 셀 간 밀도 차이를 추출 |
| **목적함수** | 고주파(High-frequency) 신호 제거 후 저주파(저밀도 영역)에서 밀도 극대 영역을 식별 |
| **중심갱신** | 웨이블릿 변환 결과에서 에너지(local maxima)가 높은 셀들을 병합하여 군집 중심으로 설정 |
| **목표** | 데이터 공간의 밀도 분포를 주파수 도메인에서 분석하여 잡음 제거 및 임의 형태의 군집 자동 식별 |
...
| 항목 | 내용 |
|------|------|
| **구성요소** | 다단계 격자 구조(Multi-resolution Grid) — 각 셀(Cell)에 통계 요약 정보(평균, 분산, 개수 등)를 저장 |
| **거리함수** | 셀 단위 통계치 기반 확률적 거리 — 유클리드 거리 대신 통계적 유사도 사용 |
| **목적함수** | 셀 통계 정보를 이용해 군집의 밀도와 분포를 추정 — 상위 레벨에서 하위 레벨로 점진적 세분화 |
| **중심갱신** | 각 셀의 요약 통계값을 갱신하며, 유사 셀들을 병합하여 상위 레벨 군집으로 형성 |
| **목표** | 통계 기반 격자 요약을 통해 빠르고 확장성 높은 군집화 수행 — 노이즈에 강하고 대용량 데이터 처리 가능 |
...
| 항목 | 내용 |
|------|------|
| **구성요소** | 격자 기반 + 부분공간(Subspace) 탐색 결합 — 고차원 데이터에서 밀도 기반 부분공간 군집을 탐색 |
| **거리함수** | 명시적 거리함수 대신 셀의 점유율(density) 기반 유사도 사용 |
| **목적함수** | 밀도 임계값 $\tau$ 이상인 격자 셀을 부분공간 후보로 탐색하여 연결된 셀을 군집으로 정의 |
| **중심갱신** | 각 부분공간 내 고밀도 셀을 연결하여 군집 형성 — 셀의 중심 또는 평균 좌표로 대표값 결정 |
| **목표** | 차원의 저주(Curse of Dimensionality)를 완화하며, 고차원 데이터의 유의미한 부분공간 클러스터 자동 탐색 |
...
| 항목 | 내용 |
|------|------|
| **구성요소** | 데이터 공간을 격자로 분할한 후, **경계점(gradient-based boundary)** 을 찾아 군집 분리 |
| **거리함수** | 유클리드 거리 기반 밀도 기울기(gradient) 계산<br>$\nabla f(x) = \frac{\partial f}{\partial x}$ 를 이용해 경계 검출 |
| **목적함수** | 격자 내 밀도 함수의 기울기 변화가 큰 경계선을 최소화 — 내부 밀도 균질성 극대화 |
| **중심갱신** | 각 밀도 영역(gradient basin)의 중심점을 평균 또는 모드로 갱신 |
| **목표** | 격자 공간 내에서 밀도 기울기를 활용해 군집 경계를 명확히 구분 — 경계 감지 기반의 효율적 자동 군집화 |
