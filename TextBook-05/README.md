...
| **거리함수** | 유클리드 거리 (제곱 거리)<br>$d(x_i, m_j) = \|x_i - m_j\|^2 = \sum_{k=1}^{p}(x_{ik} - m_{jk})^2$ |
| **목적함수** | 전체 제곱거리 최소화<br>$J = \sum_{j=1}^{K}\sum_{x_i \in C_j} \|x_i - m_j\|^2$ |
| **중심갱신** | 각 군집의 평균으로 갱신<br>$m_j = \frac{1}{|C_j|}\sum_{x_i \in C_j} x_i$ |
