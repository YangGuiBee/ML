...
|모델명|개요|비교|이론|적용분야|
|---|---|---|---|---|
|[2-1] Binomial Regression<br>(이항 회귀)|이항 반응변수의 성공 확률을 모델링하는 GLM|선형회귀를 이산 확률모형으로 확장|이항분포와 로짓 링크를 결합한 확률 회귀|임상시험 성공률, 품질 합격률|
|[2-2] Poisson Regression<br>(포아송 회귀)|단위당 발생 횟수를 모델링|Binomial의 희귀사건 극한 형태|평균과 분산이 동일한 count 분포 가정|사고 발생 건수, 콜 유입량|
|[2-3] Quasi-Poisson Regression<br>(준포아송 회귀)|과산포 count 데이터를 위한 보정 모델|Poisson의 평균 구조는 유지, 분산만 확장|분산에 스케일 계수를 도입한 준우도 접근|환경 데이터 발생 빈도|
|[2-4] Negative Binomial Regression<br>(음이항 회귀)|과산포를 구조적으로 반영한 count 모델|Poisson을 혼합분포로 일반화|포아송-감마 혼합으로 분산 증가 허용|의료 방문 횟수, 보험 사고 수|
|[2-5] Gamma Regression<br>(감마 회귀)|양의 연속형 반응변수 모델|정규회귀를 비대칭 연속 분포로 확장|감마분포 기반 평균-분산 연결|보험금 규모, 비용 분석|
|[2-6] Inverse Gaussian Regression<br>(역가우시안 회귀)|긴 꼬리를 갖는 양의 연속값 모델|Gamma보다 꼬리가 긴 분포 사용|평균의 세제곱에 비례하는 분산 구조|공정 소요시간, 생존시간|
|[2-7] Tweedie Regression<br>(트위디 회귀)|이산+연속 혼합 반응변수 모델|Poisson과 Gamma를 하나의 틀로 통합|분산을 평균의 거듭제곱으로 일반화|보험 순손실, 강수량|
|[2-8] Cox Proportional Hazard Model<br>(콕스 비례위험 모형)|시간-사건 데이터의 회귀 모형|GLM을 생존분석으로 확장|기저위험함수를 비모수로 분리|임상 생존분석, 신뢰성 평가|
|[2-9] Zero-Inflated Model<br>(영과잉 모형)|0이 과도한 count 데이터 처리|Poisson 또는 NB에 구조적 0 추가|이진 생성 과정과 count 과정 결합|사고 0건 지역 분석|
|[2-10] Hurdle Model<br>(허들 모형)|0과 양수 발생을 분리 모델링|Zero-Inflated에서 발생 구조를 명시적 분리|발생 여부와 크기를 단계적으로 모델링|소비 발생 여부와 금액|
|[2-11] Generalized Additive Model<br>(일반화 가법모형)|비선형 효과를 가법 구조로 표현|GLM의 선형항을 함수로 대체|변수별 스무딩 함수의 합|환경·역학 비선형 효과|
|[2-12] Bayesian GLM<br>(베이즈 GLM)|사전지식을 포함한 GLM|GLM에 베이즈 추론 도입|사후분포 기반 추론 체계|소표본 추론, 불확실성 분석|
|[2-13] Regularized GLM<br>(정규화 GLM)|고차원 회귀를 위한 GLM 확장|GLM에 벌점항 추가|편향-분산 절충 최적화|유전자 데이터, 텍스트 회귀|


|모델명|수식|수식의 항목 설명|그래프형태|
|---|---|---|---|
|[2-1] Binomial Regression<br>(이항 회귀)|$Y \sim \mathrm{Binomial}(n,p),\;\log\frac{p}{1-p}=X\beta$|$Y$: 성공 횟수, $n$: 시행 수, $p$: 성공확률, $X\beta$: 선형예측자|S자형 확률 곡선|
|[2-2] Poisson Regression<br>(포아송 회귀)|$Y \sim \mathrm{Poisson}(\lambda),\;\log(\lambda)=X\beta$|$\lambda$: 발생률, 로그 링크로 양수 보장|지수적 증가 곡선|
|[2-3] Quasi-Poisson Regression<br>(준포아송 회귀)|$\mathbb{E}(Y)=\mu,\;\mathrm{Var}(Y)=\phi\mu$|$\mu$: 평균, $\phi$: 과산포 계수|Poisson 대비 분산 확장|
|[2-4] Negative Binomial Regression<br>(음이항 회귀)|$\mathrm{Var}(Y)=\mu+\alpha\mu^2$|$\alpha$: 분산 증가율, 평균의 제곱 항 포함|두꺼운 꼬리 count 분포|
|[2-5] Gamma Regression<br>(감마 회귀)|$Y \sim \mathrm{Gamma}(k,\theta),\;\log(\mu)=X\beta$|$k$: 형태모수, $\theta$: 척도모수|우측 비대칭 연속 분포|
|[2-6] Inverse Gaussian Regression<br>(역가우시안 회귀)|$\mathrm{Var}(Y)=\phi\mu^3$|$\mu$: 평균, $\phi$: 분산 계수|매우 긴 우측 꼬리|
|[2-7] Tweedie Regression<br>(트위디 회귀)|$\mathrm{Var}(Y)=\phi\mu^p$|$p$: 분포 지수, $\phi$: 분산 계수|0 포함 연속 분포|
|[2-8] Cox Proportional Hazard Model<br>(콕스 비례위험 모형)|$h(t)=h_0(t)\exp(X\beta)$|$h(t)$: 위험함수, $h_0(t)$: 기저위험|시간에 따른 위험 곡선|
|[2-9] Zero-Inflated Model<br>(영과잉 모형)|$P(Y=0)=\pi+(1-\pi)f(0)$|$\pi$: 구조적 0 확률, $f$: count 분포|0에서 질량 집중|
|[2-10] Hurdle Model<br>(허들 모형)|$P(Y>0)=g(X\beta),\;Y^+ \sim f$|$g$: 발생 확률 모델, $Y^+$: 양의 반응|0 이후 분포 분리|
|[2-11] Generalized Additive Model<br>(일반화 가법모형)|$g(\mu)=\beta_0+\sum_j f_j(x_j)$|$f_j$: 비선형 스무딩 함수|부드러운 비선형 곡선|
|[2-12] Bayesian GLM<br>(베이즈 GLM)|$p(\beta\mid y)\propto p(y\mid\beta)p(\beta)$|$p(\beta)$: 사전분포, $p(\beta\mid y)$: 사후분포|분포 형태의 추정|
|[2-13] Regularized GLM<br>(정규화 GLM)|$\min\{-\ell(\beta)+\lambda\lVert\beta\rVert_p\}$|$\ell(\beta)$: 로그우도, $\lambda$: 규제강도, $p$: 규제 차수|계수 수축 곡선|
