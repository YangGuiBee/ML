...
|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|**[5-1] 릿지 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \Vert \beta \Vert_2^2$|$y$: 반응변수 벡터; $X$: 설계행렬; $\beta$: 회귀계수 벡터; $\Vert\cdot\Vert_2$: L2 노름; $\lambda$: 정규화 강도|계수가 부드럽게 0으로 수축|다중공선성 존재 회귀|
|**[5-2] 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \Vert \beta \Vert_1$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\Vert\cdot\Vert_1$: 절댓값 합 노름; $\lambda$: 정규화 강도|계수 일부가 정확히 0|고차원 변수 선택|
|**[5-3] 엘라스틱넷 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda_1 \Vert \beta \Vert_1 + \lambda_2 \Vert \beta \Vert_2^2$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\lambda_1$: L1 강도; $\lambda_2$: L2 강도|희소성과 안정성 동시 확보|유전자·텍스트 데이터|
|**[5-4] 그룹 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \sum_{g=1}^G \Vert \beta_g \Vert_2$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\beta_g$: g번째 그룹 계수 벡터; $G$: 그룹 개수; $\lambda$: 정규화 강도|그룹 단위 선택|유전자 경로, 센서 그룹|
|**[5-5] 희소 그룹 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda_1 \sum_{g=1}^G \Vert \beta_g \Vert_2 + \lambda_2 \Vert \beta \Vert_1$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\beta_g$: 그룹 계수; $G$: 그룹 수; $\lambda_1$: 그룹 벌점; $\lambda_2$: 개별 벌점|그룹·개별 동시 희소화|계층적 특징 선택|
|**[5-6] 적응적 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \sum_{j=1}^p w_j \lvert \beta_j \rvert$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\beta_j$: j번째 계수; $p$: 변수 개수; $w_j$: 가중치; $\lambda$: 정규화 강도|중요 계수 덜 축소|통계적 추론 중심|
|**[5-7] SCAD 벌점 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(\lvert \beta_j \rvert)$, $p'_\lambda(t)=\begin{cases}\lambda,&0\le t\le\lambda\\ \frac{a\lambda-t}{a-1},&\lambda<t\le a\lambda\\ 0,&t>a\lambda\end{cases}$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\beta_j$: j번째 계수; $p$: 변수 수; $\lambda$: 정규화 강도; $p_\lambda$: SCAD 벌점; $p'_\lambda$: 벌점 미분; $a$: 형상 파라미터 $(a>2)$|큰 계수에서 벌점 소멸|oracle property 요구 연구|
|**[5-8] MCP 벌점 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(\lvert \beta_j \rvert)$, $p_\lambda(t)=\begin{cases}\lambda t-\frac{t^2}{2\gamma},&0\le t\le\gamma\lambda\\ \frac{1}{2}\gamma\lambda^2,&t>\gamma\lambda\end{cases}$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\beta_j$: j번째 계수; $p$: 변수 수; $\lambda$: 정규화 강도; $p_\lambda$: MCP 벌점; $\gamma$: 완만도 파라미터 $(\gamma>1)$|선형 감소 후 평탄화|대규모 고차원 회귀|
