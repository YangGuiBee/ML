...
|모델명|개요|이론|특징|
|---|---|---|---|
|**[2-1] 이항 회귀**|이항 반응변수의 성공 확률 모델링|이항분포와 로짓 링크를 결합한 확률 회귀|선형회귀를 이산 확률모형으로 확장<br>→ 확률 해석과 계수 해석이 직관적|
|**[2-2] 포아송 회귀**|단위당 발생횟수 모델링|평균과 분산이 동일한 count 분포 가정|Binomial의 희귀사건 극한 형태<br>→ 단순 구조로 발생률 해석 용이|
...
|**[2-10] 허들 모형**|0과 양수 발생 분리 모델링|발생 여부와 크기를 단계적으로 모델링|Zero-Inflated에서 발생 구조를 명시적으로 분리<br>→ 발생 메커니즘 해석 용이|
