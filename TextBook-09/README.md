...
|모델명|개요|이론|수식|적용분야|
|---|---|---|---|---|
|[2-1] Binomial Regression<br>(이항 회귀)|이항 반응변수의 확률을 설명하는 GLM|Exponential family 기반, 성공확률의 선형예측|$Y \sim \mathrm{Binomial}(n,p),\;\log\frac{p}{1-p}=X\beta$|임상시험 성공률, 품질관리 합격률|
|[2-2] Poisson Regression<br>(포아송 회귀)|단위 시간·공간당 발생 횟수 모델링|평균과 분산이 동일한 count 분포 가정|$Y \sim \mathrm{Poisson}(\lambda),\;\log(\lambda)=X\beta$|사고 발생 건수, 콜센터 유입량|
|[2-3] Quasi-Poisson Regression<br>(준포아송 회귀)|과산포 count 데이터를 위한 포아송 확장|평균 구조 유지, 분산만 확장|$\mathbb{E}(Y)=\mu,\;\mathrm{Var}(Y)=\phi\mu$|환경 데이터 발생 빈도|
|[2-4] Negative Binomial Regression<br>(음이항 회귀)|과산포가 심한 count 데이터 모델|포아송–감마 혼합으로 분산 증가 허용|$\mathrm{Var}(Y)=\mu+\alpha\mu^2$|의료 방문 횟수, 보험 사고 수|
|[2-5] Gamma Regression<br>(감마 회귀)|양의 연속형 반응변수 모델|왜도 큰 분포, 로그 링크 사용|$Y \sim \mathrm{Gamma}(k,\theta),\;\log(\mu)=X\beta$|보험금 규모, 비용 데이터|
|[2-6] Inverse Gaussian Regression<br>(역가우시안 회귀)|긴 우측 꼬리를 가진 양의 연속값|평균과 분산이 특정 비율로 연동|$\mathrm{Var}(Y)=\phi\mu^3$|공정 소요시간, 생존시간|
|[2-7] Tweedie Regression<br>(트위디 회귀)|이산+연속 혼합 반응변수 모델|분산이 평균의 거듭제곱 형태|$\mathrm{Var}(Y)=\phi\mu^p$|보험 순손실, 강수량|
|[2-8] Cox Proportional Hazard Model<br>(콕스 비례위험 모형)|생존시간과 공변량 관계 모델|기저위험함수 비모수 처리|$h(t)=h_0(t)\exp(X\beta)$|의학 생존분석, 신뢰성 분석|
|[2-9] Zero-Inflated Model<br>(영과잉 모형)|0이 과도한 count 데이터 처리|이진 발생 과정과 count 과정 분리|$P(Y=0)=\pi+(1-\pi)f(0)$|교통사고 0건 지역 분석|
|[2-10] Hurdle Model<br>(허들 모형)|0과 양수 발생 메커니즘 분리|첫 발생 여부와 크기 모델 분리|$P(Y>0)=g(X\beta),\;Y^+ \sim f$|소비 발생 여부+금액|
|[2-11] Generalized Additive Model<br>(일반화 가법모형)|비선형 효과를 가법적으로 모델링|각 변수별 스무딩 함수 적용|$g(\mu)=\beta_0+\sum_j f_j(x_j)$|환경·역학 비선형 효과 분석|
|[2-12] Bayesian GLM<br>(베이즈 GLM)|사전분포를 포함한 GLM|사후분포 기반 추론|$p(\beta\mid y)\propto p(y\mid\beta)p(\beta)$|소표본 추론, 불확실성 분석|
|[2-13] Regularized GLM<br>(정규화 GLM)|고차원 회귀를 위한 벌점 기반 GLM|편향-분산 절충 최적화|$\min\{-\ell(\beta)+\lambda\lVert\beta\rVert_p\}$|유전자 데이터, 텍스트 회귀|


|모델명|비교|이론|수식|수식의 항목 설명|
|---|---|---|---|---|
|[2-1] Binomial Regression<br>(이항 회귀)|선형회귀를 이항 반응변수로 확장한 기본 GLM|이항분포 + 로짓 링크를 사용한 확률 모델|$Y \sim \mathrm{Binomial}(n,p),\;\log\frac{p}{1-p}=X\beta$|$Y$: 성공 횟수, $n$: 시행 수, $p$: 성공확률, $X\beta$: 선형예측자|
|[2-2] Poisson Regression<br>(포아송 회귀)|Binomial에서 희귀사건 가정으로 단순화|단위당 발생 횟수를 평균으로 직접 모델링|$Y \sim \mathrm{Poisson}(\lambda),\;\log(\lambda)=X\beta$|$\lambda$: 발생률, $X$: 설명변수, $\beta$: 회귀계수|
|[2-3] Quasi-Poisson Regression<br>(준포아송 회귀)|Poisson의 평균=분산 가정을 완화|평균 구조는 동일하고 분산만 조정|$\mathbb{E}(Y)=\mu,\;\mathrm{Var}(Y)=\phi\mu$|$\mu$: 평균, $\phi$: 과산포 계수|
|[2-4] Negative Binomial Regression<br>(음이항 회귀)|Poisson을 혼합분포로 일반화|포아송-감마 혼합으로 분산 증가 허용|$\mathrm{Var}(Y)=\mu+\alpha\mu^2$|$\alpha$: 분산 증가 계수, $\mu$: 평균|
|[2-5] Gamma Regression<br>(감마 회귀)|정규회귀를 양의 연속값으로 확장|왜도가 큰 연속형 반응변수 모델|$Y \sim \mathrm{Gamma}(k,\theta),\;\log(\mu)=X\beta$|$k$: 형태모수, $\theta$: 척도모수|
|[2-6] Inverse Gaussian Regression<br>(역가우시안 회귀)|Gamma보다 꼬리가 긴 분포 사용|평균과 분산이 비선형적으로 연결|$\mathrm{Var}(Y)=\phi\mu^3$|$\mu$: 평균, $\phi$: 분산 계수|
|[2-7] Tweedie Regression<br>(트위디 회귀)|Poisson·Gamma를 하나의 틀로 통합|분산을 평균의 거듭제곱으로 일반화|$\mathrm{Var}(Y)=\phi\mu^p$|$p$: 분포 지수, $\phi$: 분산 계수|
|[2-8] Cox Proportional Hazard Model<br>(콕스 비례위험 모형)|GLM을 시간-사건 데이터로 확장|기저위험함수를 분리한 반모수 모델|$h(t)=h_0(t)\exp(X\beta)$|$h(t)$: 위험함수, $h_0(t)$: 기저위험|
|[2-9] Zero-Inflated Model<br>(영과잉 모형)|Poisson/NB에 구조적 0 추가|이진 생성 과정과 count 과정 결합|$P(Y=0)=\pi+(1-\pi)f(0)$|$\pi$: 구조적 0 확률, $f$: count 분포|
|[2-10] Hurdle Model<br>(허들 모형)|Zero-Inflated에서 발생 구조 분리|0 여부와 양의 값 모델을 분리|$P(Y>0)=g(X\beta),\;Y^+ \sim f$|$g$: 발생 확률 모델, $Y^+$: 양의 반응|
|[2-11] Generalized Additive Model<br>(일반화 가법모형)|GLM의 선형항을 함수로 대체|비선형 효과를 가법 구조로 모델링|$g(\mu)=\beta_0+\sum_j f_j(x_j)$|$f_j$: 스무딩 함수, $x_j$: 입력 변수|
|[2-12] Bayesian GLM<br>(베이즈 GLM)|GLM에 사전분포를 도입|추론을 사후분포로 수행|$p(\beta\mid y)\propto p(y\mid\beta)p(\beta)$|$p(\beta)$: 사전분포, $p(\beta\mid y)$: 사후분포|
|[2-13] Regularized GLM<br>(정규화 GLM)|GLM에 벌점항을 추가|과적합 방지와 변수 선택|$\min\{-\ell(\beta)+\lambda\lVert\beta\rVert_p\}$|$\ell(\beta)$: 로그우도, $\lambda$: 규제강도|
