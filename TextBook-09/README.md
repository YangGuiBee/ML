...
|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|**[5-7] SCAD 벌점 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \sum_{j=1}^p p_\lambda(|\beta_j|)$,  $p'_\lambda(t)=\begin{cases}\lambda,&0\le t\le\lambda\\ \frac{a\lambda-t}{a-1},&\lambda<t\le a\lambda\\ 0,&t>a\lambda\end{cases}$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수 벡터; $\beta_j$: j번째 회귀계수; $p$: 변수 개수; $\lambda$: 정규화 강도; $p_\lambda(\cdot)$: SCAD 벌점 함수; $p'_\lambda(\cdot)$: 벌점의 미분; $a$: SCAD 형상 파라미터 $(a>2)$|큰 계수에서 벌점이 0으로 수렴|이론적 일관성, oracle property 요구 연구|
|**[5-8] MCP 벌점 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \sum_{j=1}^p p_\lambda(|\beta_j|)$,  $p_\lambda(t)=\begin{cases}\lambda t-\frac{t^2}{2\gamma},&0\le t\le\gamma\lambda\\ \frac{1}{2}\gamma\lambda^2,&t>\gamma\lambda\end{cases}$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수 벡터; $\beta_j$: j번째 회귀계수; $p$: 변수 개수; $\lambda$: 정규화 강도; $p_\lambda(\cdot)$: MCP 벌점 함수; $\gamma$: 벌점 완만도 파라미터 $(\gamma>1)$|벌점이 선형적으로 감소 후 평탄화|대규모 고차원 데이터, 계산 효율 중시 문제|
