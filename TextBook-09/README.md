...
|**[5-7] SCAD 벌점 회귀**|
$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(|\beta_j|)$
<br>
$p'_\lambda(t)=\begin{cases}
\lambda,\;0\le t\le\lambda;
\;\frac{a\lambda-t}{a-1},\;\lambda<t\le a\lambda;
\;0,\;t>a\lambda
\end{cases}$|$y$: 반응변수 벡터;<br>$X$: 설계행렬;<br>$\beta$: 회귀계수 벡터;<br>$\beta_j$: j번째 회귀계수;<br>$p$: 변수 개수;<br>$\lambda$: 정규화 강도;<br>$p_\lambda(\cdot)$: SCAD 벌점 함수;<br>$p'_\lambda(\cdot)$: 벌점의 미분;<br>$t$: 계수의 절댓값;<br>$a$: SCAD 형상 파라미터 $(a>2)$|큰 계수에서 벌점 소멸|oracle property 요구 연구|
...
\end{cases}$|$y$: 반응변수 벡터;<br>$X$: 설계행렬;<br>$\beta$: 회귀계수 벡터;<br>$\beta_j$: j번째 회귀계수;<br>$p$: 변수 개수;<br>$\lambda$: 정규화 강도;<br>$p_\lambda(\cdot)$: MCP 벌점 함수;<br>$t$: 계수의 절댓값;<br>$\gamma$: 벌점 완만도 파라미터 $(\gamma>1)$|선형 감소 후 평탄화|대규모 고차원 회귀|
