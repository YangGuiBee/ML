...
|모델명|개요|이론|특징|
|---|---|---|---|
|**[2-1] 이항 회귀**|이항 반응변수의 성공 확률을 모델링하는 GLM|이항분포와 로짓 링크를 결합한 확률 회귀|선형회귀를 이산 확률모형으로 확장→확률 해석과 계수 해석이 직관적|
|**[2-2] 포아송 회귀**|단위당 발생 횟수를 모델링|평균과 분산이 동일한 count 분포 가정|Binomial의 희귀사건 극한 형태→단순 구조로 발생률 해석 용이|
|**[2-3] 준포아송 회귀**|과산포 count 데이터를 위한 보정 모델|분산에 스케일 계수를 도입한 준우도 접근|Poisson의 평균 구조는 유지하고 분산만 확장→Poisson의 해석력을 유지하며 유연성 확보|
|**[2-4] 음이항 회귀**|과산포를 구조적으로 반영한 count 모델|포아송-감마 혼합으로 분산 증가 허용|Poisson을 혼합분포로 일반화→심한 과산포 데이터에 안정적|
|**[2-5] 감마 회귀**|양의 연속형 반응변수 모델|감마분포 기반 평균-분산 연결|정규회귀를 비대칭 연속 분포로 확장→비용·금액 데이터에 적합|
|**[2-6] 역가우시안 회귀**|긴 꼬리를 갖는 양의 연속값 모델|평균의 세제곱에 비례하는 분산 구조|Gamma보다 꼬리가 긴 분포 사용→극단값에 강한 모델링|
|**[2-7] 트위디 회귀**|이산+연속 혼합 반응변수 모델|분산을 평균의 거듭제곱으로 일반화|Poisson과 Gamma를 하나의 틀로 통합→0과 양수를 동시에 처리 가능|
|**[2-8] 콕스 비례위험 모형**|시간-사건 데이터의 회귀 모형|기저위험함수를 비모수로 분리|GLM을 생존분석으로 확장→검열 데이터 처리 가능|
|**[2-9] 영과잉 모형**|0이 과도한 count 데이터 처리|이진 생성 과정과 count 과정 결합|Poisson 또는 NB에 구조적 0 추가→0 과잉 문제에 특화|
|**[2-10] 허들 모형**|0과 양수 발생을 분리 모델링|발생 여부와 크기를 단계적으로 모델링|Zero-Inflated에서 발생 구조를 명시적으로 분리→발생 메커니즘 해석 용이|
|**[2-11] 일반화 가법모형**|비선형 효과를 가법 구조로 표현|변수별 스무딩 함수의 합|GLM의 선형항을 함수로 대체→비선형성과 해석력의 균형|
|**[2-12] 베이즈 GLM** |사전지식을 포함한 GLM|사후분포 기반 추론 체계|GLM에 베이즈 추론 도입→불확실성 정량화 가능|
|**[2-13] 정규화 GLM**|고차원 회귀를 위한 GLM 확장|편향-분산 절충 최적화|GLM에 벌점항 추가→과적합 방지와 변수 선택|
