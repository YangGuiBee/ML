...
|   번호  | 명칭                                                    | 기본 수식 (md 표기용 LaTeX)                                                                                                                                    | 대표 그래프 모양                                                                       | 주요 적용 분야                                                |                                                  |
| :---: | :---------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------ | :------------------------------------------------------ | ------------------------------------------------ |
| [2-1] | **로지스틱 회귀 (Logistic Regression)**                     | $g(\mu)=\log\left(\frac{\mu}{1-\mu}\right)=\beta_0+\beta_1x_1+\cdots+\beta_kx_k$  <br>→ $\mu = \frac{1}{1+e^{-(\beta_0+\beta_1x_1+\cdots+\beta_kx_k)}}$ | S자형 (Sigmoid Curve)                                                             | 이진 분류(Binary Classification), 확률 예측, 의료 진단(질병 유무), 신용평가 |                                                  |
| [2-2] | **포아송 회귀 (Poisson Regression)**                       | $g(\mu)=\log(\mu)=\beta_0+\beta_1x_1+\cdots+\beta_kx_k$  <br>→ $\mu = e^{(\beta_0+\beta_1x_1+\cdots+\beta_kx_k)}$                                       | 지수형 증가 (Exponential Growth)                                                     | 사건 발생 횟수 예측(교통사고, 방문 수, 클릭 수 등), Count Data             |                                                  |
| [2-3] | **콕스 비례위험 회귀 (Cox’s Proportional Hazard Regression)** | $h(t                                                                                                                                                    | X) = h_0(t)\exp(\beta_1x_1+\cdots+\beta_kx_k)$  <br>($h_0(t)$: baseline hazard) | 생존확률 감소형 (Monotonic Decreasing)                         | 생존분석(Survival Analysis), 의학(환자 생존율), 공학(제품 고장시간) |
| [2-4] | **감마 회귀 (Gamma Regression)**                          | $g(\mu)=\frac{1}{\mu}=\beta_0+\beta_1x_1+\cdots+\beta_kx_k$  <br>→ $\mu=\frac{1}{\beta_0+\beta_1x_1+\cdots+\beta_kx_k}$                                 | 우하향 곡선 (Inverse Relation)                                                       | 양의 연속값 데이터(비용, 지출, 보험금, 대기시간)                           |                                                  |
| [2-5] | **음이항 회귀 (Negative Binomial Regression)**             | $g(\mu)=\log(\mu)=\beta_0+\beta_1x_1+\cdots+\beta_kx_k$  <br>Var$(Y)=\mu+\alpha\mu^2$                                                                   | 지수형 증가 + 과산포 허용                                                                 | 과산포(Over-dispersion) 있는 사건수 예측 (범죄건수, 질병발생 수 등)         |                                                  |
| [2-6] | **일반화 가법모형 (Generalized Additive Model, GAM)**        | $g(E[Y]) = \beta_0 + f_1(x_1) + f_2(x_2) + \cdots + f_k(x_k)$  <br>(비선형 함수 $f_i$ 사용)                                                                    | 비선형 부드러운 곡선 (Smooth Curve)                                                      | 비선형 관계 모델링, 환경 데이터, 질병발생 위험요인, 복잡한 상호작용 데이터             |                                                  |
| [2-7] | **트위디 회귀 (Tweedie Regression)**                       | $g(\mu)=\log(\mu)=\beta_0+\beta_1x_1+\cdots+\beta_kx_k$  <br>Var$(Y)=\phi\mu^p$  (지수 $p$에 따라 분포 달라짐)                                                    | 분포 형태 가변 (p 값에 따라 달라짐)                                                          | 보험금, 지출, Claim Modeling, 비선형/비정규 데이터 예측                 |                                                  |
