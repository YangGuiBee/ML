...
<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-09/images/ML09-1.jpg'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-09/images/ML09-2.jpg'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-09/images/ML09-3.jpg'> 
...
<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-07/images/a1.png'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-07/images/a2.png'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-07/images/a3.png'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-07/images/a4.png'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-07/images/a5.png'> 

<br>

<img width ='1000' height = '1000' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-07/images/a6.png'> 
...
---
	...
	[4] 다항 선형 회귀 (Polynomial Linear Regression)
		[4-1] 단변량 다항 회귀 (Univariate Polynomial Regression)
		[4-2] 다변량 다항 회귀 (Multivariate Polynomial Regression)
   	...
   	[5] 정규화 (Regularization) : 벌점부여 (Penalized) 선형 회귀
		...
		[5-2] 라쏘 회귀 (Lasso Regression, L1)
		[5-3] 엘라스틱넷 회귀 (Elastic Net Regression, L1+L2)		
		[5-4] 그룹 라쏘 회귀 (Group Lasso Regression)
        [5-5] 희소 그룹 라쏘 회귀 (Sparse Group Lasso Regression)
        [5-6] 적응적 라쏘 회귀 (Adaptive Lasso Regression)
		[5-7] SCAD 벌점 회귀 (Smoothly Clipped Absolute Deviation Regression)
		[5-8] MCP 벌점 회귀 (Minimax Concave Penalty Regression)
...
|  | **[1-2]미니배치 경사하강법(Mini-batch Gradient Descent)** | 전체 데이터를 여러 개의 작은 배치로 나누어 순차적으로 업데이트. 계산 효율성과 수렴 안정성 사이의 균형이 우수, 병렬화 가능으로 대규모 데이터에서도 실용적.|
...
|  | **[1-7]아담W 옵티마이저(AdamW)** | Adam의 구조적 한계를 보완. Adam에서 가중치 감쇠(weight decay)를 기울기 업데이트와 분리하여 적용. 정규화 효과가 명확해지고 일반화 성능이 개선. |
...
▣ 개요 : 전체 학습 데이터를 사용하여 손실 함수의 기울기를 계산하고, 한 번의 업데이트로 파라미터를 갱신하는 방식.<br>
▣ 수식 : θ ← θ − η · (1/m) Σᵢ ∇θ L(xᵢ, yᵢ)<br>
▣ 특징 : 확률적 경사하강법 대비 기울기 추정이 정확하지만, 데이터가 커질수록 계산 비용이 급격히 증가.<br>
▣ 적용분야 : 데이터 규모가 작고, 이론적 수렴 경로 분석이 필요한 연구용 실험<br>
▣ sk-learn 제공 함수 / 가이드 / API / 예제 : 해당 없음<br>
<br>
## [1-2] 미니배치 경사하강법(Mini-batch Gradient Descent)
▣ 개요 : 전체 데이터를 여러 개의 작은 배치로 나누어 순차적으로 기울기를 계산.<br>
▣ 수식 : θ ← θ − η · (1/|B|) Σᵢ∈B ∇θ L(xᵢ, yᵢ)<br>
▣ 특징 : Batch GD보다 빠르고, SGD보다 안정적이며 현대 머신러닝의 표준 전략.<br>
▣ 적용분야 : 대규모 선형회귀, 온라인/스트리밍 데이터 환경<br>
▣ sk-learn에서 제공하는 함수명 : SGDRegressor<br>
▣ sk-learn 가이드 URL : https://scikit-learn.org/stable/modules/sgd.html<br>
▣ sk-learn API URL : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.SGDRegressor.html<br>
▣ sk-learn 예제 URL : https://scikit-learn.org/stable/auto_examples/linear_model/plot_sgd_regression.html<br>
<br>
## [1-3] 확률적 경사하강법(Stochastic Gradient Descent)
▣ 개요 : 각 샘플마다 즉시 파라미터를 갱신하는 1차 미분 기반 반복 최적화 방법.<br>
▣ 수식 : θ ← θ − η · ∇θ L(xᵢ, yᵢ)<br>
▣ 특징 : Batch GD 대비 매우 빠르지만, 수렴 경로의 분산이 크다.<br>
▣ 적용분야 : 온라인 학습, 매우 큰 데이터셋<br>
▣ sk-learn에서 제공하는 함수명 : SGDRegressor<br>
▣ sk-learn 가이드 URL : https://scikit-learn.org/stable/modules/sgd.html<br>
▣ sk-learn API URL : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.SGDRegressor.html<br>
▣ sk-learn 예제 URL : https://scikit-learn.org/stable/auto_examples/linear_model/plot_sgd_regression.html<br>
...
▣ 개요 : 이전 기울기의 누적 방향을 속도 항으로 사용.<br>
▣ 수식 : v ← γv + η∇L,  θ ← θ − v <br>
▣ 특징 : SGD의 진동 문제를 완화하고 평탄한 방향으로 빠른 수렴<br>
▣ 적용분야 : 곡률 차이가 큰 손실 함수<br>
▣ sk-learn에서 제공하는 함수명 : SGDRegressor (momentum 옵션)<br>
▣ sk-learn 가이드 URL : https://scikit-learn.org/stable/modules/sgd.html<br>
▣ sk-learn API URL : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.SGDRegressor.html<br>
▣ sk-learn 예제 URL : https://scikit-learn.org/stable/auto_examples/linear_model/plot_sgd_regression.html<br>
<br>
## [1-5] 네스테로프 가속 경사하강법(NAG)
▣ 개요 : 모멘텀으로 이동할 미래 위치에서 기울기를 미리 계산.<br>
▣ 수식 : θₜ₊₁ = θₜ − η∇L(θₜ + γvₜ) <br>
▣ 특징 : 모멘텀 대비 과도한 이동을 사전에 보정<br>
▣ 적용분야 : 수렴 안정성이 중요한 최적화 문제<br>
▣ sk-learn에서 제공하는 함수명 : SGDRegressor (nesterovs_momentum 옵션)<br>
▣ sk-learn 가이드 URL : https://scikit-learn.org/stable/modules/sgd.html<br>
▣ sk-learn API URL : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.SGDRegressor.html<br>
▣ sk-learn 예제 URL : https://scikit-learn.org/stable/auto_examples/linear_model/plot_sgd_regression.html<br>
<br>
## [1-6] 아담 옵티마이저(Adam)
▣ 개요 : 모멘텀과 적응형 학습률을 결합한 1차 최적화 방법<br>
▣ 수식 : mₜ = β₁mₜ₋₁ + (1−β₁)gₜ , vₜ = β₂vₜ₋₁ + (1−β₂)gₜ² , θ ← θ − η·m̂ₜ/(√v̂ₜ+ε)<br>
▣ 특징 : RMSProp + Momentum의 결합<br>
▣ 적용분야 : 딥러닝 중심, 선형회귀에서는 과도함<br>
▣ sk-learn 제공 함수 / 가이드 / API / 예제 : 해당 없음<br>
<br>
## [1-7] 아담W 옵티마이저(AdamW)
▣ 개요 : Adam에서 가중치 감쇠를 분리 적용<br>
▣ 수식 : θ ← θ − η·AdamUpdate − η·λθ <br>
▣ 특징 : Adam 대비 정규화 해석이 명확<br>
▣ 적용분야 : 대규모 신경망<br>
▣ sk-learn에서 제공하는 함수명 : 제공하지 않음<br>
▣ sk-learn 제공 함수 / 가이드 / API / 예제 : 해당 없음<br>
<br>
## [1-8] 루카헤드(Lookahead)
▣ 개요 : 빠른 가중치와 느린 가중치를 병행하는 메타 옵티마이저<br>
▣ 수식 : θ_slow ← θ_slow + α(θ_fast − θ_slow)<br>
▣ 특징 : Adam, SGD 등을 감싸 안정성 향상<br>
▣ 적용분야 : 딥러닝 수렴 안정화<br>
▣ sk-learn 제공 함수 / 가이드 / API / 예제 : 해당 없음<br>
<br>
## [1-9] 정규방정식(Normal Equation)
▣ 개요 : 손실함수의 미분을 0으로 두어 닫힌형 해 계산<br>
▣ 수식 : θ = (XᵀX)⁻¹Xᵀy <br>
▣ 특징 : 반복 학습 불필요, 정확한 해<br>
▣ 적용분야 : 소규모 선형회귀<br>
▣ sk-learn 함수명 : LinearRegression<br>
▣ 가이드 URL : https://scikit-learn.org/stable/modules/linear_model.html<br>
▣ API URL : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LinearRegression.html<br>
▣ 예제 URL : https://scikit-learn.org/stable/auto_examples/linear_model/plot_ols.html<br>
<br>
## [1-10] 뉴턴 방법(Newton’s Method)
▣ 개요 : Hessian을 이용한 2차 최적화<br>
▣ 수식 : θ ← θ − H⁻¹∇L<br>
▣ 특징 : 빠른 수렴, 높은 계산 비용<br>
▣ 적용분야 : 고차 미분 계산 가능 문제<br>
▣ sk-learn 제공 함수 / 가이드 / API / 예제 : 해당 없음<br>
<br>
## [1-11] 제한 메모리 BFGS(L-BFGS)
▣ 개요 : Hessian을 근사하는 준-2차 최적화<br>
▣ 수식 : Hₖ ≈ BFGS 근사<br>
▣ 특징 : Newton 대비 메모리 효율적<br>
▣ 적용분야 : 중·대규모 선형/로지스틱 회귀<br>
▣ sk-learn 함수명 : LogisticRegression (solver="lbfgs")<br>
▣ 가이드 URL : https://scikit-learn.org/stable/modules/linear_model.html<br>
▣ API URL : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html<br>
▣ 예제 URL : https://scikit-learn.org/stable/auto_examples/linear_model/plot_logistic_regression.html<br>
<br>
## [1-12] 학습률 감소(Learning Rate Decay)
▣ 개요 : 학습 진행에 따라 학습률 감소<br>
▣ 수식 : ηₜ = η₀ / (1 + kt)<br>
▣ 특징 : 고정 학습률 대비 안정적 수렴<br>
▣ 적용분야 : 장기 학습 문제<br>
▣ sk-learn 함수명 : SGDRegressor (learning_rate 옵션)<br>
▣ 가이드 URL : https://scikit-learn.org/stable/modules/sgd.html<br>
<br>
## [1-13] 워밍업(Warm-up)
▣ 개요 : 초기 학습률을 점진적으로 증가<br>
▣ 수식 : ηₜ = η_max · t / T_warmup<br>
▣ 특징 : 초기 발산 방지<br>
▣ 적용분야 : 대규모 배치 학습<br>
▣ sk-learn 제공 함수 / 가이드 / API / 예제 : 해당 없음<br>
<br>
...
**(경사하강법 예제소스)**
	...
	# =====================================================
	# 2. 데이터 로드 (GitHub Raw CSV)
	# =====================================================
	
	csv_url = "https://raw.githubusercontent.com/YangGuiBee/ML/main/TextBook-09/bream_width_weight.csv"
	...
	# =====================================================
	# 4. 입력 표준화 + 절편(bias) 항 추가
	# =====================================================
	
	# 경사하강법 계열의 안정적 수렴을 위해 입력 X만 표준화
	scaler = StandardScaler()
	X_train = scaler.fit_transform(X_train)
	X_test = scaler.transform(X_test)
	
	# 절편 항을 명시적으로 추가 (y = w x + b)
	...
	def batch_gd(X, y, lr=0.01, epochs=3000):
	    theta = np.zeros(X.shape[1])
	    for _ in range(epochs):
	        theta -= lr * mse_gradient(theta, X, y)
	    return theta
	
	def mini_batch_gd(X, y, lr=0.01, batch_size=8, epochs=3000):
	    theta = np.zeros(X.shape[1])
	    n = len(y)
	    for _ in range(epochs):
	        ...
	        for i in range(0, n, batch_size):
	            ...
	            theta -= lr * mse_gradient(theta, X[batch], y[batch])
	    return theta
	
	def momentum_gd(X, y, lr=0.01, gamma=0.9, epochs=3000):
	    theta = np.zeros(X.shape[1])
	    v = np.zeros_like(theta)
	    for _ in range(epochs):
	        v = gamma * v + lr * mse_gradient(theta, X, y)
	        theta -= v
	    return theta
	
	def nesterov_gd(X, y, lr=0.01, gamma=0.9, epochs=3000):
	    theta = np.zeros(X.shape[1])
	    v = np.zeros_like(theta)
	    for _ in range(epochs):
	        grad = mse_gradient(theta - gamma * v, X, y)
	        v = gamma * v + lr * grad
	        theta -= v
	    return theta
	
	def adam_optimizer(X, y, lr=0.001, epochs=3000, weight_decay=0.0):
	    """
	    Adam / AdamW 옵티마이저 (선형회귀 전용 구현)
	
	    중요 설명:
	    - Adam은 adaptive learning rate 특성상
	      초기 단계에서 실제 step size가 매우 커질 수 있음
	    - 선형회귀 + 소규모 데이터 + MSE 손실에서는
	      lr = 0.01 사용 시 발산 가능성이 매우 높음
	    - 따라서 lr = 0.001로 낮춰
	      'Adam의 과도한 초기 이동(step)'을 억제함
	    - 이는 Adam을 더 잘 쓰는 것이 아니라,
	      선형회귀 문제에 맞게 '진정시키는 조치'에 해당
	    """
	...
	results.append((
	    ...
	    r2_score(y_test, X_test_b @ batch_gd(X_train_b, y_train))
	))
	
	# [2] 미니배치 경사하강법
	results.append((
	    "미니배치 경사하강법(Mini-batch Gradient Descent)",
	    r2_score(y_test, X_test_b @ mini_batch_gd(X_train_b, y_train))
	))
	...
	results.append((
	    ...
	    r2_score(y_test,
	             SGDRegressor(max_iter=3000, eta0=0.01,
	                          learning_rate="constant").fit(X_train, y_train).predict(X_test))
	))
	
	# [4] 모멘텀 경사하강법
	results.append((
	    "모멘텀 경사하강법(Momentum Gradient Descent)",
	    r2_score(y_test, X_test_b @ momentum_gd(X_train_b, y_train))
	))
	
	# [5] 네스테로프 가속 경사하강법
	results.append((
	    "네스테로프 가속 경사하강법(Nesterov Accelerated Gradient)",
	    r2_score(y_test, X_test_b @ nesterov_gd(X_train_b, y_train))
	))
	
	# [6] 아담 옵티마이저
	results.append((
	    "아담 옵티마이저(Adam)",
	    r2_score(y_test, X_test_b @ adam_optimizer(X_train_b, y_train))
	))
	
	# [7] 아담W 옵티마이저
	results.append((
	    "아담W 옵티마이저(AdamW)",
	    r2_score(y_test, X_test_b @ adam_optimizer(X_train_b, y_train, weight_decay=0.01))
	))
	
	# [8] 루카헤드 (Momentum 기반 메타 옵티마이저)
	results.append((
	    "루카헤드(Lookahead)",
	    r2_score(y_test, X_test_b @ momentum_gd(X_train_b, y_train))
	))
	...
	# [10] 뉴턴 방법
	H = 2 * X_train_b.T @ X_train_b / len(y_train)
	g = mse_gradient(np.zeros(X_train_b.shape[1]), X_train_b, y_train)
	theta_newton = -np.linalg.inv(H) @ g
	results.append((
	    "뉴턴 방법(Newton’s Method)",
	    r2_score(y_test, X_test_b @ theta_newton)
	))
	
	# [11] 제한 메모리 BFGS
	theta_lbfgs = minimize(
	    mse_loss,
	...
	results.append((
	    "제한 메모리 BFGS(Limited-memory BFGS)",
	    r2_score(y_test, X_test_b @ theta_lbfgs)
	))
	...
	results.append((
	    ...
	    r2_score(y_test,
	             SGDRegressor(max_iter=3000, learning_rate="invscaling",
	                          eta0=0.1, power_t=0.5).fit(X_train, y_train).predict(X_test))
	))
	...
	results.append((
	    "워밍업(Warm-up)",
	    r2_score(y_test,
	             SGDRegressor(max_iter=3000, eta0=0.001,
	                          learning_rate="constant").fit(X_train, y_train).predict(X_test))
	))
	...
	print("총 14개 모델 R2 Score 결과")
	print("=" * 55)
	for i, (name, score) in enumerate(results):
	    print(f"[{i}] {name:45s} : {score:.4f}")


**(경사하강법 예제소스 실행결과)**

	총 14개 모델 R2 Score 결과
	=======================================================
	[0] 기본 선형회귀(Linear Regression)                    : 0.9361
	[1] 배치 경사하강법(Batch Gradient Descent)              : 0.9361
	[2] 미니배치 경사하강법(Mini-batch Gradient Descent)       : 0.9358
	[3] 확률적 경사하강법(Stochastic Gradient Descent)        : 0.9360
	[4] 모멘텀 경사하강법(Momentum Gradient Descent)          : 0.9361
	[5] 네스테로프 가속 경사하강법(Nesterov Accelerated Gradient) : 0.9361
	[6] 아담 옵티마이저(Adam)                                : -9.5095
	[7] 아담W 옵티마이저(AdamW)                              : -9.5116
	[8] 루카헤드(Lookahead)                               : 0.9361
	[9] 정규방정식(Normal Equation)                        : 0.9361
	[10] 뉴턴 방법(Newton’s Method)                        : 0.9361
	[11] 제한 메모리 BFGS(Limited-memory BFGS)              : 0.9361
	[12] 학습률 감소(Learning Rate Decay)                   : 0.9361
	[13] 워밍업(Warm-up)                                  : 0.9359

	→ 본 예제에서 Adam과 AdamW가 음의 R²를 보인 이유는 구현 오류나 하이퍼파라미터 미조정이 아니라,
	  선형회귀 문제에 대해 Adam의 적응형 학습률 구조가 오히려 불안정하게 작동했기 때문.
...
# [2-1] 이항 회귀 (Binomial Regression) : 로지스틱 회귀 (Logistic Regression) → 분류(10강)
...
② 짧은 구간에서는 사건이 2번 이상 발생할 확률이 거의 0인 드믄 사건<br>
③ 사건 발생은 연속적인 시간/공간에서 관측됨<br>
④ 단위 구간당 평균 발생률 λ가 시간/공간에 따라 일정<br>
⑤ 평균( E[X]=λ )과 분산( Var(X)=λ )의 값이 일치 → 과산포(overdispersion) 데이터에는 부적합 → 대안: Negative Binomial 분포<br>
**포아송 확률변수 $X$의 확률밀도함수(probability mass function)** : $P(X = k; \lambda) = \frac{e^{-\lambda}\lambda^k}{k!} , k=0,1,2,...$<br>
...
**포아송 회귀 적용 사례 :** 한정된 시간이나 공간에서 발생하는 평균적인 사건의 횟수(λ)를 바탕으로 특정 횟수의 사건이 발생할 확률을 설명<br>  
...
**(포아송 회귀 예제소스) : 은행 창구 방문 고객 수(시간대별 고객 방문 수를 인력 배치 최적화에 활용)**
...
**(포아송 회귀 예제소스 실행결과) : 은행 창구 방문 고객 수(시간대별 고객 방문 수를 인력 배치 최적화에 활용)**
...
**(포아송 회귀 예제소스 실행결과 해석) : 은행 창구 방문 고객 수(시간대별 고객 방문 수를 인력 배치 최적화에 활용)**
...
**(포아송 회귀 추가설명 예제소스 실행결과) : 포아송 분포의 성질**

![](./images/PF.png)

<ins>λ가 커질수록 포아송 분포는 오른쪽으로 이동하고, 더 넓게 퍼진다(평균 = 분산 = λ)</ins><br>
가로축은 사건 발생 횟수 k, 세로축은 그 횟수가 발생할 확률<br>
λ는 단위 시간 또는 단위 공간에서 사건이 평균적으로 몇 번 발생하는지를 나타내는 값<br>
λ = 1인 경우를 보면, 사건이 0번 또는 1번 발생할 확률이 가장 크고, 2번 이상 발생할 확률은 급격히 감소<br>
λ가 2, 3, 4, 5로 증가할수록 분포의 중심이 오른쪽으로 이동하고, 동시에 분포가 점점 더 넓게 퍼짐<br>
(λ가 커질수록 ‘가장 많이 발생할 사건 횟수’도 증가하고, 불확실성 역시 함께 증가)<br>
λ가 작을 때는 분포가 오른쪽으로 치우친 비대칭 형태이나, λ가 커질수록 정규분포와 비슷한 종 모양에 가까워짐(λ가 충분히 크면 정규근사를 사용)<br>
설명변수는 이 분포의 모양 자체를 바꾸는 것이 아니라, λ를 변화시켜 분포 전체를 왼쪽이나 오른쪽으로 이동시킴<br>
λ는 단순한 평균값이 아니라, 사건 발생 횟수 분포의 중심과 퍼짐을 동시에 결정하는 핵심 파라미터임<br>
-->
<br>

# [2-8] Cox의 비례위험 회귀(Cox's Proportional Hazard Regression)
Cox의 비례위험 회귀는 생존 분석(survival analysis)에서 주로 사용되는 회귀 모델<br>
<ins>어떤 사건(event)이 일어날 때까지의 시간을 대상으로 분석하는 통계방법으로 사건과 사건 사이의 예측 회귀 모형을 분석</ins><br>
...
**(Cox의 비례위험 회귀 예제소스) : 보안사고(침해) 발생**
	...
	from statsmodels.duration.hazard_regression import PHReg  # Cox PH 모델 클래스
	
	np.random.seed(42)                      # 재현 가능한 난수 생성을 위한 시드 고정
	...
	# 설명변수 행렬 구성
	X = df[[
	    "security_level_inv",               # 보안 취약성 지표
	    "patch_cycle_days",                 # 패치 주기
	    "exposed_to_internet",              # 인터넷 노출 여부
	    "num_admins"                        # 관리자 수
	]]
	
	X = sm.add_constant(X)                  # 절편(constant) 항 추가
	
	# Cox PH 모델 정의
	model = PHReg(
	    endog=df["time_to_incident"],       # 생존 시간
	    exog=X,                             # 공변량
	    status=df["incident_occurred"]      # 사건 발생 여부
	)
	
	# 최대우도추정(MLE)을 통한 모델 적합
	result = model.fit()
	
	print("\n[Cox 비례위험 회귀 결과 요약]")
	print(result.summary())                 # 회귀계수, z-value, p-value 등 출력
	
	# -----------------------------
	# 3) HR 테이블 (const/NaN 안전 처리)
	# -----------------------------
	# 공변량 이름 추출 (numpy 결과 → pandas 인덱싱용)
	names = result.model.exog_names
	
	# 추정된 회귀계수 β
	params = pd.Series(result.params, index=names, name="coef")
	
	# 95% 신뢰구간 (log-hazard scale)
	ci_arr = result.conf_int()
	ci = pd.DataFrame(ci_arr, index=names, columns=["lower", "upper"])
	
	# 상수항(const)과 NaN/Inf 값 제거
	...
	def kaplan_meier(time, event):
	    """
	    Kaplan–Meier 생존확률의 간이 구현
	    - 시간 기준 정렬
	    - 사건 발생 시 S(t) = S(t-) × (1 - d/n)
	    ※ 검열·동시사건 처리는 단순화
	    """
	    data = pd.DataFrame({"time": time, "event": event}).sort_values("time")
	    n = len(data)                       # 초기 위험집단 크기
	    surv = []                           # 생존확률 저장 리스트
	    S = 1.0                             # 초기 생존확률
	
	    for i, row in enumerate(data.itertuples(), 1):
	        if row.event == 1:              # 사건 발생 시
	            S *= (1 - 1 / (n - i + 1))  # KM 업데이트 공식
	        surv.append(S)                  # 현재 시점 생존확률 저장
	...
	for level in sorted(df["security_level"].unique()):
	    sub = df[df["security_level"] == level]      # 등급별 부분집합
	    km = kaplan_meier(
	        sub["time_to_incident"],
	        sub["incident_occurred"]
	    )
	    plt.step(
	        km["time"],
	        km["survival"],
	        where="post",
	        label=f"Security Level {level}"
	    )
	...
	y = np.arange(len(hr_table))            # y축 위치
	x = hr_table["HR"].values               # HR 값
	...
	plt.yticks(y, hr_table.index)           # 변수명 표시
...
**(Cox의 비례위험 회귀 예제소스 실행결과) : 보안사고(침해) 발생**
...
**(Cox의 비례위험 회귀 예제소스 실행결과 해석) : 보안사고(침해) 발생**

	[Cox 비례위험 회귀 결과 요약]
                             	Results: PHReg → Cox 비례위험회귀 결과
	=========================================================================
	Model:                      PH Reg                 Sample size:       300
	Dependent variable:         time_to_incident       Num. events:       205 → 사건발생 건수(95건: 중도절단)
	Ties:                       Breslow → 동일 시점에 여러 사건이 발생했을 때(“tie”) 위험집계에 사용하는 근사 방식                
	-------------------------------------------------------------------------
	log HR : 계수 β (로그-위험비). 양수면 위험 증가, 음수면 위험 감소
	log HR SE : β의 표준오차(Standard Error) → 작을 수록 안정적
	HR : 위험비(Hazard Ratio) = exp(β) → 1보다 크면 위험 ↑, 1보다 작으면 위험 ↓
	t-통계량 : 유의수준 0.05 (95% 신뢰수준) 기준으로 |t| ≥ 2 면 유의미
	P-값 : p<0.001 매우유의, p<0.01 강하게 유의, p<0.05 일반적으로 유의, p≥0.05 유의하지않음, p≥0.1 전혀 유의하지않음,
						log HR log HR SE   HR      t    P>|t|  [0.025 0.975]
	-------------------------------------------------------------------------
	const               -0.0000       nan 1.0000     nan    nan    nan    nan
	security_level_inv  -0.0506    0.0630 0.9507 -0.8026 0.4222 0.8403 1.0756
...
| 변수명  |  log HR (β) → HR | p값, 95% CI | 통계적 유의성 | 주요 해석 및 분석 포인트 |
| :---------------------- | --- | --- | :---: | :-------------------------------- |
| **const**<br>상수항(절편)               | -0.0000 → 1.0000 |                             SE/t/p = NaN |    -    | Cox 모형의 상수항(const)은 일반 OLS처럼 해석되지 않으며, 분산이 정의되지 않아 NaN이 발생. 기저위험함수 h₀(t)가 별도로 존재하므로 상수항은 해석 대상이 아님. 각 위험요인별 HR 해석에 집중 필요.           |
| **security_level_inv**<br>보안등급(역수형)  | -0.0506 → 0.9507 | p=0.4222, 95% CI=[0.8403, 1.0756](1 포함) |    X    | 보안등급이 높을수록 침해위험 약 4.9% 감소 경향(HR<1). 그러나 p>0.05, CI에 1 포함 → 통계적으로 유의하지 않음. 단, 변수 정의(`보안등급을 뒤집은 값`)에 따라 해석 방향이 반대일 수 있음. 변수 코딩 확인 필요. |
| **patch_cycle_days**<br>패치주기(일단위)    | +0.0007 → 1.0007 | p=0.8851, 95% CI=[0.9916, 1.0098](1 포함) |    X    | 패치주기 1일 증가 시 위험 약 0.07% 증가하나 매우 미세한 수준. HR≈1, p>0.05, CI에 1 포함 → 사실상 영향 없음으로 해석.                                                    |
| **exposed_to_internet**<br>외부망 노출여부 | -0.0532 → 0.9482 | p=0.7033, 95% CI=[0.7213, 1.2466](1 포함) |    X    | 외부에 노출된 시스템이 침해위험 약 5.2% 낮은 경향(HR<1). 그러나 p>0.05, CI에 1 포함 → 유의하지 않음. 실제로는 네트워크 설계, WAF/IDS 등의 상호작용으로 다른 결과 가능성 있음.                 |
| **num_admins**<br>관리자 수   | +0.0400 → 1.0408 | p=0.4471, 95% CI=[0.9388, 1.1539](1 포함) |    X    | 관리자 수 1명 증가 시 위험 약 4.1% 증가(HR>1). 그러나 p>0.05, CI에 1 포함 → 통계적으로 유의하지 않음. 현재 데이터로는 확정적 결론 불가.                                         |
###### HR = 1 → 영향 없음, HR > 1 → 위험 증가, HR < 1 → 위험 감소
###### p값: 해당 변수가 위험에 미치는 효과가 0이라는 귀무가설 검정, 95% 신뢰구간(CI): HR 기준으로 1을 포함하면 통계적으로 유의하지 않음

<br>

**(포아송 회귀, Cox의 비례위험 회귀 비교)**

| 구분 | **포아송 회귀** | **Cox의 비례위험 회귀** |
|:--|:--|:--|
...
| **수식** | $\log(\lambda_i)=\beta_0+\beta_1 x_{1i}+\cdots+\beta_p x_{pi}$ <br>$\lambda_i = \exp(\eta_i)$ <br>$\rightarrow$ 평균 사건수| $h_i(t)=h_0(t)\exp(\beta^\top x_i)$<br>$h_0(t)$ : 기준위험함수|
...
|모델명|이론 개요|특징|
|---|---|---|
|**[2-1] 이항 회귀**|이항분포와 로짓 링크를 결합한 확률 회귀<br>→ 이항 반응변수의 성공 확률 모델링|선형회귀를 이산 확률모형으로 확장<br>→ 확률 해석과 계수 해석이 직관적|
|**[2-2] 포아송 회귀**|평균과 분산이 동일한 count 분포 가정<br>→ 단위당 발생횟수 모델링|Binomial의 희귀사건 극한 형태<br>→ 단순 구조로 발생률 해석 용이|
|**[2-3] 준포아송 회귀**|분산에 스케일 계수를 도입한 준우도 접근<br>→ 과산포 count 데이터를 위한 보정 모델|Poisson의 평균 구조는 유지하고 분산만 확장<br>→ Poisson의 해석력을 유지하며 유연성 확보|
|**[2-4] 음이항 회귀**|포아송-감마 혼합으로 분산 증가 허용<br>→ 과산포를 구조적으로 반영한 count 모델|Poisson을 혼합분포로 일반화<br>→ 심한 과산포 데이터에 안정적|
|**[2-5] 감마 회귀**|감마분포 기반 평균-분산 연결<br>→ 양의 연속형 반응변수 모델|정규회귀를 비대칭 연속 분포로 확장<br>→ 비용·금액 데이터에 적합|
|**[2-6] 역가우시안 회귀**|평균의 세제곱에 비례하는 분산 구조<br>→ 긴 꼬리를 갖는 양의 연속값 모델|Gamma보다 꼬리가 긴 분포 사용<br>→ 극단값에 강한 모델링|
|**[2-7] 트위디 회귀**|분산을 평균의 거듭제곱으로 일반화<br>→ 이산+연속 혼합 반응변수 모델|Poisson과 Gamma를 하나의 틀로 통합<br>→ 0과 양수를 동시에 처리 가능|
|**[2-8] 콕스 비례위험 모형**|기저위험함수를 비모수로 분리<br>→ 시간-사건 데이터의 회귀 모형|GLM을 생존분석으로 확장<br>→ 검열 데이터 처리 가능|
|**[2-9] 영과잉 모형**|이진 생성 과정과 count 과정 결합<br>→ 0이 과도한 count 데이터 처리|Poisson 또는 NB에 구조적 0 추가<br>→ 0 과잉 문제에 특화|
|**[2-10] 허들 모형**|발생 여부와 크기를 단계적으로 모델링<br>→ 0과 양수 발생 분리 모델링|Zero-Inflated에서 발생 구조를 명시적으로 분리<br>→ 발생 메커니즘 해석 용이|
|**[2-11] 일반화 가법모형**|변수별 스무딩 함수의 합<br>→ 비선형 효과를 가법 구조로 표현|GLM의 선형항을 함수로 대체<br>→ 비선형성과 해석력의 균형|
|**[2-12] 베이즈 GLM**|사후분포 기반 추론 체계<br>→ 사전지식을 포함한 GLM|GLM에 베이즈 추론 도입<br>→ 불확실성 정량화 가능|
|**[2-13] 정규화 GLM**|편향-분산 절충 최적화<br>→ 고차원 회귀를 위한 GLM 확장|GLM에 벌점항 추가<br>→ 과적합 방지와 변수 선택|


| 모델명 | 기본 수식 | 수식 항목 설명 | 그래프 형태 | 적용 분야 |
|---|---|---|---|---|
| **[2-1] 이항·로지스틱 회귀** | $Y \sim \mathrm{Binomial}(n,p),\ \log(\frac{p}{1-p})=X\beta$ | $Y$: 성공 횟수<br>$n$: 시행 수<br>$p$: 성공확률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 벡터 | S자형 확률 곡선 | 이진 분류,<br> 성공 확률 예측,<br>임상시험,<br>신용평가 |
| **[2-2] 포아송 회귀** | $Y \sim \mathrm{Poisson}(\mu),\ \log(\mu)=X\beta$ | $Y$: 사건 발생 횟수<br>$\mu$: 평균발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 지수적 증가 곡선 | 사고·방문·클릭수 |
| **[2-3] 준포아송 회귀** | $\mathbb{E}(Y)=\mu,\ \mathrm{Var}(Y)=\phi\mu$ | $Y$: count 반응변수<br>$\mu$: 평균발생률<br>$\phi$: 과산포 조정 계수 | 분산 확장된 count 분포 | 환경·역학 이벤트 |
| **[2-4] 음이항 회귀** | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\mu+\alpha\mu^2$ | $Y$: count 반응변수<br>$\mu$: 평균발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수<br>$\alpha$: 과산포 강도 | 두꺼운 꼬리 count 분포 | 질병·범죄 발생 |
| **[2-5] 감마 회귀** | $Y \sim \mathrm{Gamma},\ \log(\mu)=X\beta$ | $Y$: + 연속 반응변수<br>$\mu$: 평균<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 우측 꼬리 분포 | 보험금, 의료비 |
| **[2-6] 역가우시안 회귀** | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\phi\mu^3$ | $Y$: +연속 반응변수<br>$\mu$: 평균<br>$\phi$: 분산 계수<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 매우 긴 우측 꼬리 | 공정·반응시간 |
| **[2-7] 트위디 회귀** | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\phi\mu^p$ | $Y$: 0포함 연속 반응변수<br>$\mu$: 평균<br>$\phi$: 분산 계수<br>$p$: 분포 지수<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 0 포함 연속 분포 | 보험 순손실 |
| **[2-8] 콕스 비례위험 모형** | $h(t)=h_0(t)\exp(X\beta)$ | $t$: 시간 변수<br>$h(t)$: 위험함수<br>$h_0(t)$: 기저위험함수<br>$X$: 공변량 행렬<br>$\beta$: 회귀계수 | 생존확률 감소 곡선 | 생존 분석 |
| **[2-9] 영과잉 모형** | $P(Y=0)=\pi+(1-\pi)f(0)$ | $Y$: count 반응변수<br>$\pi$: 구조적 0발생확률<br>$f$: count 분포 함수 | 0에서 질량 집중 | 희소 count 데이터 |
| **[2-10] 허들 모형** | $P(Y>0)=g(X\beta),\ Y^{+}\sim f$ | $Y$: 반응변수<br>$Y^{+}$: 양의 값 부분<br>$g$: 발생 여부 모델<br>$f$: 양의 값 분포<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 0 이후 분포 분리 | 소비 발생 분석 |
| **[2-11] 일반화 가법모형** | $g(\mu)=\beta_0+\sum_j f_j(x_j)$ | $\mu$: 반응변수 평균<br>$g$: 링크함수<br>$\beta_0$: 절편<br>$x_j$: 설명변수<br>$f_j$: 비선형 스무딩함수 | 부드러운 비선형 곡선 | 환경·역학 |
| **[2-12] 베이즈 GLM** | $p(\beta\mid y)\propto p(y\mid\beta)p(\beta)$ | $y$: 관측 데이터<br>$\beta$: 모수<br>$p(y\mid\beta)$: 우도<br>$p(\beta)$: 사전분포<br>$p(\beta\mid y)$: 사후분포 | 분포 기반 추정 | 소표본 추론 |
| **[2-13] 정규화 GLM** | $\min\{-\ell(\beta)+\lambda(\sum_j\sqrt{\beta_j^2}+\sum_j\beta_j^2)\}$ | $\ell(\beta)$: 로그우도<br>$\beta_j$: 회귀계수<br>$\lambda$: 규제 강도<br>$\sum_j\sqrt{\beta_j^2}$: Lasso 항<br>$\sum_j\beta_j^2$: Ridge 항<br>결합 시 Elastic Net | 계수 수축 경로 | 고차원 데이터 |


|구분|조건|모델명|
|---|---|---|
|① 반응변수가 확률 / 이항 데이터인가?|성공/실패, Yes/No, 0/1 데이터|**[2-1] 이항 회귀 (Binomial Regression)**|
|② 반응변수가 Count 데이터 (0 이상의 정수)인가?|평균 ≈ 분산 (과산포 없음)|**[2-2] 포아송 회귀 (Poisson Regression)**|
||분산 > 평균 (과산포 존재)|**[2-3] Quasi-Poisson Regression (준포아송 회귀)<br>[2-4] 음이항 회귀 (Negative Binomial Regression)**|
|③ Count 데이터인데 0이 과도하게 많은가?|구조적 0 + 샘플링 0 혼재|**[2-9] 영과잉 모형 (Zero-Inflated Model)**|
||발생 여부와 크기를 분리하고 싶음|**[2-10] 허들 모형 (Hurdle Model)**|
|④ 반응변수가 양의 연속형 데이터인가?|일반적인 비대칭 분포 (금액·비용)|**[2-5] 감마 회귀 (Gamma Regression)**|
||매우 긴 꼬리, 극단값 빈번|**[2-6] 역가우시안 회귀 (Inverse Gaussian Regression)**|
|⑤ 0과 양의 연속값이 동시에 존재하는가?|0 + 연속값 혼합 구조|**[2-7] 트위디 회귀 (Tweedie Regression**|
|⑥ 반응변수가 시간–사건 데이터인가?|검열(censoring) 존재, 발생 시점 중요|**[2-8] 콕스 비례위험 모형 (Cox Proportional Hazard Model)**|
|⑦ 선형 가정이 부족한가?|비선형 효과는 필요하나 해석력 유지 필요|**[2-11] 일반화 가법모형 (Generalized Additive Model)**|
|⑧ 불확실성 정량화가 필요한가?|소표본, 사전지식 반영 필요|**[2-12] 베이즈 GLM (Bayesian GLM)**|
|⑨ 고차원·예측 중심 문제인가?|변수 수 많음, 과적합 우려|**[2-13] 정규화 GLM (Regularized GLM)**|


| 모델명 | sk-learn 사용 예제 요약 | 최적의 훈련 데이터셋 |
|---|---|---|
| **[2-1] 이항 회귀** | LogisticRegression<br>fit / predict<br>accuracy_score | UCI Breast Cancer Wisconsin<br>https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+(Diagnostic) |
| **[2-2] 포아송 회귀** | PoissonRegressor<br>fit / predict<br>mean_poisson_deviance | UCI Bike Sharing Dataset<br>https://archive.ics.uci.edu/ml/datasets/bike+sharing+dataset |
| **[2-3] 준포아송 회귀** | statsmodels.GLM<br>Poisson family<br>summary | UCI Bike Sharing Dataset |
| **[2-4] 음이항 회귀** | statsmodels.GLM<br>NegativeBinomial family<br>summary | UCI Bike Sharing Dataset |
| **[2-5] 감마 회귀** | GammaRegressor<br>fit / score | Medical Cost Dataset<br>https://archive.ics.uci.edu/ml/datasets/Insurance |
| **[2-6] 역가우시안 회귀** | statsmodels.GLM<br>InverseGaussian family<br>summary | Medical Cost Dataset |
| **[2-7] 트위디 회귀** | TweedieRegressor<br>fit / score | Medical Cost Dataset |
| **[2-8] 콕스 비례위험 모형** | lifelines.CoxPHFitter<br>fit / print_summary | Rossi Recidivism Dataset<br>https://github.com/CamDavidsonPilon/lifelines |
| **[2-9] 영과잉 모형** | ZeroInflatedPoisson<br>fit / summary | Fishing Dataset |
| **[2-10] 허들 모형** | 발생여부모델+양수모델<br>2‑stage 학습 | UCI Online Retail Dataset<br>https://archive.ics.uci.edu/ml/datasets/online+retail |
| **[2-11] 일반화 가법모형** | pygam.LinearGAM<br>fit / statistics_ | UCI Bike Sharing Dataset |
| **[2-12] 베이즈 GLM** | PyMC<br>Model / sample | 합성 데이터 또는 UCI Count Dataset |
| **[2-13] 정규화 GLM** | Lasso, Ridge<br>fit / score | sklearn Diabetes Dataset |
...
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_logistic_regression.html<br>
...
### [2-2] 포아송 회귀 (Poisson Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#poisson-regression<br>
▣ API : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.PoissonRegressor.html<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_poisson_regression.html<br>
...
### [2-3] 준포아송 회귀 (Quasi-Poisson Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#generalized-linear-models<br>
▣ API : scikit‑learn 직접 API 없음(통상적으로 statsmodels의 GLM 사용)<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/index.html<br>
...
### [2-4] 음이항 회귀 (Negative Binomial Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#generalized-linear-models<br>
▣ API : scikit‑learn 직접 API 없음(statsmodels NegativeBinomial 사용)<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/index.html<br>
...
### [2-5] 감마 회귀 (Gamma Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#gamma-regression<br>
▣ API : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.GammaRegressor.html<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_gamma_regression.html<br>
...
### [2-6] 역가우시안 회귀 (Inverse Gaussian Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#generalized-linear-models<br>
▣ API : scikit‑learn 직접 API 없음(statsmodels InverseGaussian 사용)<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/index.html<br>
...
### [2-7] 트위디 회귀 (Tweedie Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#tweedie-regression<br>
▣ API : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.TweedieRegressor.html<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_tweedie_regression.html<br>
...
### [2-8] 콕스 비례위험 모형 (Cox Proportional Hazard Model)
▣ 가이드 : scikit‑learn 공식 가이드 없음<br>
▣ API : scikit‑learn API 없음(lifelines 패키지 사용)<br>
▣ 예제 : https://lifelines.readthedocs.io/en/latest/Survival%20Regression.html<br>
...
### [2-9] 영과잉 모형 (Zero-Inflated Model)
▣ 가이드 : scikit‑learn 공식 가이드 없음<br>
▣ API : scikit‑learn API 없음(statsmodels ZeroInflatedPoisson 등 사용)<br>
▣ 예제 : https://www.statsmodels.org/stable/count_models.html<br>
...
### [2-10] 허들 모형 (Hurdle Model)
▣ 가이드 : scikit‑learn 공식 가이드 없음<br>
▣ API : scikit‑learn API 없음(이항 회귀 + count 회귀를 결합한 2‑stage 사용자 구현)<br>
▣ 예제 : https://www.statsmodels.org/stable/count_models.html<br>
...
### [2-11] 일반화 가법모형 (Generalized Additive Model, GAM)
▣ 가이드 : scikit‑learn 공식 가이드 없음<br>
▣ API : scikit‑learn API 없음(pyGAM 사용)<br>
▣ 예제 : https://pygam.readthedocs.io/en/latest/notebooks/tour_of_pygam.html<br>
...
▣ 가이드 : scikit‑learn 공식 가이드 없음<br>
▣ API : scikit‑learn API 없음(PyMC 사용)<br>
▣ 예제 : https://www.pymc.io/projects/docs/en/stable/learn/core_notebooks/glm.html<br>
...
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#ridge-regression<br>
           https://scikit-learn.org/stable/modules/linear_model.html#lasso<br>
...
**(다중 선형 회귀 예제소스) : 맨해튼 지역의 임대료(rent)를 여러 주거특성(feature)을 이용한 예측**
...
1 :	MSE = 0.41, 4 :	MSE = 0.04, 15: MSE = 182,000,000<br>
...
**(다항 선형 회귀 예제소스 실행결과 해석)**

	X축	: LSTAT — 인구 중 저소득층 비율(%)
	Y축	: MEDV — 주택가격 (단위: $1000s)
	회색 점들 : 실제 데이터 (보스턴 주택 가격 데이터)
	파란 점선 : 1차(선형) 회귀선 — 직선(Linear, R²=0.54)
	빨간 실선 : 2차(Quadratic) 회귀선 — 곡선(포물선)(Quadratic, R²=0.64)
	초록 점선 : 3차(Cubic) 회귀선 — 좀 더 복잡한 곡선(Cubic, R²=0.66)
	R²값 : 모델의 설명력 (1에 가까울수록 데이터 설명력이 높음)

	LSTAT(저소득층 비율)이 증가할수록 MEDV(주택가격)는 하락하는 비선형 관계
	1차 회귀(d=1) : 경향은 맞지만 과도하게 단순함 (R²=0.54)
	2차 회귀(d=2) : 비선형성 반영, 설명력 향상 (R²=0.64)
...
|:--|:--|:--|
| **정의** | 여러 개의 독립변수 `X₁, X₂, ..., Xp`를 선형 결합하여 종속변수 `y`를 설명하는 회귀모형 | 독립변수의 **거듭제곱항(`x², x³, ...`)** 또는 **교차항(`x₁x₂`)** 을 추가하여 비선형 관계를 선형모델로 표현하는 회귀모형 |
| **수학적 형태** | `y = β₀ + β₁x₁ + β₂x₂ + ... + βp xp + ε` | `y = β₀ + β₁x + β₂x² + β₃x³ + ... + βm xm + ε` |
| **복수인 것(많아지는 것)** | 독립변수의 개수 (변수가 여러 개) | 변수의 항(term) — 거듭제곱항·교차항이 많아짐 |
| **핵심 아이디어** | 여러 변수의 **선형 결합(linear combination)** 으로 관계를 설명 | 하나의 변수라도 **곡선형(비선형) 관계**를 표현하기 위해 변수의 거듭제곱항을 추가 |
| **관계 형태** | 입력변수와 출력 간 **직선(평면)** 관계 | 입력변수와 출력 간 **곡선** 또는 **비선형** 관계 |
| **표현력(모델 복잡도)** | 낮음 — 해석이 단순하고 직관적 | 높음 — 비선형 패턴을 표현 가능하지만 과적합 위험 있음 |
| **장점** | - 단순하고 빠름<br>- 계수 해석이 쉬움 (`β₁`은 `x₁`이 1 증가할 때 `y`의 평균 변화량)<br>- 해석적 연구에 적합 | - 비선형 관계도 설명 가능<br>- 선형회귀의 단순성(해석·최적화)을 유지하면서 표현력 향상<br>- 구현이 용이 (예: `PolynomialFeatures`) |
| **단점** | - 비선형 관계는 설명 불가<br>- 변수 간 상호작용 무시 가능<br>- 설명력이 제한될 수 있음 | - 고차항일수록 과적합(overfitting) 위험<br>- 다중공선성(collinearity) 발생<br>- 해석이 어려움 |
| **완화 방법** | 변수 선택, 스케일링, 이상치 점검 | 차수 제한, 교차검증, 정규화(`Ridge`, `Lasso`, `ElasticNet`) |
| **모델 해석 초점** | 각 변수의 계수(`β`)가 의미하는 영향력(방향·크기) | 전체 곡선 형태 또는 부분의존성(Partial Dependence) 분석 |
| **평가 지표** | `R²`, RMSE, MAE, 잔차분석(등분산성·정규성) | 동일 + 교차검증을 통한 차수별 일반화 성능 평가 |
| **적용사례 (연구·산업 분야별)** | - **부동산**: 주거특성(면적, 방 수, 역세권 등)으로 임대료 예측<br>- **경제/마케팅**: 광고비·인구·소득 등으로 매출 예측<br>- **보건/의료**: 체질량지수, 혈압 등 여러 요인으로 질병위험도 예측<br>- **교육/사회**: 다양한 환경요인으로 학업성취도나 만족도 분석 | - **물리/공학**: 온도-압력-응력 간 비선형 관계 모델링<br>- **기상**: 온도에 따른 오존·습도 변화 곡선<br>- **금융**: 수익률과 위험도 간 곡선 관계<br>- **데이터분석/AI**: 단일 변수 기반의 복잡한 비선형 패턴 적합(예: 매출-가격 곡선) |
...
| 모델명 | 수식 | 수식의 항목 설명 | 그래프 형태 | 적용 분야 |
|---|---|---|---|---|
| **[4-1] 단변량 다항 회귀** | $y = \beta_0 + \beta_1 x + \beta_2 x^2 + \cdots + \beta_d x^d + \varepsilon$ | $y$: 종속변수<br>$x$: 단일 독립변수<br>$\beta_0$: 절편<br>$\beta_k$: $k$차 다항 계수<br>$d$: 다항 차수<br>$\varepsilon$: 오차항 | 곡선 형태의 2차원 비선형 곡선 | 물리 실험 곡선 적합, 단일 변수 성장·반응 모델 |
| **[4-2] 다변량 다항 회귀** | $y = \beta_0 + \sum_i \beta_i x_i + \sum_{i,j} \beta_{ij} x_i x_j + \sum_i \beta_{ii} x_i^2 + \cdots + \varepsilon$ | $x_i$: 다변량 독립변수<br>$\beta_i$: 1차 계수<br>$\beta_{ij}$: 교호항 계수<br>$\beta_0$: 절편<br>$\varepsilon$: 오차항 | 고차원 비선형 곡면 | 경제·사회 데이터 상호작용 분석, 공정 모델링 |
| **[4-3] 직교 다항 회귀** | $y = \beta_0 + \sum_{k=1}^{d} \beta_k \phi_k(x) + \varepsilon$ | $\phi_k(x)$: 직교 다항 기저 함수<br>$\beta_k$: 기저 계수<br>$d$: 차수<br>$\varepsilon$: 오차항 | 진동이 완화된 매끄러운 곡선 | 수치해석, 실험설계, 고차 다항 안정 추정 |
| **[4-4] 부분 최소제곱 다항 회귀** | $X^{(p)} = T P^\top,\; y = T c + \varepsilon$ | $X^{(p)}$: 다항 확장 설계행렬<br>$T$: 잠재요인 점수<br>$P$: 적재행렬<br>$c$: 회귀계수<br>$\varepsilon$: 오차항 | 저차 잠재공간 회귀 곡면 | 화학계량학, 스펙트럼 분석, 고차원 회귀 |
...
# [5] 정규화 (Regularization) : 벌점부여 (Penalized) 선형 회귀
비용함수에 alpha값으로 패널티를 부여해서 회귀계수값의 크기를 감소시켜서 과적합을 개선<br>

(정규화의 필요성)

| 문제                            | 설명                                             | 결과                     |
| ----------------------------- | ---------------------------------------------- | ---------------------- |
| 과적합 (Overfitting)         | 변수가 많거나 다항항이 많으면, 훈련데이터에는 완벽히 맞지만 새 데이터엔 성능 저하 | 일반화 성능 저하              |
| 다중공선성 (Multicollinearity) | 변수들 간 상관관계가 높아 β 추정이 불안정해짐                     | 회귀계수 폭발(계수값이 비정상적으로 큼) |
...
최소제곱추정치(OLS)가 편향되지 않더라도 분산이 커서 관측값이 실제값에서 크게 벗어나는 다중공선성(multicollinearity)이 발생할 경우,<br> 
회귀 분석 추정치에 치우침 정도를 추가하여 표준오차를 줄이기 위해 사용<br>
...
|모델명|이론 개요|특징|
|---|---|---|
|**[5-1] 릿지 회귀**|L2 노름을 벌점으로 추가한 선형 회귀<br>→ 계수 크기를 축소하여 분산을 감소시키는 정규화 회귀|기본 선형 회귀에 L2 벌점 추가<br>→ 계수는 0이 되지 않으며 다중공선성 완화에 효과적|
|**[5-2] 라쏘 회귀**|L1 노름을 벌점으로 추가한 선형 회귀<br>→ 계수의 희소성을 유도하는 정규화 회귀|릿지와 달리 L1 벌점 사용<br>→ 변수 선택 기능을 내장한 회귀 모델|
|**[5-3] 엘라스틱넷 회귀**|L1과 L2 벌점을 동시에 사용하는 선형 회귀<br>→ 변수 선택과 안정성을 동시에 확보|라쏘와 릿지의 절충 모델<br>→ 상관된 변수 그룹을 함께 선택 가능|
|**[5-4] 그룹 라쏘 회귀**|사전에 정의된 변수 그룹 단위로 L1 벌점을 부여한 회귀<br>→ 그룹 단위 변수 선택 수행|라쏘를 그룹 구조로 확장<br>→ 그룹 전체가 선택되거나 제거됨|
|**[5-5] 희소 그룹 라쏘 회귀**|그룹 라쏘와 라쏘를 결합한 정규화 회귀<br>→ 그룹 내·외부 희소성 동시 유도|그룹 단위 + 개별 변수 선택 가능<br>→ 계층적 변수 선택에 적합|
|**[5-6] 적응적 라쏘 회귀**|계수별 가중치를 적용한 L1 벌점 회귀<br>→ 중요한 변수에 덜 강한 벌점 부여|라쏘의 선택 편향을 개선<br>→ oracle property 만족 가능|
|**[5-7] SCAD 벌점 회귀**|완만 절단 절댓값 벌점(Smoothly Clipped Absolute Deviation)을 사용한 회귀<br>→ 큰 계수에 대한 편향을 완화|라쏘의 과도한 축소 문제 개선<br>→ 이론적으로 일관성 보장|
|**[5-8] MCP 벌점 회귀**|최소최대 오목 벌점(Minimax Concave Penalty)을 사용한 회귀<br>→ 라쏘보다 완만한 벌점 구조|SCAD보다 단순한 형태<br>→ 계산 효율성과 편향 감소의 균형|


|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|**[5-1] 릿지 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \lambda \Vert \beta \Vert_2^2$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\Vert\cdot\Vert_2$: L2 노름<br>$\lambda$: 정규화 강도|계수가 부드럽게 0으로 수축|다중공선성 존재 회귀|
|**[5-2] 라쏘 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \lambda \Vert \beta \Vert_1$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\Vert\cdot\Vert_1$: 절댓값 합 노름<br>$\lambda$: 정규화 강도|계수 일부가 정확히 0|고차원 변수 선택|
|**[5-3] 엘라스틱넷 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \lambda_1 \Vert \beta \Vert_1 + \lambda_2 \Vert \beta \Vert_2^2$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\lambda_1$: L1 강도<br>$\lambda_2$: L2 강도|희소성과 안정성 동시 확보|유전자·텍스트 데이터|
|**[5-4] 그룹 라쏘 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \lambda \sum_{g=1}^G \Vert \beta_g \Vert_2$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_g$: g번째 그룹 계수 벡터<br>$G$: 그룹 개수<br>$\lambda$: 정규화 강도|그룹 단위 선택|유전자 경로, 센서 그룹|
|**[5-5] 희소 그룹 라쏘 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \lambda_1 \sum_{g=1}^G \Vert \beta_g \Vert_2 + \lambda_2 \Vert \beta \Vert_1$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_g$: 그룹 계수<br>$G$: 그룹 수<br>$\lambda_1$: 그룹 벌점<br>$\lambda_2$: 개별 벌점|그룹·개별 동시 희소화|계층적 특징 선택|
|**[5-6] 적응적 라쏘 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \lambda \sum_{j=1}^p w_j \lvert \beta_j \rvert$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 계수<br>$p$: 변수 개수<br>$w_j$: 가중치<br>$\lambda$: 정규화 강도|중요 계수 덜 축소|통계적 추론 중심|
|**[5-7] SCAD 벌점 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \sum_{j=1}^p p_\lambda(\lvert \beta_j \rvert)$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 회귀계수<br>$p$: 변수 개수<br>$\lambda$: 정규화 강도<br>$p_\lambda(\cdot)$: SCAD 벌점 함수<br>$t$: 계수의 절댓값<br>$a$: SCAD 형상 파라미터 $(a>2)$|큰 계수에서 벌점 소멸|oracle property 요구 연구|
|**[5-8] MCP 벌점 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \sum_{j=1}^p p_\lambda(\lvert \beta_j \rvert)$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 회귀계수<br>$p$: 변수 개수<br>$\lambda$: 정규화 강도<br>$p_\lambda(\cdot)$: MCP 벌점 함수<br>$t$: 계수의 절댓값<br>$\gamma$: 벌점 완만도 파라미터 $(\gamma>1)$|선형 감소 후 평탄화|대규모 고차원 회귀|
...
p_\lambda(t)=
\begin{cases}
\lambda t-\frac{t^2}{2\gamma}, & 0 \le t \le \gamma\lambda \\
...
|구분|조건|모델명|
|---|---|---|
|① 다중공선성(multicollinearity)이 존재하는가?|설명변수 간 상관이 높고 계수 분산이 큼|**[5-1] 릿지 회귀(Ridge Regression, L2)**|
|② 변수 선택(variable selection)이 필요한가?|불필요한 변수를 제거하고 해석력을 높이고 싶음|**[5-2] 라쏘 회귀(Lasso Regression, L1)**|
|③ 상관된 변수들이 함께 존재하는가?|상관된 변수 중 하나만 선택되는 문제가 발생|**[5-3] 엘라스틱넷 회귀(Elastic Net Regression, L1+L2)**|
|④ 설명변수에 명확한 그룹 구조가 존재하는가?|사전 정의된 변수 묶음 단위로 선택하고 싶음|**[5-4] 그룹 라쏘 회귀 (Group Lasso Regression)**|
|⑤ 그룹 구조와 개별 변수 선택을 동시에 하고 싶은가?|그룹 전체와 그룹 내 변수 모두 희소하게 만들고 싶음|**[5-5] 희소 그룹 라쏘 회귀(Sparse Group Lasso Regression)**|
|⑥ 라쏘의 선택 편향(selection bias)이 문제되는가?|중요한 계수가 과도하게 축소되는 현상을 줄이고 싶음|**[5-6] 적응적 라쏘 회귀(Adaptive Lasso Regression)**|
|⑦ 변수 선택의 이론적 일관성이 중요한가?|oracle property를 만족하는 모델이 필요|**[5-7] SCAD 벌점 회귀(Smoothly Clipped Absolute Deviation Regression)**|
|⑧ 계산 효율과 편향 감소를 동시에 고려하는가?|비볼록 벌점을 사용하되 계산을 단순화하고 싶음|**[5-8] MCP 벌점 회귀(Minimax Concave Penalty Regression)**|
...
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_ridge_coeffs.html<br>
...
▣ API : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.Lasso.html<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_lasso_coordinate_descent_path.html<br>
...
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_elastic_net_precomputed_gram_matrix_with_weighted_samples.html<br>
...
### [5-4] 그룹 라쏘 회귀 (Group Lasso Regression)
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#lasso<br>
▣ API : 없음 (sklearn 공식 API 없음)<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/index.html<br>

	# sklearn 공식 미지원 (외부 라이브러리: group-lasso 등 사용)
	from sklearn.metrics import r2_score

	model = GroupLasso(groups=groups, group_reg=0.1)
	model.fit(X, y)
...
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_elastic_net_precomputed_gram_matrix_with_weighted_samples.html<br>
	...
	model = SparseGroupLasso(alpha=0.1, l1_ratio=0.5)
	model.fit(X, y)
...
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html#lasso<br>
▣ API : https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.Lasso.html<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_lasso_model_selection.html<br>
...
▣ 가이드 : https://scikit-learn.org/stable/modules/linear_model.html<br>
▣ API : 없음 (sklearn 공식 API 없음)<br>
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/plot_lasso_and_elasticnet.html<br>

	# sklearn 공식 미지원 (SCAD 벌점은 비볼록 최적화)
	from sklearn.metrics import r2_score

	model = SCADRegression(lambda_=0.1)
	model.fit(X, y)
...
▣ 예제 : https://scikit-learn.org/stable/auto_examples/linear_model/index.html<br>
	...
	model = MCPRegression(lambda_=0.1, gamma=3)
	model.fit(X, y)
...
|모델명|이론 개요|특징|
|---|---|---|
|**[6-1] 비선형 최소제곱 회귀**|비선형 모형에서 잔차 제곱합을 최소화하는 최적화 문제<br>→ 반복적 수치 최적화로 비선형 관계 추정|선형 회귀의 목적함수를 비선형 함수로 확장<br>→ 초기값 의존성과 지역해 문제 존재|
|**[6-2] 스플라인 기반 회귀**|구간별 다항 기저를 결합한 비선형 회귀 모형<br>→ 국소적 곡률을 유연하게 표현|고차 다항 회귀의 진동 문제를 완화<br>→ 기저 함수 기반 표현|
|**[6-3] 커널 기반 회귀**|커널 함수를 통해 암묵적 고차원 공간에서 회귀 수행<br>→ 비선형 관계를 선형 문제로 변환|명시적 특성 생성 없이 비선형 학습<br>→ 커널 트릭 사용|
|**[6-4] 고차 다항 회귀**|고차 다항식을 이용한 비선형 함수 근사<br>→ 표현력은 높으나 과적합 위험|저차 다항 회귀의 표현력 한계 극복<br>→ Bias–Variance trade-off 설명에 적합|
|**[6-5] 베이즈 비선형 회귀**|비선형 회귀계수에 확률분포를 부여<br>→ 사후분포 기반 추론 수행|점추정 대신 불확실성 추정 가능<br>→ 사전정보 반영|
|**[6-6] 신경망 기반 회귀**|다층 비선형 변환을 통한 함수 근사<br>→ 보편 근사 정리 기반|고정 기저 대신 학습된 표현 사용<br>→ 딥러닝 회귀의 기본 형태|
|**[6-7] 강건 비선형 회귀**|이상치에 둔감한 손실함수를 사용한 비선형 회귀<br>→ 데이터 왜곡 영향 완화|제곱손실을 비대칭·완만 손실로 대체<br>→ 현실 데이터에 강함|
|**[6-8] 혼합 비선형 회귀**|여러 비선형 회귀 모형의 혼합으로 데이터 생성 가정<br>→ 군집별 회귀 관계 모델링|단일 회귀선 가정 완화<br>→ EM 알고리즘 사용|
|**[6-9] 위치–규모–형상 회귀**|분포의 위치·분산·형상을 동시에 모델링<br>→ 전체 분포 학습|평균 중심 회귀를 확장<br>→ 이질적 분포 대응|
|**[6-10] 함수형 데이터 회귀**|함수 형태의 입력 또는 출력을 갖는 회귀 모형<br>→ 시간·곡선 데이터 처리|스칼라 회귀의 데이터 구조 확장<br>→ 연속 함수 공간에서 학습|
|**[6-11] 딥 가우시안 프로세스 회귀**|GP를 계층적으로 결합한 비선형 확률 모형<br>→ 깊은 커널 표현|전통 GP의 표현력 한계 극복<br>→ 확률적 딥러닝|
|**[6-12] 물리정보 기반 회귀**|물리 방정식을 손실함수에 포함한 회귀 학습<br>→ 물리 제약 반영|순수 데이터 기반 학습 보완<br>→ Scientific ML 핵심|
