...
| 분류          | 알고리즘 (영문)           | 한글명칭           | 주요 특징                 |
| ----------- | ------------------- | -------------- | --------------------- |
| **데이터 기반**  | Batch GD            | 배치 경사하강법       | 전체 데이터 사용, 안정적이지만 느림  |
|             | Stochastic GD (SGD) | 확률적 경사하강법      | 샘플 1개씩 업데이트, 빠르지만 불안정 |
|             | Mini-batch GD       | 미니배치 경사하강법     | 속도와 안정성의 균형           |
| **모멘텀 기반**  | Momentum            | 모멘텀 경사하강법      | 관성 효과로 빠른 수렴          |
|             | NAG                 | 네스테로프 가속 경사하강법 | 예측된 위치에서 기울기 계산       |
| **적응형 학습률** | Adagrad             | 아다그라드          | 희소 데이터에 유리            |
|             | RMSProp             | 알엠에스프롭         | RNN 학습에 적합            |
|             | Adam                | 아담             | 가장 널리 사용되는 기본 옵티마이저   |
|             | AdaDelta            | 아다델타           | 학습률 자동 조정             |
|             | AdamW               | 아담W            | 정규화 문제 개선             |
|             | Nadam               | 나담             | Adam + Nesterov 결합형   |
| **최신 변형**   | AMSGrad             | 에이엠에스그라드       | Adam의 불안정한 수렴 개선      |
|             | LAMB                | 램브             | 대규모 배치 학습용            |
|             | AdaBelief           | 아다빌리프          | 일반화 성능 강화             |
|             | Lion                | 라이온            | 단순하지만 높은 효율           |
|             | Lookahead           | 루카헤드           | 두 옵티마이저를 병행해 안정성 향상   |


| 구분 | 모델명 | 주요 특징 |
| --- | --- | --- |
| 배치 전략 | 배치 경사하강법(Batch Gradient Descent) | 전체 학습 데이터를 사용하여 한 번의 파라미터 업데이트를 수행한다. 기울기 추정이 정확하고 수렴 경로가 안정적이지만, 데이터 규모가 커질수록 계산 비용과 메모리 사용량이 급격히 증가한다. 선형회귀의 이론적 설명이나 기준선(baseline) 모델로 적합하다. |
|  | 미니배치 경사하강법(Mini-batch Gradient Descent) | 전체 데이터를 여러 개의 작은 배치로 나누어 순차적으로 업데이트한다. 계산 효율성과 수렴 안정성 사이의 균형이 우수하며, 병렬화가 가능해 대규모 데이터에서도 실용적이다. 현대 머신러닝에서 가장 일반적으로 사용되는 배치 전략이다. |
| 1차 최적화 | 확률적 경사하강법(Stochastic Gradient Descent) | 샘플 하나 또는 매우 작은 단위로 파라미터를 즉시 업데이트한다. 계산 속도가 빠르고 온라인 학습에 적합하지만, 기울기 추정의 분산이 커 수렴 과정이 불안정할 수 있다. 학습률 설정에 매우 민감하다. |
|  | 모멘텀 경사하강법(Momentum Gradient Descent) | 이전 기울기의 누적 정보를 속도 항으로 사용하여 업데이트한다. 진동을 줄이고 평탄한 방향으로는 빠르게 이동하여 수렴 속도를 개선한다. 단순 SGD의 느린 수렴 문제를 효과적으로 완화한다. |
|  | 네스테로프 가속 경사하강법(Nesterov Accelerated Gradient) | 현재 위치가 아닌, 모멘텀에 의해 이동할 미래 위치에서 기울기를 미리 계산한다. 과도한 이동을 사전에 보정하여 모멘텀보다 더 안정적이고 이론적으로 수렴 특성이 우수하다. |
|  | 아담 옵티마이저(Adam) | 모멘텀과 적응형 학습률을 결합한 방법으로, 각 파라미터별로 학습률을 자동 조정한다. 초기 설정에 비교적 둔감하고 빠른 수렴을 보여 기본 선택지로 널리 사용된다. |
|  | 아담W 옵티마이저(AdamW) | Adam에서 가중치 감쇠(weight decay)를 기울기 업데이트와 분리하여 적용한다. 정규화 효과가 명확해지고 일반화 성능이 개선된다. Adam의 구조적 한계를 보완한 실무 표준에 가깝다. |
| 메타 옵티마이저 | 루카헤드(Lookahead) | 기본 옵티마이저를 감싸서 빠른 업데이트와 느린 업데이트를 병행한다. 빠른 가중치는 탐색을 담당하고, 느린 가중치는 안정성을 제공한다. SGD, Adam 등 다양한 옵티마이저와 결합 가능하다. |
| 2차 최적화 | 정규방정식(Normal Equation) | 손실 함수의 미분을 0으로 두어 닫힌형 해를 직접 계산한다. 반복 학습이 필요 없고 결과가 정확하지만, 특성 수가 많아질수록 행렬 역연산 비용이 급격히 증가한다. 데이터 규모가 작은 선형회귀에 적합하다. |
|  | 뉴턴 방법(Newton’s Method) | 1차 미분과 함께 2차 미분(Hessian)을 사용하여 파라미터를 업데이트한다. 매우 빠른 수렴을 보이지만 Hessian 계산과 역행렬 연산 비용이 크다. |
|  | 제한 메모리 BFGS(Limited-memory BFGS) | Hessian을 명시적으로 계산하지 않고 근사하여 사용하는 준-2차 방법이다. 대규모 선형회귀와 로지스틱 회귀에서 효율성과 수렴 성능이 뛰어나 실제 연구 및 산업 현장에서 자주 활용된다. |
| 학습률 제어 | 학습률 감소 기법(Learning Rate Decay) | 학습이 진행됨에 따라 학습률을 점진적으로 감소시켜 초기에는 빠른 탐색을, 후반에는 안정적인 수렴을 유도한다. 고정 학습률 대비 수렴 안정성이 높다. |
|  | 워밍업(Warm-up) | 학습 초반에는 매우 작은 학습률에서 시작하여 점진적으로 증가시킨다. 초기 기울기 폭주를 방지하고, 특히 미니배치 학습이나 대규모 배치 학습에서 안정적인 초기 수렴을 돕는다. |
