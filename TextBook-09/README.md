...
|**[5-2] 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \Vert \beta \Vert_1$|$y$: 반응변수<br>$X$: 설계행렬<br>$\beta$: 회귀계수<br>$\Vert\cdot\Vert_1$: 절댓값 합 노름<br>$\lambda$: 정규화 강도|계수 일부가 정확히 0|고차원 변수 선택|
|**[5-3] 엘라스틱넷 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda_1 \Vert \beta \Vert_1 + \lambda_2 \Vert \beta \Vert_2^2$|$y$: 반응변수<br>$X$: 설계행렬<br>$\beta$: 회귀계수<br>$\lambda_1$: L1 강도<br>$\lambda_2$: L2 강도|희소성과 안정성 동시 확보|유전자·텍스트 데이터|
|**[5-4] 그룹 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \sum_{g=1}^G \Vert \beta_g \Vert_2$|$y$: 반응변수<br>$X$: 설계행렬<br>$\beta$: 회귀계수<br>$\beta_g$: g번째 그룹 계수 벡터<br>$G$: 그룹 개수<br>$\lambda$: 정규화 강도|그룹 단위 선택|유전자 경로, 센서 그룹|
|**[5-5] 희소 그룹 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda_1 \sum_{g=1}^G \Vert \beta_g \Vert_2 + \lambda_2 \Vert \beta \Vert_1$|$y$: 반응변수<br>$X$: 설계행렬<br>$\beta$: 회귀계수<br>$\beta_g$: 그룹 계수<br>$G$: 그룹 수<br>$\lambda_1$: 그룹 벌점<br>$\lambda_2$: 개별 벌점|그룹·개별 동시 희소화|계층적 특징 선택|
|**[5-6] 적응적 라쏘 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \lambda \sum_{j=1}^p w_j \lvert \beta_j \rvert$|$y$: 반응변수<br>$X$: 설계행렬<br>$\beta$: 회귀계수<br>$\beta_j$: j번째 계수<br>$p$: 변수 개수<br>$w_j$: 가중치<br>$\lambda$: 정규화 강도|중요 계수 덜 축소|통계적 추론 중심|
