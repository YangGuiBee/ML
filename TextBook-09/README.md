...
| **[2-2] Poisson Regression**<br>(포아송 회귀) | $Y \sim \mathrm{Poisson}(\mu),\ \log(\mu)=X\beta$ | $Y$: 사건 발생 횟수<br>$\mu$: 단위당 평균발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 지수적 증가 곡선 | 사고·방문·클릭수 |
| **[2-3] Quasi-Poisson Regression**<br>(준포아송 회귀) | $\mathbb{E}(Y)=\mu,\ \mathrm{Var}(Y)=\phi\mu$ | $Y$: count 반응변수<br>$\mu$: 평균발생률<br>$\phi$: 과산포 조정 계수 | 분산 확장된 count 분포 | 환경·역학 이벤트 |
| **[2-4] Negative Binomial Regression**<br>(음이항 회귀) | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\mu+\alpha\mu^2$ | $Y$: count 반응변수<br>$\mu$: 평균발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수<br>$\alpha$: 과산포 강도 | 두꺼운 꼬리 count 분포 | 질병·범죄 발생 |
...
| **[2-9] Zero-Inflated Model**<br>(영과잉 모형) | $P(Y=0)=\pi+(1-\pi)f(0)$ | $Y$: count 반응변수<br>$\pi$: 구조적 0발생확률<br>$f$: count 분포 함수 | 0에서 질량 집중 | 희소 count 데이터 |
