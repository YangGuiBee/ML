...
(실행 결과 해석) : 은행 창구 방문 고객 수(시간대별 고객 방문 수를 인력 배치 최적화에 활용)

	MAE: 1.196 → 평균절대오차(Mean Absolute Error): 실제 방문 고객수와 예측값의 평균 차이가 약 1.2명 수준(시간대별로 ±1명 내외의 오차로 예측)
	Mean Poisson Deviance: 1.151 → 포아송 분포에 기반한 적합도 지표인 포아송 손실(Deviance)은 값이 작을수록 모델이 데이터에 잘 맞음을 의미(우수)
...
|   No  | 입력 조건 요약                               |  예측 방문 수  | 해석                                                   |
| :---: | :------------------------------------- | :-------: | :--------------------------------------------------- |
| 0 | 16시 / 주중 / 월급날 / 비X / 22°C / 대형지점  | 7.96명 | 영업 후반(16시) + 월급날 + 좋은 날씨 조건 → 방문 피크 시간대. 약 8명 내외 예상. |
| 1 | 11시 / 주중 / 월급날X / 비O / 18°C / 소형지점 | 1.28명 | 오전 방문이지만 비가 오고, 소형지점이며, 월급날 아님 → 방문 거의 없음(1명 수준).    |
| 2 | 13시 / 주말 / 월급날X / 비X / 27°C / 중형지점 | 0.99명 | 주말 + 점심시간 + 중형지점 → 방문이 가장 적은 조건 (1명 미만).             |
