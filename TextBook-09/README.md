...
---
	...
	[4] 다항 선형 회귀 (Polynomial Linear Regression)
   	...
   	[5] 정규화 (Regularization) : 벌점부여 (Penalized) 선형 회귀
		...
		[5-5] SCAD (Smoothly Clipped Absolute Deviation)
		[5-6] MCP (Minimax Concave Penalty)
...
|**[5-7] 완만 절단 절댓값 벌점 회귀**|비볼록 SCAD 벌점을 사용한 회귀<br>→ 큰 계수에 대한 편향을 완화|라쏘의 과도한 축소 문제 개선<br>→ 이론적으로 일관성 보장|
|**[5-8] 최소최대 오목 벌점 회귀**|비볼록 MCP 벌점을 사용한 회귀<br>→ 라쏘보다 완만한 벌점 구조|SCAD보다 단순한 형태<br>→ 계산 효율성과 편향 감소의 균형|
...
|**[5-7] 완만 절단 절댓값 벌점 회귀**|$\sum_j p_\lambda(\lvert \beta_j \rvert)$|$p_\lambda$: SCAD 벌점 함수|큰 계수 완만 축소|이론적 일관성 요구 연구|
|**[5-8] 최소최대 오목 벌점 회귀**|$\sum_j p_\lambda(\lvert \beta_j \rvert)$|$p_\lambda$: MCP 벌점 함수|비볼록, 계산 효율적|대규모 고차원 회귀|
