...
|---|---|---|---|---|
|[2-1] Binomial Regression<br>(이항 회귀)|$Y \sim \mathrm{Binomial}(n,p),\;\log\frac{p}{1-p}=X\beta$|$Y$: 성공 횟수, $n$: 시행 수, $p$: 성공확률, $X$: 설명변수 행렬, $\beta$: 회귀계수|S자형 확률 곡선|임상시험 성공 확률|
|[2-2] Poisson Regression<br>(포아송 회귀)|$Y \sim \mathrm{Poisson}(\lambda),\;\log(\lambda)=X\beta$|$\lambda$: 단위당 발생률, 로그 링크로 양수 제약|지수적 증가 곡선|사고 발생 건수|
|[2-3] Quasi-Poisson Regression<br>(준포아송 회귀)|$\mathbb{E}(Y)=\mu,\;\mathrm{Var}(Y)=\phi\mu$|$\mu$: 평균, $\phi$: 분산 확장 계수|분산이 넓은 count 분포|환경 이벤트 빈도|
|[2-4] Negative Binomial Regression<br>(음이항 회귀)|$\mathrm{Var}(Y)=\mu+\alpha\mu^2$|$\alpha$: 과산포 강도, 평균 제곱 항 포함|두꺼운 꼬리 count 분포|의료 방문 횟수|
|[2-5] Gamma Regression<br>(감마 회귀)|$Y \sim \mathrm{Gamma}(k,\theta),\;\log(\mu)=X\beta$|$k$: 형태모수, $\theta$: 척도모수, $\mu$: 평균|우측 비대칭 연속 분포|보험금 규모|
|[2-6] Inverse Gaussian Regression<br>(역가우시안 회귀)|$\mathrm{Var}(Y)=\phi\mu^3$|$\mu$: 평균, $\phi$: 분산 계수, 평균 증가 시 분산 급증|매우 긴 우측 꼬리|공정 소요시간|
|[2-7] Tweedie Regression<br>(트위디 회귀)|$\mathrm{Var}(Y)=\phi\mu^p$|$p$: 분포 지수, $\phi$: 분산 계수, $p$에 따라 분포 결정|0 포함 연속 분포|보험 순손실|
|[2-8] Cox Proportional Hazard Model<br>(콕스 비례위험 모형)|$h(t)=h_0(t)\exp(X\beta)$|$h(t)$: 위험함수, $h_0(t)$: 기저위험, $\exp(X\beta)$: 상대위험|시간-위험 곡선|생존 분석|
|[2-9] Zero-Inflated Model<br>(영과잉 모형)|$P(Y=0)=\pi+(1-\pi)f(0)$|$\pi$: 구조적 0 확률, $f$: count 분포 밀도|0에서 질량 집중|사고 0건 지역|
|[2-10] Hurdle Model<br>(허들 모형)|$P(Y>0)=g(X\beta),\;Y^+ \sim f$|$g$: 발생 여부 모델, $Y^+$: 양의 반응변수 분포|0 이후 분포 분리|소비 발생 분석|
|[2-11] Generalized Additive Model<br>(일반화 가법모형)|$g(\mu)=\beta_0+\sum_j f_j(x_j)$|$f_j$: 변수별 비선형 함수, $g$: 링크함수|부드러운 비선형 곡선|환경·역학 분석|
|[2-12] Bayesian GLM<br>(베이즈 GLM)|$p(\beta\mid y)\propto p(y\mid\beta)p(\beta)$|$p(\beta)$: 사전분포, $p(y\mid\beta)$: 우도, $p(\beta\mid y)$: 사후분포|분포 기반 추정|소표본 추론|
|[2-13] Regularized GLM<br>(정규화 GLM)|$\min\{-\ell(\beta)+\lambda\lVert\beta\rVert_p\}$|$\ell(\beta)$: 로그우도, $\lambda$: 규제 강도, $\lVert\beta\rVert_p$: 벌점항|계수 수축 곡선|고차원 데이터|
