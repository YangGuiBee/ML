...
|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|**[5-1] 릿지 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \lambda \lVert \beta \rVert_2^2$|$y$: 반응변수; $X$: 설계행렬; $\beta$: 회귀계수; $\lambda$: 정규화 강도|계수 크기를 부드럽게 축소|다중공선성 존재하는 회귀 문제|
|**[5-2] 라쏘 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \lambda \lVert \beta \rVert_1$|$\lVert \beta \rVert_1$: 절댓값 합 벌점; 나머지 항은 릿지와 동일|계수 일부가 정확히 0|고차원 변수 선택 문제|
|**[5-3] 엘라스틱넷 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \lambda_1 \lVert \beta \rVert_1 + \lambda_2 \lVert \beta \rVert_2^2$|$\lambda_1$: L1 강도; $\lambda_2$: L2 강도|희소성 + 안정성 동시 확보|유전자·텍스트 데이터|
|**[5-4] 그룹 라쏘 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \lambda \sum_g \lVert \beta_g \rVert_2$|$\beta_g$: g번째 변수 그룹 계수|그룹 단위 선택|유전자 경로, 다중 센서 데이터|
|**[5-5] 희소 그룹 라쏘 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \lambda_1 \sum_g \lVert \beta_g \rVert_2 + \lambda_2 \lVert \beta \rVert_1$|그룹 벌점과 개별 벌점 동시 적용|계층적 희소 구조|멀티레벨 특징 선택|
|**[5-6] 적응적 라쏘 회귀**|$\min_\beta \lVert y - X\beta \rVert_2^2 + \lambda \sum_j w_j \lvert \beta_j \rvert$|$w_j$: 초기 추정치 기반 가중치|중요 변수 편향 감소|통계적 추론 중심 회귀|
|**[5-7] SCAD 벌점 회귀**|$\sum_j p_\lambda(\lvert \beta_j \rvert)$|$p_\lambda$: SCAD 벌점 함수|큰 계수 완만 축소|이론적 일관성 요구 연구|
|**[5-8] MCP 벌점 회귀**|$\sum_j p_\lambda(\lvert \beta_j \rvert)$|$p_\lambda$: MCP 벌점 함수|비볼록, 계산 효율적|대규모 고차원 회귀|
...
|**[5-7] SCAD 벌점 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(\lvert \beta_j \rvert)$<br>$p'_\lambda(t)=\begin{cases}\lambda,\;0\le t\le\lambda;\;\frac{a\lambda-t}{a-1},\;\lambda<t\le a\lambda;\;0,\;t>a\lambda\end{cases}$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 회귀계수<br>$p$: 변수 개수<br>$\lambda$: 정규화 강도<br>$p_\lambda(\cdot)$: SCAD 벌점 함수<br>$p'_\lambda(\cdot)$: 벌점의 미분<br>$t$: 계수의 절댓값<br>$a$: SCAD 형상 파라미터 $(a>2)$|큰 계수에서 벌점 소멸|oracle property 요구 연구|
|**[5-8] MCP 벌점 회귀**|$\min_\beta \Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(\lvert \beta_j \rvert)$<br>$p_\lambda(t)=\begin{cases}\lambda t-\frac{t^2}{2\gamma},\;0\le t\le\gamma\lambda;\;\frac{1}{2}\gamma\lambda^2,\;t>\gamma\lambda\end{cases}$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 회귀계수<br>$p$: 변수 개수<br>$\lambda$: 정규화 강도<br>$p_\lambda(\cdot)$: MCP 벌점 함수<br>$t$: 계수의 절댓값<br>$\gamma$: 벌점 완만도 파라미터 $(\gamma>1)$|선형 감소 후 평탄화|대규모 고차원 회귀|
