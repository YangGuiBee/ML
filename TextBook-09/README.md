...
|---|---|---|---|---|
| **[2-1] Binomial / Logistic Regression**<br>(이항·로지스틱 회귀) | $Y \sim \mathrm{Binomial}(n,p),\ \log(\frac{p}{1-p})=X\beta$ | $Y$: 성공 횟수<br>$n$: 시행 수<br>$p$: 성공확률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 벡터 | S자형 확률 곡선 | 이진 분류,<br> 성공 확률 예측,<br>임상시험,<br>신용평가 |
| **[2-2] Poisson Regression**<br>(포아송 회귀) | $Y \sim \mathrm{Poisson}(\mu),\ \log(\mu)=X\beta$ | $Y$: 사건 발생 횟수<br>$\mu$: 평균발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 지수적 증가 곡선 | 사고·방문·클릭수 |
...
| **[2-5] Gamma Regression**<br>(감마 회귀) | $Y \sim \mathrm{Gamma},\ \log(\mu)=X\beta$ | $Y$: + 연속 반응변수<br>$\mu$: 평균<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 우측 꼬리 분포 | 보험금, 의료비 |
| **[2-6] Inverse Gaussian Regression**<br>(역가우시안 회귀) | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\phi\mu^3$ | $Y$: +연속 반응변수<br>$\mu$: 평균<br>$\phi$: 분산 계수<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 매우 긴 우측 꼬리 | 공정·반응시간 |
| **[2-7] Tweedie Regression**<br>(트위디 회귀) | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\phi\mu^p$ | $Y$: 0포함 연속 반응변수<br>$\mu$: 평균<br>$\phi$: 분산 계수<br>$p$: 분포 지수<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 0 포함 연속 분포 | 보험 순손실 |
...
| **[2-11] Generalized Additive Model**<br>(일반화 가법모형) | $g(\mu)=\beta_0+\sum_j f_j(x_j)$ | $\mu$: 반응변수 평균<br>$g$: 링크함수<br>$\beta_0$: 절편<br>$x_j$: 설명변수<br>$f_j$: 비선형 스무딩함수 | 부드러운 비선형 곡선 | 환경·역학 |
