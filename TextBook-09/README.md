...
###### HR = 1 → 영향 없음, HR > 1 → 위험 증가, HR < 1 → 위험 감소 <br>
...
| **수식** | <img src="https://latex.codecogs.com/png.image?\dpi{120}\bg_white\log(\lambda_i)=\beta_0+\beta_1x_{1i}+\cdots+\beta_px_{pi}" alt="Poisson formula"/> <br> <img src="https://latex.codecogs.com/png.image?\dpi{120}\bg_white\lambda_i=e^{\eta_i}" alt="Poisson formula2"/> → 평균 사건수 | ![Cox](./images/cox_eq.png) <br> ![Baseline hazard](./images/cox_baseline_en.png) |
...
4	MSE = 4.32e-02	MSE = 0.04
15	MSE = 1.82e+08	MSE = 182000000
...
|모델명|이론 개요|특징|
|---|---|---|
|[4-1] 단변량 다항 회귀|단일 독립변수를 다항식 기저로 확장하여 선형 회귀로 추정하는 모델로 입력 변수 하나의 비선형 관계를 선형 회귀 틀에서 모델링함|기본 선형 회귀를 단일 변수의 고차 다항식으로 확장한 모델로 단순하지만 차수가 증가할수록 과적합과 진동 현상이 발생함|
|[4-2] 다변량 다항 회귀|여러 독립변수와 그 교호항을 포함한 다항식 회귀 모델로 변수 간 상호작용을 포함한 비선형 구조를 표현함|단변량 다항 회귀를 다변수로 확장한 모델로 교호항 증가로 설명력은 높으나 차원의 저주와 다중공선성 문제가 발생함|
|[4-3] 직교 다항 회귀|서로 직교하는 다항 기저를 사용한 다항 선형 회귀로 다중공선성을 줄이고 수치적 안정성을 향상시킴|일반 다항 회귀의 기저를 직교 기저로 변환한 모델로 회귀계수 해석은 어렵지만 추정 안정성이 우수함|
|[4-4] 부분 최소제곱 다항 회귀|다항 특성 공간에서 부분 최소제곱 방법을 적용한 회귀 모델로 반응변수와 공분산이 큰 잠재요인을 이용해 회귀를 수행함|다항 회귀에 차원 축소를 결합한 모델로 고차 다항 및 고차원 데이터에서도 안정적인 추정이 가능함|
|[4-5] 커널 다항 회귀|다항 커널을 이용해 다항 특성 공간에서 선형 회귀를 수행하는 모델로 명시적 특성 생성 없이 비선형 관계를 모델링함|명시적 다항 확장을 커널 트릭으로 대체한 모델로 고차 다항 모델을 계산 효율적으로 구현할 수 있음|


|모델명|수식|수식의 항목 설명|그래프형태|적용분야|
|---|---|---|---|---|
|[4-1] 단변량 다항 회귀|$y = \beta_0 + \beta_1 x + \beta_2 x^2 + \cdots + \beta_d x^d + \varepsilon$|$y$: 종속변수; $x$: 단일 독립변수; $\beta_0$: 절편; $\beta_k$: $k$차 다항 계수; $d$: 다항 차수; $\varepsilon$: 오차항|곡선 형태의 2차원 비선형 곡선|물리 실험 곡선 적합, 단일 변수 성장 모델|
|[4-2] 다변량 다항 회귀|$y = \beta_0 + \sum_i \beta_i x_i + \sum_{i,j} \beta_{ij} x_i x_j + \varepsilon$|$x_i$: 다변량 독립변수; $\beta_i$: 1차 계수; $\beta_{ij}$: 교호항 계수; $\beta_0$: 절편; $\varepsilon$: 오차항|고차원 곡면 형태|경제·사회 데이터의 변수 간 상호작용 분석|
|[4-3] 직교 다항 회귀|$y = \beta_0 + \sum_{k=1}^{d} \beta_k \phi_k(x) + \varepsilon$|$\phi_k(x)$: 서로 직교하는 다항 기저 함수; $\beta_k$: 직교 기저 계수; $d$: 차수; $\varepsilon$: 오차항|진동이 완화된 매끄러운 곡선|수치해석, 실험설계, 고차 다항 안정 추정|
|[4-4] 부분 최소제곱 다항 회귀|$X^{(p)} = T P^\top,\; y = T c + \varepsilon$|$X^{(p)}$: 다항 확장 설계행렬; $T$: 잠재요인 점수; $P$: 적재행렬; $c$: 회귀계수; $\varepsilon$: 오차항|저차 잠재공간에서의 회귀 곡면|화학계량학, 스펙트럼 분석, 고차원 회귀|
