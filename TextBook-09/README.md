...
0, & t > a\lambda
\end{cases}$|$y$: 반응변수 벡터; $X$: 설계행렬; $\beta$: 회귀계수 벡터; $\beta_j$: j번째 회귀계수; $p$: 변수 개수; $\lambda$: 정규화 강도; $p_\lambda(\cdot)$: SCAD 벌점 함수; $p'_\lambda(\cdot)$: SCAD 벌점의 미분; $t$: 계수의 절댓값; $a$: SCAD 형상 파라미터 $(a>2)$|큰 계수에서 벌점이 0으로 수렴|oracle property 요구 연구|
...
\end{cases}$|$y$: 반응변수 벡터; $X$: 설계행렬; $\beta$: 회귀계수 벡터; $\beta_j$: j번째 회귀계수; $p$: 변수 개수; $\lambda$: 정규화 강도; $p_\lambda(\cdot)$: MCP 벌점 함수; $t$: 계수의 절댓값; $\gamma$: 벌점 완만도 파라미터 $(\gamma>1)$|벌점이 선형적으로 감소 후 평탄화|대규모 고차원 회귀|
