...
| **[2-13] Regularized GLM**<br>(정규화 GLM) | $\min\{-\ell(\beta)+\lambda(\sum_j\sqrt{\beta_j^2}+\sum_j\beta_j^2)\}$ | $\ell(\beta)$: 로그우도<br>$\sum_j\sqrt{\beta_j^2}$: Lasso<br>$\sum_j\beta_j^2$: Ridge<br>결합 시 Elastic Net | 계수 수축 경로 | 고차원 데이터 |
