...
| :---------------------- | ---------------: | ---------------------------------------: | :-----: | :---------------------------------------------------------------------------------------------------------------------------------- |
| **const**               | -0.0000 → 1.0000 |                             SE/t/p = NaN |    -    | Cox 모형의 상수항(const)은 일반 OLS처럼 해석되지 않으며, 분산이 정의되지 않아 NaN이 발생. 기저위험함수 h₀(t)가 별도로 존재하므로 상수항은 해석 대상이 아님. 각 위험요인별 HR 해석에 집중 필요.           |
| **security_level_inv**  | -0.0506 → 0.9507 | p=0.4222, 95% CI=[0.8403, 1.0756] (1 포함) |    ❌    | 보안등급이 높을수록 침해위험 약 4.9% 감소 경향(HR<1). 그러나 p>0.05, CI에 1 포함 → 통계적으로 유의하지 않음. 단, 변수 정의(`보안등급을 뒤집은 값`)에 따라 해석 방향이 반대일 수 있음. 변수 코딩 확인 필요. |
| **patch_cycle_days**    | +0.0007 → 1.0007 | p=0.8851, 95% CI=[0.9916, 1.0098] (1 포함) |    ❌    | 패치주기 1일 증가 시 위험 약 0.07% 증가하나 매우 미세한 수준. HR≈1, p>0.05, CI에 1 포함 → 사실상 영향 없음으로 해석.                                                    |
| **exposed_to_internet** | -0.0532 → 0.9482 | p=0.7033, 95% CI=[0.7213, 1.2466] (1 포함) |    ❌    | 외부에 노출된 시스템이 침해위험 약 5.2% 낮은 경향(HR<1). 그러나 p>0.05, CI에 1 포함 → 유의하지 않음. 실제로는 네트워크 설계, WAF/IDS 등의 상호작용으로 다른 결과 가능성 있음.                 |
| **num_admins**          | +0.0400 → 1.0408 | p=0.4471, 95% CI=[0.9388, 1.1539] (1 포함) |    ❌    | 관리자 수 1명 증가 시 위험 약 4.1% 증가(HR>1). 그러나 p>0.05, CI에 1 포함 → 통계적으로 유의하지 않음. 현재 데이터로는 확정적 결론 불가.                                         |
