...
| **security_level_inv**<br>보안등급(역수형)  | -0.0506 → 0.9507 | p=0.4222, 95% CI=[0.8403, 1.0756] (1 포함) |    ❌    | 보안등급이 높을수록 침해위험 약 4.9% 감소 경향(HR<1). 그러나 p>0.05, CI에 1 포함 → 통계적으로 유의하지 않음. 단, 변수 정의(`보안등급을 뒤집은 값`)에 따라 해석 방향이 반대일 수 있음. 변수 코딩 확인 필요. |
| **patch_cycle_days**<br>패치주기(일단위)    | +0.0007 → 1.0007 | p=0.8851, 95% CI=[0.9916, 1.0098] (1 포함) |    ❌    | 패치주기 1일 증가 시 위험 약 0.07% 증가하나 매우 미세한 수준. HR≈1, p>0.05, CI에 1 포함 → 사실상 영향 없음으로 해석.                                                    |
| **exposed_to_internet**<br>외부망 노출여부 | -0.0532 → 0.9482 | p=0.7033, 95% CI=[0.7213, 1.2466] (1 포함) |    ❌    | 외부에 노출된 시스템이 침해위험 약 5.2% 낮은 경향(HR<1). 그러나 p>0.05, CI에 1 포함 → 유의하지 않음. 실제로는 네트워크 설계, WAF/IDS 등의 상호작용으로 다른 결과 가능성 있음.                 |
| **num_admins**<br>관리자 수   | +0.0400 → 1.0408 | p=0.4471, 95% CI=[0.9388, 1.1539] (1 포함) |    ❌    | 관리자 수 1명 증가 시 위험 약 4.1% 증가(HR>1). 그러나 p>0.05, CI에 1 포함 → 통계적으로 유의하지 않음. 현재 데이터로는 확정적 결론 불가.                                         |
...
| 모델                                                          | 기본 수식                                                                                                      | 그래프 형태 및 주요 적용 분야                                                                    |
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **[4-1] 단변량 다항 회귀**<br>(Univariate Polynomial Regression)   | (\hat{y}=\beta_0+\beta_1 x+\beta_2 x^2+\cdots+\beta_p x^p)                                                 | **그래프:** U자형, S자형, W자형 등 다양한 곡선<br>**적용:** 단일 변수의 비선형 관계 — 성장 곡선, 온도–압력 곡선, 거리–시간 관계 |
| **[4-2] 다변량 다항 회귀**<br>(Multivariate Polynomial Regression) | (\hat{y}=\beta_0+\sum_i \beta_i x_i + \sum_{i,j}\beta_{ij} x_i x_j + \sum_i \beta_{ii} x_i^2 + \cdots)     | **그래프:** 다차원 비선형 곡면<br>**적용:** 다요인 비선형 모델 — 제조 공정, 에너지 소비, 경제·환경 지표 예측               |
| **[4-3] 직교 다항 회귀**<br>(Orthogonal Polynomial Regression)    | (\hat{y}=\beta_0 P_0(x)+\beta_1 P_1(x)+\cdots+\beta_p P_p(x))<br>((P_k(x)): 직교 다항식)                        | **그래프:** 동일 차수 대비 진동이 적은 매끈한 곡선<br>**적용:** 다중공선성 방지 필요 — 시계열 스무딩, 물리·공학적 근사          |
| **[4-4] PLS 다항 회귀**<br>(PLS–Polynomial Regression)          | (\hat{y}=\sum_{k=1}^m c_k t_k),  (t_k=f_k(X)): 다항 기반 잠재변수                                                  | **그래프:** 차원 축소된 안정적 비선형 곡선<br>**적용:** 고차 다항·고차원 데이터 — 스펙트럼 분석, 화학계량학(Chemometrics)   |
| **[4-5] 커널 다항 회귀**<br>(Kernel Polynomial Regression)        | (K(x_i,x_j)=(\gamma x_i^\top x_j+r)^d)<br>회귀식: (\hat{y}=\sum_i \alpha_i K(x,x_i))                          | **그래프:** 고차 커널 기반의 복잡한 비선형 곡선<br>**적용:** 고비선형 패턴 — 이미지 특징, 금융 변동성, 복잡 패턴 예측          |
| **[4-6] 정규화 다항 회귀**<br>(Regularized Polynomial Regression)  | Ridge: (\min_\beta |y-X_p\beta|^2+\lambda|\beta|^2)<br>Lasso: (\min_\beta |y-X_p\beta|^2+\lambda|\beta|_1) | **그래프:** 과적합이 억제된 안정된 곡선 형태<br>**적용:** 고차 다항에서의 과적합 방지 — 금융/의료 예측, 소규모 데이터           |
