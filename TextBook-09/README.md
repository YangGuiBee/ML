...
|**[5-7] SCAD 벌점 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(|\beta_j|)$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 회귀계수<br>$p$: 변수 개수<br>$\lambda$: 정규화 강도<br>$p_\lambda(\cdot)$: SCAD 벌점 함수<br>$t$: 계수의 절댓값<br>$a$: SCAD 형상 파라미터 $(a>2)$|큰 계수에서 벌점이 0으로 수렴|oracle property 요구 연구|
|**[5-8] MCP 벌점 회귀**|$\min_\beta \;\Vert y - X\beta \Vert_2^2 + \sum_{j=1}^p p_\lambda(|\beta_j|)$|$y$: 반응변수 벡터<br>$X$: 설계행렬<br>$\beta$: 회귀계수 벡터<br>$\beta_j$: j번째 회귀계수<br>$p$: 변수 개수<br>$\lambda$: 정규화 강도<br>$p_\lambda(\cdot)$: MCP 벌점 함수<br>$t$: 계수의 절댓값<br>$\gamma$: 벌점 완만도 파라미터 $(\gamma>1)$|벌점이 선형적으로 감소 후 평탄화|대규모 고차원 회귀|
