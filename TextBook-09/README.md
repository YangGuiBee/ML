...
|---|---|---|---|---|
| **[2-1] Binomial / Logistic Regression**<br>(이항·로지스틱 회귀) | $Y \sim \mathrm{Binomial}(n,p),\ \log(\frac{p}{1-p})=X\beta$ | $Y$: 성공 횟수<br>$n$: 시행 수<br>$p$: 성공확률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 벡터 | S자형 확률 곡선 | 이진 분류, 성공 확률 예측<br>임상시험, 신용평가 |
| **[2-2] Poisson Regression**<br>(포아송 회귀) | $Y \sim \mathrm{Poisson}(\mu),\ \log(\mu)=X\beta$ | $Y$: 사건 발생 횟수<br>$\mu$: 단위당 평균 발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 지수적 증가 곡선 | 사고·방문·클릭 수 |
| **[2-3] Quasi-Poisson Regression**<br>(준포아송 회귀) | $\mathbb{E}(Y)=\mu,\ \mathrm{Var}(Y)=\phi\mu$ | $Y$: count 반응변수<br>$\mu$: 평균 발생률<br>$\phi$: 과산포 조정 계수 | 분산 확장된 count 분포 | 환경·역학 이벤트 |
| **[2-4] Negative Binomial Regression**<br>(음이항 회귀) | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\mu+\alpha\mu^2$ | $Y$: count 반응변수<br>$\mu$: 평균 발생률<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수<br>$\alpha$: 과산포 강도 | 두꺼운 꼬리 count 분포 | 질병·범죄 발생 |
| **[2-5] Gamma Regression**<br>(감마 회귀) | $Y \sim \mathrm{Gamma},\ \log(\mu)=X\beta$ | $Y$: 양의 연속 반응변수<br>$\mu$: 평균<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 우측 꼬리 분포 | 보험금, 의료비 |
| **[2-6] Inverse Gaussian Regression**<br>(역가우시안 회귀) | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\phi\mu^3$ | $Y$: 양의 연속 반응변수<br>$\mu$: 평균<br>$\phi$: 분산 계수<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 매우 긴 우측 꼬리 | 공정·반응시간 |
| **[2-7] Tweedie Regression**<br>(트위디 회귀) | $\log(\mu)=X\beta,\ \mathrm{Var}(Y)=\phi\mu^p$ | $Y$: 0 포함 연속 반응변수<br>$\mu$: 평균<br>$\phi$: 분산 계수<br>$p$: 분포 지수<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 0 포함 연속 분포 | 보험 순손실 |
| **[2-8] Cox Proportional Hazard Model**<br>(콕스 비례위험 모형) | $h(t)=h_0(t)\exp(X\beta)$ | $t$: 시간 변수<br>$h(t)$: 위험함수<br>$h_0(t)$: 기저위험함수<br>$X$: 공변량 행렬<br>$\beta$: 회귀계수 | 생존확률 감소 곡선 | 생존 분석 |
| **[2-9] Zero-Inflated Model**<br>(영과잉 모형) | $P(Y=0)=\pi+(1-\pi)f(0)$ | $Y$: count 반응변수<br>$\pi$: 구조적 0 발생 확률<br>$f$: count 분포 함수 | 0에서 질량 집중 | 희소 count 데이터 |
| **[2-10] Hurdle Model**<br>(허들 모형) | $P(Y>0)=g(X\beta),\ Y^{+}\sim f$ | $Y$: 반응변수<br>$Y^{+}$: 양의 값 부분<br>$g$: 발생 여부 모델<br>$f$: 양의 값 분포<br>$X$: 설명변수 행렬<br>$\beta$: 회귀계수 | 0 이후 분포 분리 | 소비 발생 분석 |
| **[2-11] Generalized Additive Model**<br>(일반화 가법모형) | $g(\mu)=\beta_0+\sum_j f_j(x_j)$ | $\mu$: 반응변수 평균<br>$g$: 링크함수<br>$\beta_0$: 절편<br>$x_j$: 설명변수<br>$f_j$: 비선형 스무딩 함수 | 부드러운 비선형 곡선 | 환경·역학 |
| **[2-12] Bayesian GLM**<br>(베이즈 GLM) | $p(\beta\mid y)\propto p(y\mid\beta)p(\beta)$ | $y$: 관측 데이터<br>$\beta$: 모수<br>$p(y\mid\beta)$: 우도<br>$p(\beta)$: 사전분포<br>$p(\beta\mid y)$: 사후분포 | 분포 기반 추정 | 소표본 추론 |
| **[2-13] Regularized GLM**<br>(정규화 GLM) | $\min\{-\ell(\beta)+\lambda(\sum_j\sqrt{\beta_j^2}+\sum_j\beta_j^2)\}$ | $\ell(\beta)$: 로그우도<br>$\beta_j$: 회귀계수<br>$\lambda$: 규제 강도<br>$\sum_j\sqrt{\beta_j^2}$: Lasso 항<br>$\sum_j\beta_j^2$: Ridge 항<br>결합 시 Elastic Net | 계수 수축 경로 | 고차원 데이터 |
