...
# 📘 연관규칙 알고리즘 수식 요약 (GitHub 수식 이미지 완전판 ✅)
...
| **[AR-4]** | **Multi-level Association Rules** (다계층 연관규칙) | 데이터의 계층(Level) 구조 반영 | ![](https://latex.codecogs.com/png.image?\dpi{140}Support\_l(X)=\frac{count\_l(X)}{N\_l})<br>![](https://latex.codecogs.com/png.image?\dpi{140}Confidence\_l(X\Rightarrow%20Y)=\frac{Support\_l(X\cup%20Y)}{Support\_l(X)})<br>![](https://latex.codecogs.com/png.image?\dpi{140}min\_sup\_1>min\_sup\_2>min\_sup\_3) | ![](https://latex.codecogs.com/png.image?\dpi{140}\max_{l}\;Support_{l}(X),\;Confidence_{l}(X\Rightarrow%20Y)\ge%20min\_conf_{l},\;Support_{l}(X)\ge%20min\_sup_{l}) |
| **[AR-5]** | **Multi-dimensional Association Rules** (다차원 연관규칙) | 다중 속성(Attribute) 간 연관 분석 | ![](https://latex.codecogs.com/png.image?\dpi{140}Support(A\_1=a\_1,\dots,A\_k=a\_k)=\frac{count(A\_1=a\_1,\dots,A\_k=a\_k)}{N})<br>![](https://latex.codecogs.com/png.image?\dpi{140}Confidence((A\_1=a\_1,\dots,A\_i=a\_i)\Rightarrow(A\_j=a\_j))=\frac{Support(A\_1=a\_1,\dots,A\_i=a\_i,A\_j=a\_j)}{Support(A\_1=a\_1,\dots,A\_i=a\_i)}) | ![](https://latex.codecogs.com/png.image?\dpi{140}\max_{A_{1},...,A_{k}}\;Support(A_{1}=a_{1},...,A_{k}=a_{k}),\;Confidence((A_{1}=a_{1},...,A_{i}=a_{i})\Rightarrow(A_{j}=a_{j}))\ge%20min\_conf) |
...
# 📘 연관규칙 알고리즘 수식 요약 (폰트 통일판 ✅)

| 구분 | 알고리즘 | 핵심 아이디어 | 주요 수학식 (이미지 렌더링) | 목적함수 / 평가함수 (이미지 렌더링) |
|------|-----------|----------------|-------------------------------|----------------------|
| **[AR-1]** | **Apriori** (선험적 알고리즘) | 단조성(Apriori Property)을 이용해 빈발 항목집합 탐색 | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Support(X)=\frac{count(X)}{N})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Confidence(X\Rightarrow%20Y)=\frac{Support(X\cup%20Y)}{Support(X)})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Lift(X\Rightarrow%20Y)=\frac{Support(X\cup%20Y)}{Support(X)\cdot%20Support(Y)}) | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20\max%20Support(X),\;Confidence(X\Rightarrow%20Y)\ge%20min\_conf) |
| **[AR-2]** | **FP-Growth** (Frequent Pattern Growth) | FP-Tree로 트랜잭션을 압축 저장 후 조건부 패턴 확장 | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20FP=\{(I,Support(I))\mid%20Support(I)\ge%20min\_sup\})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20FP(I)=\bigcup_{i\in%20I}FP(\mathrm{CondBase}(i))) | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Support\ge%20min\_sup;\;\text{patterns}) |
| **[AR-3]** | **Eclat** (Equivalence Class Transformation) | TID 집합의 교집합으로 Support 계산 | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Support(X)=\left%7C\bigcap_{i\in%20X}T(i)\right%7C) | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20\max_{X}\left%7C\bigcap_{i\in%20X}T(i)\right%7C\;\text{s.t.}\;\left%7C\bigcap_{i\in%20X}T(i)\right%7C\ge%20min\_sup) |
| **[AR-4]** | **Multi-level Association Rules** (다계층 연관규칙) | 데이터의 계층(Level) 구조 반영 | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Support\_l(X)=\frac{count\_l(X)}{N\_l})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Confidence\_l(X\Rightarrow%20Y)=\frac{Support\_l(X\cup%20Y)}{Support\_l(X)})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20min\_sup\_1>min\_sup\_2>min\_sup\_3) | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20\max_{l}\;Support_{l}(X),\;Confidence_{l}(X\Rightarrow%20Y)\ge%20min\_conf_{l},\;Support_{l}(X)\ge%20min\_sup_{l}) |
| **[AR-5]** | **Multi-dimensional Association Rules** (다차원 연관규칙) | 다중 속성(Attribute) 간 연관 분석 | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Support(A\_1=a\_1,\dots,A\_k=a\_k)=\frac{count(A\_1=a\_1,\dots,A\_k=a\_k)}{N})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Confidence((A\_1=a\_1,\dots,A\_i=a\_i)\Rightarrow(A\_j=a\_j))=\frac{Support(A\_1=a\_1,\dots,A\_i=a\_i,A\_j=a\_j)}{Support(A\_1=a\_1,\dots,A\_i=a\_i)}) | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20\max_{A_{1},...,A_{k}}\;Support(A_{1}=a_{1},...,A_{k}=a_{k}),\;Confidence((A_{1}=a_{1},...,A_{i}=a_{i})\Rightarrow(A_{j}=a_{j}))\ge%20min\_conf) |
| **[AR-6]** | **Artificial Immune System (AIS)** (인공면역시스템) | 항원–항체 친화도(Affinity) 기반의 진화 탐색 | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20Affinity(Ab,Ag)=\frac{match(Ab,Ag)}{%7CAg%7C})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20P(Ab\_i)=\frac{Affinity(Ab\_i,Ag)}{\sum_{j}Affinity(Ab\_j,Ag)})<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20P\_{clone}=\alpha\cdot%20Affinity(Ab\_i,Ag))<br>![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20P\_{mutation}=e^{-\beta\cdot%20Affinity(Ab\_i,Ag)}) | ![](https://latex.codecogs.com/png.image?\dpi{150}\Large%20\max_{Ab}Affinity(Ab,Ag)) |
