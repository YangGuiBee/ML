...
| **[AR-6]** | **Artificial Immune System (AIS)**<br>(인공면역시스템) | 항원–항체 친화도(Affinity) 기반의 진화 탐색 | $Affinity(Ab,Ag)=\dfrac{match(Ab,Ag)}{|Ag|}$<br>$P(Ab\_i)=\dfrac{Affinity(Ab\_i,Ag)}{\sum\_jAffinity(Ab\_j,Ag)}$<br>$P\_{clone}=\alpha\,Affinity(Ab\_i,Ag)$<br>$P\_{mutation}=e^{-\beta\,Affinity(Ab\_i,Ag)}$ | $\max Affinity(Ab,Ag)$ — 면역 선택·클론·돌연변이 기반 최적화 |
