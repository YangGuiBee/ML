...
| 장점                             | 단점                                              |
|----------------------------------|---------------------------------------------------|
| 모형의 해석이 쉽다입력값이 주어질때 설명변수의 영역을 따라가며, 출력값이 어떻게 나올 지 알기 쉬움) | 예측력이 다른 지도학습에 비해 떨어진다.|
...
▣ 회귀식  : $\widehat{f}(x) = \sum_{m=1}^{M}C_mI\left\{(x_1,x_2)\in R_m\right\}$
비용함수(cost function)를 최소로 할때 최상의 분할 : 
