...
| **평가 관점**                           | **지표**                                    | **의미 / 설명**                                                         | **주로 사용되는 알고리즘 유형**        |
| ----------------------------------- | ----------------------------------------- | ------------------------------------------------------------------- | -------------------------- |
| **① 재구성 기반 (Reconstruction-based)** | **(1) 재구성 오류 (Reconstruction Error)**     | 차원 축소 후 복원한 데이터 (\hat{X}) 와 원본 (X) 간의 평균 제곱 오차(MSE)로, 정보 손실 정도를 정량화 | PCA, SVD, NMF, Autoencoder |
|                                     | **(2) 분산 유지율 (Explained Variance Ratio)** | 각 주성분이 데이터 전체 분산 중 얼마나 설명하는지를 나타냄. 값이 높을수록 정보 보존이 잘 됨               | PCA, Linear DR             |
|                                     | **(3) 상호 정보량 (Mutual Information)**       | 축소 전후 데이터 간 **정보량(I(X;Y))** 비교로, 차원축소가 정보의존성을 얼마나 보존하는지 평가          | Autoencoder, 비선형 DR        |



| **평가 관점**                                     | **지표**                                      | **의미 / 설명**                                      | **주로 사용되는 알고리즘 유형**      |
| --------------------------------------------- | ------------------------------------------- | ------------------------------------------------ | ------------------------ |
| **② 구조 보존 기반 (Structure Preservation-based)** | **(4) Silhouette Score**                    | 축소된 저차원 공간에서 군집 간 분리도와 군집 내 응집도를 동시에 평가          | t-SNE, UMAP, Isomap      |
|                                               | **(5) Davies-Bouldin Index (DBI)**          | 각 군집의 평균 거리 대비 군집 간 거리 비율로, 값이 작을수록 클러스터링 품질이 높음 | t-SNE, UMAP, PCA+K-means |
|                                               | **(6) Adjusted Rand Index (ARI)**           | 축소 전후 혹은 레이블 간의 군집 일치도를 평가 (1에 가까울수록 일치)         | 지도기준 존재 시 성능 비교          |
|                                               | **(7) Normalized Mutual Information (NMI)** | 축소된 결과의 군집과 원본 레이블(또는 고차원 구조) 간 상호 정보량 기반 유사도 평가 | 클러스터 구조 비교용              |
