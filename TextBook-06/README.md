...
▣ 정의: 데이터가 <ins>특정한 확률 분포(주로 가우시안 분포)들의 혼합으로 생성되었다고 가정하고 이를 추정</ins>하는 방식.<br>
...
▣ 정의: 기본적인 빈발 패턴 탐색을 넘어 <ins>데이터의 계층 구조, 다차원 속성, 또는 양적 속성을 반영하여 더 정교한 규칙을 추출</ins>하는 방식.<br>
...
▣ 정의: 데이터 간의 <ins>'시간적 순서' 또는 '사건의 선후 관계'를 고려</ins>하여 발생하는 빈발 패턴을 찾아내는 방식.<br>
...
▣ 정의: <ins>그래프 구조, 시공간 데이터, 또는 데이터의 유용성(이익) 등 특정한 제약 조건이나 데이터 형태에 특화된 패턴</ins>을 탐색하는 방식.<br>
...
▣ 정의: 고차원 공간에 복잡하게 꼬여 있거나 휘어져 있는 구조(매니폴드)를 펼쳐서 데이터를 저차원으로 투영하는 방식.<br>
매니폴드(Manifold) : 고차원 공간에 존재하지만, 국소적으로 보면 저차원의 평면처럼 보이는 구조(고차원 공간에 존재하지만, 국소적으로 보면 저차원의 평면처럼 보이는 구조)<br>
<img width ='600' height = '400' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-06/images/Manifold.png'> 
<br>
...
▣ 정의: <ins>데이터가 가진 특수한 구조(텍스트의 토픽 구조, 다중 소스 데이터 간의 상관관계 등)에 대한 통계적 가정이나 도메인 지식을 반영하여, 범용적인 방식보다 해당 데이터 타입에 최적화된 형태로 정보를 압축</ins>하는 방식.<br>
...
▣ 정의: 데이터가 특정 확률 분포를 따른다고 가정하고, 해당 분포에서 <ins>발생 확률이 매우 낮은 데이터를 이상치로 판정</ins>하는 방식.<br>
...
▣ 정의: 데이터 포인트 간의 거리를 측정하여 <ins>주변에 다른 데이터가 멀리 떨어져 있는 객체를 이상치로 분류</ins>하는 방식.<br>
...
▣ 정의: 각 데이터 포인트 주변의 국소적 밀도를 측정하여 <ins>이웃에 비해 밀도가 현저히 낮은 데이터</ins>를 찾아내는 방식.<br>
...
▣ 정의: <ins>데이터가 가진 '보통의 법칙'을 먼저 배운 뒤, 그 법칙에서 크게 벗어나거나 혼자 뚝 떨어져 있는 데이터</ins>를 이상치로 지목하는 방식.<br>
...
▣ 정의: 여러 개의 <ins>이상치 탐지 모델을 결합</ins>하여 개별 모델의 편향이나 분산을 줄이고 탐지 정확도를 높이는 방식.<br>
...
▣ 정의: 시간 흐름에 따라 수집된 데이터에서 <ins>추세(Trend), 계절성(Seasonality) 등의 패턴을 벗어난 시점</ins>을 찾아내는 방식.<br>
...
▣ 정의: 변수의 개수가 매우 많은 고차원 공간에서 <ins>'차원의 저주' 문제를 극복하며 부분 공간이나 각도로 이상치를 탐지</ins>하는 특화 방식.<br>
...
▣ 정의: <ins>학습 데이터의 확률 분포를 학습하여 그와 유사한 새로운 데이터를 생성해내는 신경망 모델</ins>.<br>
...
▣ 정의: 로우 데이터(Raw Data)로부터 다운스트림 태스크(분류, 회귀 등)에 <ins>유용한 핵심 특징(Feature)을 스스로 추출하도록 학습</ins>하는 방식.<br>
...
▣ 정의: 신경망을 통해 고차원 데이터를 저차원 잠재 공간으로 투영함과 동시에 <ins>최적의 군집을 할당</ins>하는 방식.<br>
...
▣ 정의: 고차원 입력 데이터를 <ins>정보 손실을 최소화하면서 저차원의 압축된 표현으로 변환</ins>하는 신경망 기술.<br>
...
▣ 정의: 데이터의 각 상태에 '에너지'라는 스칼라 값을 부여하여, 관측된 데이터에는 낮은 에너지를, <ins>비정상 데이터에는 높은 에너지를 할당하도록 학습</ins>하는 방식.<br>
...
<br>
## 5.6 Deep Learning-based Methods (딥러닝 기반 이상치 탐색)
▣ 정의: 신경망을 통해 데이터의 <ins>복잡한 비선형 특징을 학습하여 정상 패턴에서 벗어난 데이터를 탐지</ins>하는 방식.<br>
...
▣ 정의: 데이터 공간을 일정한 간격의 빈(Bin)으로 나누고 각 빈에 속하는 데이터의 개수를 측정하여 <ins>불연속적인 밀도를 추정</ins>하는 방식.<br>
...
▣ 정의: 각 데이터 포인트에 커널 함수를 배치하고 이들을 합산하여 <ins>매끄러운(Smooth) 확률 밀도 함수를 생성</ins>하는 방식.<br>
...
▣ 정의: 데이터가 특정한 수학적 확률 분포(정규분포 등)를 따른다고 가정하고, 해당 분포를 결정하는 <ins>매개변수(평균, 분산 등)를 추정</ins>하는 방식.<br>
...
▣ 정의: 데이터의 특정 분포 형태를 사전에 가정하지 않고, <ins>오직 주어진 데이터의 구조만을 이용하여 밀도를 직접 추정</ins>하는 방식.<br>
...
▣ 정의: <ins>신경망이나 복잡한 수학적 변환을 활용하여 고차원 공간의 복잡한 데이터 분포를 정교하게 모델링</ins>하는 방식.<br>
...
▣ 정의: 주어진 표본 데이터를 이용하여 <ins>변수 간의 선형적 상관관계를 나타내는 공분산 행렬을 직접 계산</ins>하는 방식.<br>
...
▣ 정의: 표본 공분산 행렬과 항등 행렬(Identity Matrix)과 같은 <ins>구조적 타겟 행렬을 가중 결합하여 추정치의 분산을 줄이는 방식</ins>.<br>
...
▣ 정의: 변수 간의 관계가 대부분 0(독립)이라고 가정하고, <ins>유의미한 관계만을 남겨 행렬 내의 많은 원소를 0으로 만드는 방식</ins>.<br>
...
▣ 정의: 데이터 내에 포함된 <ins>이상치(Outlier)나 오염된 샘플의 영향을 최소화하여 공분산을 추정</ins>하는 방식.<br>
...
▣ 정의: 데이터의 물리적 성질이나 도메인 지식을 바탕으로 <ins>공분산 행렬이 특정한 수학적 구조(Toeplitz, Block-diagonal 등)를 가진다고 가정하고 추정</ins>하는 방식.<br>
...
▣ 정의: 변수의 수($p$)가 <ins>표본의 수($n$)보다 훨씬 많은 'Large $p$, Small $n$' 상황에서 공분산 행렬을 정확하게 추정</ins>하기 위한 특화 기법.<br>
...
▣ 정의 : 연관규칙 학습을 위한 고전적인 알고리즘으로, <ins>빈발항목 집합(frequent itemsets)을 찾아내고 그 집합 간 연관성을 추출</ins><br>
선험적 원리(Apriori Principle) : "어떤 항목집합이 자주 발생하지 않는다면, 그 항목들을 포함하는 더 큰 항목집합도 절대 자주 발생할 수 없다"<br>
...
**향상도(lift):** 두 항목이 독립일 때 기대되는 확률 대비 함께 등장할 확률, {butter}→{bread}: 0.9375 < 1 → 독립적으로 발생할 때보다 같이 나타날 확률이 오히려 낮음<br>
	...
	support ≥ 0.4: 규칙 자체는 충분히 자주 등장 Support(A⇒B)=count(A∪B)​/N
	confidence ≥ 0.7: 규칙 신뢰도가 꽤 높음 Confidence(A⇒B)=Support(A∪B)​/Support(A)
	lift > 1: 긍정적 연관성으로 판단 가능, =1: 독립적, <1: 음의 상관관 Lift(A⇒B)=Confidence(A⇒B)/Support(B)


<br>

# [AR-2] FP-Growth(Frequent Pattern Growth) : 빈발 패턴 성장
▣ 정의: <ins>Apriori 알고리즘의 대안으로 FP-Tree(Frequent Pattern Tree)를 통해 빈발항목 집합을 생성하는 알고리즘으로, Apriori와 달리 매번 후보집합을 생성하지 않으며, 데이터의 트랜잭션을 직접 탐색하여 빈발항목 집합을 구한다.</ins><br>
...
▣ 정의: <ins>Apriori와 FP-Growth의 대안으로, 트랜잭션 간의 공통항목(교집합)을 기반으로 빈발항목을 추출</ins>하는 알고리즘<br>
...
▣ 정의: <ins>Apriori와 FP-Growth 확장버전으로 연관 규칙을 계층적으로 탐색하여 다중 수준에서 규칙을 생성</ins>하는 방식<br>
...
▣ 정의: <ins>여러 속성</ins>을 포함하여 다양한 차원의 규칙을 생성<br>
...
▣ 정의: <ins>거래 데이터를 순차적으로 결합하여 빈번한 항목 집합을 찾는 초기 연관규칙 알고리즘</ins> 중 하나<br>
...
① 차원의 저주 — 차원이 늘어날 수록 필요한 데이터가 기하급수적으로 많아지는 차원의 저주(curse of dimensionality) 문제를 해결. 특히 거리 기반 알고리즘(KNN, K-means 등)이 이 문제로 오작동.<br>
② 과적합 방지 — 특성이 100개인데 학습 데이터가 50개라면 모델은 패턴이 아닌 암기를 합니다. 불필요한 특성을 제거해 모델이 진짜 패턴만 학습하게 만든다.<br>
③ 시각화 — 사람은 2~3차원까지만 볼 수 있습니다. 1000차원짜리 이미지 데이터를 2D로 압축하면 눈으로 클러스터와 패턴을 직접 확인할 수 있다. (t-SNE, UMAP)<br>
④ 연산 비용 절감 — 특성이 줄면 학습 시간과 메모리가 극적으로 감소합니다. 실무에서 수백만 개의 특성을 그대로 쓰는 것은 사실상 불가능<br>
핵심 원칙 : 차원 축소는 데이터를 버리는 것이 아니라, 핵심 정보는 보존하면서 불필요한 잡음을 제거하는 것<br>
...
![](./images/PCA_1.png)
<br>
▣ 정의 : <ins>데이터의 분산을 최대한 보존하면서 데이터의 주요 성분(주성분)을 찾기 위해 선형 변환을 적용하는 차원 축소</ins> 알고리즘<br> 
...
▣ 정의: <ins>고차원 데이터의 국소 구조를 잘 보존하여 저차원으로 투영하는 비선형 차원 축소</ins> 알고리즘<br>
...
▣ 정의: <ins>데이터의 국소 구조와 전역 구조를 동시에 보존하면서 저차원으로 투영하는 비선형 차원 축소</ins> 알고리즘<br>
...
<img width ='1000' height = '600' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-06/images/SVD1.png'> 
<br>
영화 추천 시스템 : 행 = 영화, 열 = 사용자, 숫자 = 평점. SVD는 이 큰 표를 세 가지 의미 있는 조각으로 분해<br>
U — "각 영화가 어떤 장르 성격을 얼마나 갖고 있는가"를 숫자로 표현한 행렬(행 = 영화, 열 = 숨겨진 장르 축).<br>
Σ — 대각선에 특이값(σ₁ ≥ σ₂ ≥ …)이 나열된 행렬(숫자가 클수록 그 축이 데이터를 많이 설명)<br>
Vᵀ — "각 사용자가 어떤 장르를 얼마나 좋아하는가"를 표현한 행렬(행 = 숨겨진 장르 축, 열 = 사용자)<br>
<br>
▣ 정의: <ins>복잡한 데이터 속에 숨어 있는 핵심 패턴 몇 개로 전체를 설명. 임의의 행렬을 세 개의 행렬로 분해하는 방식</ins>ins>으로 행렬의 특이값과 특이벡터를 통해 행렬의 구조를 파악하고, 이를 통해 데이터의 패턴을 찾거나 압축하는 데 사용<br> 
...
▣ 정의 : <ins>다변량 신호에서 통계적으로 독립적인 성분을 추출하는 비선형 차원 축소</ins> 기법<br>
...
<img width ='1000' height = '600' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-06/images/NMF1.png'> 
<br>
▣ 정의 : 데이터를 <ins>비음수 행렬로 나타내고 이를 두 개의 비음수 행렬의 곱으로 분해하는 행렬 분해</ins>ins>(Matrix Factorization) 기법<br>
...
▣ 정의: MDS는 고차원 <ins>데이터 포인트 간의 거리를 보존하며 저차원으로 투영하는 차원 축소</ins> 기법<br>
...
# [DR-8] Isomap : 등거리 매핑
▣ 정의: <ins>데이터의 기하학적 구조를 보존하여 고차원 데이터를 저차원으로 투영하는 비선형 차원 축소 기법</ins><br>
...
▣ 정의 : 고차원의 데이터를 저차원(일반적으로 2차원) 공간으로 투영하여 데이터의 구조를 시각화하는 데 사용. PCA는 선형 변환을 통해 차원 축소를 수행하지만, SOM은 <ins>비선형 변환을 사용하여 더 복잡한 데이터 구조를 반영</ins>할 수 있으며, k-평균은 각 군집의 중심을 찾는 방식으로 군집화를 수행하는 반면, SOM은 <ins>뉴런이 격자 형태로 조직되어 있어 더 직관적인 시각화</ins>가 가능<br> 
...
## [연구사례] t-SNE가 동심원 데이터셋을 제대로 분리하지 못하는 이유와 해결 방안
