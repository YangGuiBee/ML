...
| 테스트 결과    | 동일                | 동일              |
| 평균 리워드    | 거의 동일             | 거의 동일           |
| 적용 분야    | 게임 AI (단순 보상 구조)<br>시뮬레이터 기반 최적 경로 탐색<br>광고 추천/콘텐츠 추천<br>금융 트레이딩 시 Off-policy로 학습할 때<br>통신망 라우팅 정책 연구
| 이동 로봇이 절벽/낭떠러지/충돌 위험이 있는 경우<br>강화학습 기반 자율주행(충돌 리스크가 큼)<br>HRL(Hierarchical RL)에서 하위 정책이 불확실한 환경<br>공정 제어(제어 행동의 실수가 큰 비용 초래)<br>로봇 팔 조작(작은 실수가 큰 문제 발생 가능) |
