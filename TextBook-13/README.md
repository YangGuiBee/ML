...
세 가지 루프가 수천, 수만 번 반복
① 에이전트는 현재 상태(state)를 보고 정책(policy : π)에 따라 행동(action)을 선택
② 환경은 그 행동(action)에 반응해서 새로운 상태(state)와 보상(reward)을 산출
③ 에이전트는 이 보상(reward)을 바탕으로 정책(policy)을 조금씩 개선합니다. 
시뮬레이터 구조 : n+1개 상태(S₀~Sn)가 있고, Sn이 목표 상태. 에이전트는 위/아래/왼쪽/오른쪽 중 하나를 선택하고, 목표에 가까워지면 +1, 잘못된 방향이면 -1의 보상. Q-테이블에는 각 상태에서 어떤 행동이 좋은지 학습된 가치가 누적 저장. 초록색으로 강조된 셀이 현재 에이전트가 가장 좋다고 생각하는 행동.

에이전트 → 환경: 현재 상태를 보고 정책(π)에 따라 **행동(Action)**을 선택합니다.
환경 → 에이전트: 행동의 결과로 새로운 **상태(State)**를 돌려줍니다.
보상(Reward): 환경이 에이전트에게 잘했는지/잘못했는지 숫자 신호(+1 / −1 / 0 등)로 알려줍니다.

주요 개념

탐색(exploration) vs 활용(exploitation): 초반엔 무작위로 탐색하고, 스텝이 쌓이면서 학습된 경험을 활용합니다 (ε-greedy).
학습률 슬라이더: 높을수록 새로운 경험을 빠르게 반영하지만 불안정해질 수 있습니다.
자동 실행을 켜면 Q-테이블이 점점 채워지면서 에이전트가 목표를 더 자주 찾아가는 것을 볼 수 있습니다.
