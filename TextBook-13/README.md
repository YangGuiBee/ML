...
| **메모리 사용량**     | 적음                          | 중간                           | 중간                               | **적음(히스토그램)**                    | 중간                          |
| **GPU 지원**      | 제한적                         | 제한적                          | 강함                               | **매우 강함**                        | 강함                          |
| **가장 잘 맞는 데이터** | 단순·선형적 데이터                  | 범용적 구조화 데이터                  | 복잡한 tabular                      | 초대규모 tabular                     | **카테고리 중심 tabular**         |
| **한 줄 요약**      | “단순하고 안정적”                  | “모든 Boosting의 기본”            | “산업·대회 최강 모델”                    | “고속·대규모 최적화”                     | “카테고리 데이터 최강”               |
...
| --------------------------- | ------------------- | ------------------------- | ------------------------------------------- | ----------------------------- |
| **1. 정의**                   | 여러 모델의 예측을 단순 투표/평균 | 같은 모델을 여러 개 학습해 평균        | 이전 모델의 오차를 다음 모델이 보완하는 순차적 학습               | 여러 모델의 예측을 새로운 메타모델이 다시 학습    |
| **2. 기본 개념**                | 병렬 결합               | 병렬 학습 + 평균화               | 순차 학습 + 가중치 업데이트                            | 이질 모델 결합 + 메타 학습              |
| **3. 목적**                   | 쉬운 방식으로 성능 안정화      | 분산(Variance) 감소           | 편향(Bias) 감소 및 성능 극대화                        | 서로 다른 모델의 강점 결합               |
| **4. 약한 학습기(Weak Learner)** | 제한 없음               | 보통 Decision Tree          | 매우 단순한 트리(Depth 1~3)                        | 다양(선형+비선형+트리 등)               |
| **5. 결합 방식**                | Hard/Soft Voting    | Bootstrap + 평균            | Additive Model(순차 가중 합)                     | Base predictions → Meta model |
| **6. 데이터 처리 구조**            | 병렬                  | 병렬                        | 순차                                          | 병렬 → 메타                       |
| **7. 성능 특성**                | 중간 수준 안정적인 성능       | 과적합 감소 → 안정적 성능           | 성능 최상(GBDT/XGB/LGBM)                        | 최고 AUC/복합 모델 성능               |
| **8. 필요성**                  | 간단한 성능 향상           | 고분산 모델 안정화                | 고성능 요구 문제                                   | 모델 다양성 활용                     |
| **9. 장점**                   | 구현 쉬움, 빠름           | 과적합 방지                    | 매우 높은 정확도                                   | 다른 앙상블보다 더 유연                 |
| **10. 단점**                  | 약한 모델이면 개선 적음       | 편향 해결은 약함                 | 느리고 튜닝 어려움                                  | 느리고 복잡도 가장 큼                  |
| **11. 과적합 위험**              | 낮음                  | 낮음                        | 중간~높음(튜닝 필수)                                | 중간(메타모델 제한적)                  |
| **12. 연산 복잡도**              | 낮음                  | 중간                        | 높음                                          | 가장 높음                         |
| **13. 대표 모델**               | Hard/Soft Voting    | Random Forest             | AdaBoost, GBDT, XGBoost, LightGBM, CatBoost | StackingClassifier            |
| **14. 확률 기반 예측**            | Soft Voting 가능      | RF는 가능                    | 대부분 가능                                      | 메타모델이 최종 확률 산출                |
| **15. 수식/조합 형태**            | 평균 μ = (1/M) Σ hᵢ   | μ = (1/M) Σ hᵢ(bootstrap) | F_t = F_{t-1} + η·h_t                       | y = f_meta(h₁(x),h₂(x),…)     |
| **16. 적용 분야**               | 기본 분류·회귀            | 이미지/의료/신용                 | 모든 구조화 데이터 산업 표준                            | 고정밀 문제(의료·금융 등)               |
| **17. 데이터 요구 조건**           | 크기 제약 거의 없음         | 중~대규모 적합                  | 대규모·고차원 유리                                  | Base 다양성 필요                   |
| **18. 클래스 불균형 대응**          | 약함                  | 약함                        | 매우 강함                                       | 메타모델이 잘 처리하면 강함               |
| **19. 특징 중요도 해석**           | 어려움                 | RF는 쉬움                    | XGB/LGBM은 매우 쉬움                             | 메타모델 따라 상이                    |
| **20. 실전 성능 순위**            | ★★★                 | ★★★★                      | ★★★★★                                       | ★★★★★ (AUC 최강)                |
