...
| 구분 | 핵심 아이디어 | 대표 알고리즘 | 장점 | 한계 | 주요 적용 분야 | 2026년 성숙도 |
|---|---|---|---|---|---|---|
| **[1] Model-free RL<br>Value-based** | 상태–행동 가치함수(Q, V)를 직접 학습하여 최적 행동 선택 | Q-learning, SARSA, DQN, Double DQN | 구조 단순, 구현 용이, 이산 행동 문제에 강함 | 연속 제어 취약, 샘플 효율 낮음, 불안정 | 게임, 추천, 단순 제어 문제 | 3단계: 표준화 |
| **[2] Model-free RL<br>Policy-based** | 정책 π(a\|s)를 직접 최적화 (확률적 행동) | REINFORCE, PPO, A3C, SAC | 연속 행동 처리 가능, 안정적 수렴 | 샘플 효율 낮음, 보상 설계 민감 | 로보틱스, 자율주행, RLHF | 4단계: 상용화 |
| **[3] Model-based RL<br>Given the Model** | 환경 모델이 주어졌을 때 계획(Planning)으로 최적 정책 계산 | Value Iteration, Policy Iteration, MPC | 이론적 최적성, 높은 샘플 효율 | 정확한 모델 필요, 현실 적용 제한 | 공정 제어, 에너지 시스템 | 3단계: 표준화 |
| **[4] Model-based RL<br>Learn the Model** | 환경 모델을 학습하고 시뮬레이션 기반 계획 수행 | Dyna, World Models, MuZero, Dreamer | 샘플 효율 우수, 장기 계획 가능 | 모델 편향, 구현 복잡 | 로보틱스, 게임, 과학 자동화 | 2단계: 시범 배포 |

**2000~2015**: Value-based / Given-model 중심
**2016~2022**: Policy-based RL 폭발적 성장
**2023~2026**: RLHF → *Policy-based RL의 대표적 상용 사례, World Model → *Model-based RL의 핵심 연구 축
**2026년 : “Policy-based RL의 상용화” + “Model-based RL의 재부상”
