...
레이블 인코딩(Label Encoding)과 원핫 인코딩(One-hot Encoding)은 범주형 데이터를 수치형 데이터로 변환하는 방법이다.
레이블 인코딩은 각 카테고리를 숫자로 대응시켜서 변환한다. 예를 들어, "red", "green", "blue"라는 3개의 카테고리가 있다면 "red"를 1로, "green"을 2로, "blue"를 3으로 변환하는 것이다. 이 방법은 간단하고 직관적이지만, 각 카테고리가 가지는 값의 크기 차이가 있을 경우 예측 결과에 영향을 미칠 수 있다.
