...
<br>
**▣ 데이터 이해 단계의 시각화 방안**
(1) 데이터 분포 및 특성 파악 : 연구 보고서 초반부에서는 데이터셋의 구조와 특성을 설명하기 위한 시각화가 필요(모델 성능이 아니라 데이터가 어떤 문제를 내포하는지 설명)

히스토그램 / 밀도 플롯 → 입력 변수의 분포, 치우침(skewness), 이상치 존재 여부 설명
박스 플롯 → 클래스별 데이터 분포 차이, 분산, 극단값 비교
막대 그래프(Bar Plot) → 클래스 불균형 여부 시각화
