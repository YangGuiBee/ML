...
지도 학습은 주어진 입력값에 대하여 신뢰성 있는 출력 값을 출력하는 함수를<b> 
현재 가지고 있는 데이터(학습 데이터라고 한다)로부터 학습하는 과정이다.<b>
수식을 이용하여 표현하면 현재 가지고 있는 학습데이터 $(X, y)$로부터 $y = f(X)$를 만족하는<b> 
여러 함수 $f$중에서 가장 최적(주어진 Task에 따라 달라짐)의 $f$를 찾는 과정이라고 할 수 있다.<b>
