...
1) 정상성(stationarity): 현상이 발생하는 횟수의 분포는 시작 시각과 관계없다. $N_t$의 분포와 $N_{s+t}−N_S$의 분포가 같고 $N_0=0$이다.
2) 독립 증분성(independent increment): 시각 0부터 $t$까지 현상이 발생하는 횟수와 시각 $t$후부터 $t+h(h>0)$까지의 발생 횟수는 서로 독립(즉, $N_t$와 $N_{t+h}−N_t$는 서로 독립)
3) 비례성(propertionality): 짧은 시간 동안에 현상이 한 번 발생할 확률은 시간에 비례한다. $P(N_h=1)=λh+o(h),h→0$  (※ λ는 양의 비례상수, o(h)는 $lim_{h→0}o(h)/h=0$)
