...
과적합(Overfitting)
	...
	Train Score는 1.0에 붙어 있는 반면, Validation Score는 그보다 훨씬 낮은 0.8 수준에서 두 점수 사이의 간격이 넓은 것은 전형적인 과적합(Overfitting)의 신호(랜덤 포레스트 모델이 훈련 데이터의 세세한 노이즈까지 너무 복잡하게 학습하여, 실제 새로운 데이터(검증 데이터)에 대해서는 그만큼의 실력을 발휘하지 못하는 경우)


모델 성능 개선 방안

	데이터 추가 확보: 주황색 선(Validation Score)이 마지막까지 우상향하는 추세이므로, 훈련 샘플을 더 많이 추가한다면 두 선이 만나면서 성능이 개선될 가능성
	...
	특성 선택 (Feature Selection): 불필요하거나 노이즈가 섞인 변수가 학습을 방해하고 있을 수 있습니다. 이전 단계에서 확인한 상관계수 히트맵을 참고하여 영향력이 적은 변수를 제거
