...
(1) 극심한 우편향(Right-Skewed) 분포 : 그래프의 가장 높은 봉우리가 왼쪽(0~50달러 사이)에 치우쳐 있고, 오른쪽으로 갈수록 꼬리가 아주 길게 늘어지는 전형적인 우편향 분포(이는 타이타닉 승객의 압도적 다수가 저가 요금에 몰려 있음을 직관적으로 증명)<br>
<br>
(2) 다봉성(Multimodal)의 조짐 : 약 100달러 부근과 200~300달러 부근에서 곡선이 완전히 바닥에 닿지 않고 살짝 솟아오르는 구간은 1등급 승객들 사이에서도 특정 가격대별로 그룹이 형성되어 있을 가능성을 시사<br>
<br>
(3) 이상치(Outlier)의 시각화 : 500달러를 넘어서는 지점까지 곡선이 이어지는 것은 확률 밀도값(Density)은 매우 낮지만, 데이터가 존재한다는 사실을 꼬리의 길이를 통해 알 수 있음<br>
