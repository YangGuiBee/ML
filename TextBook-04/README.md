...
<img width ='1000' height = '800' src = 'https://github.com/YangGuiBee/ML/blob/main/TextBook-04/images/ML04_5_1.PNG'> 
...
박스 아래쪽: 제1사분위(Q1, 25%), 박스 위쪽: 제3사분위(Q3, 75%), 박스 안 가로선: 중앙값(Median, 50%)<br>
(2) 수염(Whisker): 사분위 범위에서 ± 1.5 IQR 안에 들어오는 값으로 이 범위 내의 최소값~최대값 표시<br>
...
# 13. 바이올린 플롯 (Violin Plot)
바이올린 플롯은 박스 플롯(Box Plot)의 통계적 정보와 커널 밀도 추정(KDE)의 분포 정보를 결합한 형태로 머신러닝에서 특히 유용한 경우는 (1) 클래스별 특징값 분포의 미묘한 차이를 보여줄 때, (2) 모델별 교차검증 성능 분포의 형태(비대칭, 다봉)를 드러낼 때임. 표본 수가 적으면 밀도 추정이 불안정해 모양이 왜곡될 수 있어, 표본 규모와 함께 제시하는 것이 바람직.<br>
...
(1) 객실 등급별 요금의 분포와 불평등<br>
1등급 : 바이올린의 몸통이 길게 위로 뻗어 있는데, 이는 요금의 분산(Variation)이 매우 크다는 것을 의미. 특히 500달러가 넘는 극단적인 고액 요금 지불자가 존재하며, 전반적으로 다른 등급보다 훨씬 높은 가격대에 데이터가 분포<br>
2등급 & 3등급 : 바이올린이 아래쪽에 납작하게 눌린 모양. 이는 요금이 특정 저가 구간(약 0~50달러 사이)에 밀집되어 있음을 보여준다. 2등급보다 3등급의 바이올린 바닥이 더 넓은데, 이는 3등급에 가장 많은 승객이 몰려 있다는 것을 시사<br>
<br>
(2) 중앙값과 사분위수 (내부의 흰 점과 검은 막대)<br>
바이올린 중앙의 흰색 점은 중앙값(Median)을 나타냄. 1등급의 중앙값이 2, 3등급보다 확연히 높게 형성되어 있어 등급 간 경제적 격차를 명확히 보여준다. 가운데 굵은 검은색 막대는 사분위수 범위(IQR, 데이터의 중간 50%)를 나타냄. 1등급의 막대가 가장 길어, 같은 1등급 내에서도 지불한 요금 차이가 상당했음을 시사<br>
<br>
(3) 이상치와 밀도 (바이올린의 너비)<br>
바이올린의 너비(폭)는 해당 가격대에 얼마나 많은 데이터가 있는지를 나타냄. 3등급은 10달러 내외의 구간에서 폭이 매우 넓은데, 이는 대부분의 승객이 비슷한 저가 요금을 냈음을 의미<br>
...
(1) 극심한 우편향(Right-Skewed) 분포<br>
그래프의 가장 높은 봉우리가 왼쪽(0-50달러 사이)에 치우쳐 있고, 오른쪽으로 갈수록 꼬리가 아주 길게 늘어지는 전형적인 우편향 분포(이는 타이타닉 승객의 압도적 다수가 저가 요금에 몰려 있음을 직관적으로 증명)<br>
<br>
(2) 다봉성(Multimodal)의 조짐<br>
약 100달러 부근과 200-300달러 부근에서 곡선이 완전히 바닥에 닿지 않고 살짝 솟아오르는 구간은 1등급 승객들 사이에서도 특정 가격대별로 그룹이 형성되어 있을 가능성을 시사<br>
<br>
(3) 이상치(Outlier)의 시각화<br>
500달러를 넘어서는 지점까지 곡선이 이어지는 것은 확률 밀도값(Density)은 매우 낮지만, 데이터가 존재한다는 사실을 꼬리의 길이를 통해 알 수 있음<br>
...
---
	...
	17. 혼동 행렬 히트맵 (Confusion Matrix Heatmap)
	18. ROC 곡선 / PR 곡선 (ROC Curve / Precision–Recall Curve)
	19. 보정 곡선 / 신뢰도 도표 (Calibration Curve / Reliability Diagram)
...
# 17. 혼동 행렬 히트맵 (Confusion Matrix Heatmap)
모델이 '생존(1)'이라고 예측했는데 실제로 '사망(0)'했는지, 혹은 그 반대인지를 표 형태로 나타낸 것으로 숫자로만 보면 헷갈리니 히트맵(Heatmap)을 입혀 어디서 실수가 잦은지 한눈에 파악<br>
▣ 용도: 분류 모델의 성능(TP, FP, FN, TN)을 구체적으로 파악할 때 사용<br>
▣ 특징: 정답과 오답의 종류를 구분해 주므로 Accuracy가 높은데도 특정 클래스만 못 맞히는 문제를 파악<br>
▣ 예시: 암 진단(오진율 확인), 스팸 메일 분류 등<br>

![](./images/image04_17.png)

(1) 정답을 맞힌 경우 (Main Diagonal)<br>
TN(True Negative): 68 → 실제로 사망(Dead)한 승객을 모델이 사망했다고 올바르게 예측<br>
TP(True Positive): 46 → 실제로 생존(Survived)한 승객을 모델이 생존했다고 올바르게 예측<br>
<br>
(2) 모델이 틀린 경우 (Errors)<br>
FP(False Positive): 12 (제1종 오류) → 실제로는 사망(Dead)했는데 모델이 생존(Survived)했다고 잘못 예측<br>
FN(False Negative): 17 (제2종 오류) → 실제로는 생존(Survived)했는데 모델이 사망(Dead)했다고 잘못 예측<br>
<br>
...
# 18. ROC 곡선 / PR 곡선 (ROC Curve / Precision–Recall Curve)
ROC: 모델이 '진짜 양성'을 얼마나 잘 찾으면서도 '가짜 양성'을 얼마나 억제하는지 보여줌(곡선 아래 면적(AUC)이 1에 가까울수록 정답), PR: 정밀도(Precision)와 재현율(Recall)의 관계를 보여줌(데이터 불균형이 심할 때 ROC보다 더 냉정한 평가)<br>
▣ 용도: 임계값(Threshold) 변화에 따른 모델의 전반적인 변별력을 평가할 때 사용<br>
▣ 특징: ROC는 클래스 분포에 강건하고, PR은 희소한 클래스 판별 성능을 보기에 유리<br>
▣ 예시: 이상거래 탐지(FDS), 질병 유무 판정<br>

![](./images/image04_18.png)

(1) ROC Curve & AUC<br>
ROC 커브는 <ins>'Y축:진짜 양성 비율(Recall)'</ins>과 <ins>'X축:가짜 양성 비율(1-Specificity)'</ins> 사이의 관계<br>
AUC(Area Under Curve) = 0.83: 그래프 아래 면적의 넓이 → 1에 가까울수록 완벽한 모델이고 0.5면 무작위 추측으로 0.83은 매우 준수한 성능(모델이 사망자와 생존자를 구분하는 능력이 상당히 뛰어나다는 것을 의미)<br>
커브의 형태: 주황색 실선이 왼쪽 상단 모서리에 가까울수록 좋은 모델(현재 그래프는 초반에 가파르게 상승하다가 완만해지는데, 이는 적은 오탐(False Positive)으로도 많은 정탐(True Positive)을 잡아낼 수 있는 구간이 존재함을 보여줌)<br>
<br>
(2) Precision-Recall Curve<br>
<ins>Y축 : 정밀도(Precision)</ins>와 <ins>X축 : 재현율(Recall)</ins> 사이의 트레이드오프(Trade-off) 관계<br>
데이터의 클래스 불균형이 있을 때 ROC 커브보다 더 민감하게 성능을 반영<br>
상충 관계: 재현율(X축)을 높이려 하면(더 많은 생존자를 찾으려 하면), 정밀도(Y축)가 떨어지는 경향이 뚜렷하게 보임<br>
지그재그 패턴: 그래프가 매끄럽지 않고 계단식으로 변하는 이유는 임계값을 미세하게 조정할 때마다 개별 데이터 포인트가 '생존'에서 '사망'으로 분류가 바뀌며 정밀도가 출렁이기 때문<br>
성능 평가: 오른쪽 상단(1.0, 1.0)에 가까울수록 좋음(현재 그래프는 재현율이 0.6 수준까지는 정밀도가 0.8 이상을 유지하고 있어, 중상위권의 안정적인 예측력을 보여줌)<br>
...
# 19. 보정 곡선 / 신뢰도 도표 (Calibration Curve / Reliability Diagram)
모델이 예측한 "생존 확률"이 "실제 생존율"과 얼마나 일치하는지 직선(y=x)에 비춰보는 검량선 그래프<br>
▣ 용도: 모델이 자신의 예측을 과신하거나 과소평가하는지 확인<br>
▣ 특징: 완벽한 모델은 45도 대각선 위에 점들이 찍힌다.<br>
▣ 예시: 기상 예보(강수 확률 80%일 때 실제로 80번 비가 오는지 확인).<br>

![](./images/image04_19.png)

(1) 기준선: Perfectly Calibrated (주황색 점선)<br>
모델의 예측 확률과 실제 발생 확률이 1:1로 완벽하게 일치하는 이상적인 상태로 그래프가 이 점선에 가까울수록 모델의 결과를 그대로 믿어도 된다는 뜻<br>
<br>
(2) 모델 선: Random Forest (파란색 실선)<br>
예측 확률 0.0 ~ 0.2 구간 (과대평가 경향): 모델 선이 점선보다 위에 있는 경우는 모델은 생존 확률을 낮게(예: 10%) 예측했지만, 실제로는 그보다 더 많이(예: 15~20%) 생존했다는 뜻<br>
예측 확률 0.2 ~ 0.5 구간 (과소평가 경향): 모델 선이 점선보다 아래에 있는 경우는 모델이 "생존할 것 같다"고 예측한 비중보다 실제 생존 비중이 낮은 경우<br>
예측 확률 0.6 이상 구간: 다시 점선에 가까워지거나 위아래로 진동하는 0.8 부근에서 실제 확률이 0.9 근처로 치솟았다가 다시 떨어지는 구간이 보이는데, 이는 해당 확률 구간의 샘플 수가 적어서 발생하는 변동성일 가능성<br>
...
# 20. 잔차 플롯 / Q-Q 플롯 (Residual Plot / Q-Q Plot)
잔차 플롯은 실제값과 예측값의 차이(오차)가 특정 패턴 없이 고르게 퍼져 있는지 보고, Q-Q 플롯은 오차가 정규분포를 따르는지 확인<br>
▣ 용도: 회귀 모델의 가정이 적절한지 검토(타이타닉에서는 '요금' 예측 등 회귀 문제에 주로 쓰임)<br>
▣ 특징: 잔차가 무작위로 흩어져 있어야 좋은 모델<br>
▣ 예시: 주택 가격 예측 모델의 오차 분석<br>

![](./images/image04_20.png)

(1) 잔차 플롯(Residual Plot) : 예측값과 실제값의 차이(잔차)를 시각화<br>
점들이 $y=0$ 라인을 중심으로 위아래 두 줄로 명확히 나뉩니다. 이는 종속 변수가 0(사망)과 1(생존)뿐인 이진 분류 데이터의 특성 때문. 중앙의 녹색 선이 우상향하고 있는데, 이는 특정 예측 범위에서 오차가 편향되어 있음을 시사. 분류 문제에서는 잔차 플롯보다는 혼동 행렬이나 Calibration Curve가 모델의 '오답 경향'을 파악하는 데 더 효과적<br>
→ 잔차 플롯은 분류보다는 회귀 분석에서 모델의 '건강 상태'를 체크할 때 사용(모델이 데이터의 패턴을 충분히 학습했는지, 혹은 중요한 정보를 놓치고 있는지 판단)<br>
<br>
(2) Q-Q 플롯(Q-Q Plot) : 수집된 데이터의 분포가 정규 분포(붉은 선)를 얼마나 잘 따르는지 확인<br>
파란색 점들이 붉은 대각선 위에 놓이지 않고, 3개의 수평한 층으로 나뉘는 데, 이는 데이터가 연속적인 수치가 아니라 -1, 0, 1 등 이산적인 값을 갖기 때문에 나타나는 전형적인 패턴임. 이 데이터는 정규 분포를 따르지 않지만 예제로 사용한 Random Forest 모델은 데이터의 정규성을 가정하지 않는 비모수 모델이므로, 성능에는 문제가 없음(만약 선형 회귀나 LDA 모델을 썼다면 이 그래프를 보고 데이터 변환을 고민할 필요)<br>
→ Q-Q 플롯은 데이터의 분포가 이론적인 분포(보통 정규분포)와 얼마나 일치하는지 확인할 때 사용<br>


▣ 소스코드 (Seaborn + Matplotlib):<br>

	import scipy.stats as stats
	# 예시를 위해 '요금(Fare)' 예측 잔차라고 가정
...
# 21. 학습 곡선 / 검증 곡선 (Learning Curve / Validation Curve)
훈련 데이터 양을 늘려가며 훈련 오차와 검증 오차가 어떻게 변하는지 확인<br>
▣ 용도: 모델이 과적합(Overfitting)인지 과소적합(Underfitting)인지 진단<br>
▣ 특징: 데이터가 더 필요한지, 혹은 모델이 너무 복잡한지 판단하는 기준<br>
▣ 예시: 딥러닝 학습 시 Epoch에 따른 성능 변화 모니터링.<br>

![](./images/image04_21.png)

(1) 훈련 점수 (Train Score, 파란색 선) : 훈련 샘플 수가 100개일 때부터 500개가 넘을 때까지 1.0(100%)에 가까운 매우 높은 점수를 꾸준히 유지 → 모델이 훈련 데이터에 대해서는 거의 완벽하게 암기하거나 학습하고 있다는 뜻<br>
<br>
(2) 검증 점수 (Validation Score, 주황색 선) : 샘플 데이터가 늘어남에 따라 약 0.74에서 시작하여 최종적으로 약 0.80 부근까지 점진적으로 상승하는 모습 → 데이터가 많아질수록 모델의 일반화 성능(새로운 데이터를 맞히는 능력)이 개선되고 있음<br>

과적합(Overfitting)

	Train Score는 1.0에 붙어 있는 반면, Validation Score는 그보다 훨씬 낮은 0.8 수준에서 두 점수 사이의 간격이 넓은 과적합
	랜덤 포레스트 모델이 훈련 데이터의 세세한 노이즈까지 너무 복잡하게 학습하여, 
	실제 새로운 데이터(검증 데이터)에 대해서는 그만큼의 실력을 발휘하지 못하는 경우


모델 성능 개선 방안

	데이터 추가 확보: 주황색 선(Validation Score)이 마지막까지 우상향하는 추세이므로, 훈련 샘플을 더 많이 추가
	모델 복잡도 제한(규제): 현재 모델이 너무 '똑똑한 척'을 하고 있으므로, 랜덤 포레스트의 가지치기 필요
	max_depth(최대 깊이)를 줄이거나, min_samples_leaf(리프 노드의 최소 샘플 수)를 늘려 모델을 조금 더 단순하게 만들 필요
	특성 선택(Feature Selection): 불필요하거나 노이즈가 섞인 변수가 학습을 방해하는 경우, 상관계수 히트맵을 참고하여 영향력이 적은 변수 제거
	

▣ 소스코드 (Matplotlib):<br>

	import pandas as pd
	import numpy as np  # <--- 이 부분이 누락되어 오류가 발생했습니다.
	import matplotlib.pyplot as plt
	from sklearn.model_selection import learning_curve
	from sklearn.ensemble import RandomForestClassifier

	# 학습 곡선 데이터 계산
	# X, y는 이전에 전처리한 데이터를 그대로 사용한다고 가정합니다.
...
# 22. 특성 중요도 플롯 (Feature Importance)
모델이 생존 여부를 판단할 때 어떤 정보(성별, 나이, 요금 등)를 가장 중요하게 참고했는지 순위를 매겨 보여줌<br>
▣ 용도: 모델의 의사결정 근거를 파악하고, 불필요한 변수를 제거(Feature Selection)할 때 사용<br>
▣ 특징: 트리 기반 모델(Random Forest, XGBoost 등)에서 기본 제공하며 계산이 빠름<br>
▣ 예시: 마케팅 타겟 선정(어떤 고객 특성이 구매에 영향을 주는가?), 제조 공정 불량 원인 파악<br>

![](./images/image04_22.png)

(1) 나이 (Age) - 중요도 약 30% : "어린이 우선 구조" 원칙이 모델의 판단에 매우 강력한 기준이 되었음을 의미. 단순히 나이가 많고 적음이 아니라, 특정 연령대(영유아 등)가 생존에 결정적인 역할을 했다는 분석<br>
<br>
(2) 요금 (Fare) - 중요도 약 29% : 높은 요금을 지불한 승객(주로 1등석)의 생존율이 압도적으로 높았던 데이터의 특징이 모델에 그대로 반영<br>
<br>
(3) 성별 (Sex_male) - 중요도 약 28% : 모델은 승객이 남성인지 여성인지를 확인하는 것만으로도 생존 여부를 판별하는 데 큰 확신을 얻고 있음<br>
<br>
(4) 객실 등급 (Pclass) - 중요도 약 12% : 상대적으로 가장 낮은 비중을 차지하는 것처럼 보이지만, 사실 요금(Fare)과 밀접한 연관이 있는 변수<br>
...
# 23. SHAP 요약 플롯 / 의존 플롯 (SHAP Summary / Dependence Plot)
각 피처가 모델의 결과값(생존/사망)을 어느 방향으로, 얼마나 세게 밀었는지 모든 데이터를 점으로 찍어 보여준다<br>
▣ 용도: 모델의 전역적인 설명력을 확보(XAI의 핵심)<br>
▣ 특징: 성별이 '여성'인 것이 생존 확률을 얼마나 높였는지(빨간 점의 위치)를 직관적으로 보여줌<br>
▣ 예시: 대출 심사 거절 사유 분석<br>

![](./images/image04_23.png)

(1) Pclass × Age (좌측 상단) : Pclass 축에서 데이터 포인트들이 양옆으로 넓게 퍼짐 → 같은 객실 등급이라 하더라도 나이(Age)가 몇 살이냐에 따라 생존 확률 기여도(SHAP value)가 크게 달라짐을 의미. 특히 붉은색(높은 값)과 푸른색(낮은 값) 점들이 혼재되어 있어, 등급과 나이의 조합이 모델 판단에 복합적으로 작용<br>
<br>
(2) Age × Pclass (우측 하단) : Age 축을 기준으로 점들이 분포하며, 색상(객실 등급)에 따라 위치가 갈림 → 나이가 모델에 주는 영향력이 객실 등급에 따라 증폭되거나 감쇄. 예를 들어, 똑같은 어린이 승객이라도 1등석(파란색 계열)일 때와 3등석(빨간색 계열)일 때의 생존 기여도가 다르게 나타남<br>


▣ 소스코드 (SHAP 라이브러리 기반):<br>
	...
	# SHAP 값 계산 (X_test 데이터를 바탕으로 모델의 판단 근거 수치화)
	shap_values = explainer.shap_values(X_test)

	# 1번 클래스(생존)에 대한 SHAP 요약 플롯 시각화
	# 만약 결과가 리스트 형태가 아니라면 shap_values를 그대로 넣어야 할 수도 있습니다.
	# (최신 SHAP 버전 대응을 위해 보통 아래와 같이 사용합니다)
	if isinstance(shap_values, list):
    # 이진 분류에서 리스트로 반환될 경우 [0]은 사망, [1]은 생존에 대한 기여도입니다.
    	shap.summary_plot(shap_values[1], X_test)
	else:
    # 단일 배열로 반환될 경우 (최신 버전 등)
    	shap.summary_plot(shap_values, X_test)
	
<br>

# 24. 부분의존도(PDP) / ICE 플롯 (Partial Dependence / Individual Conditional Expectation)
다른 변수들은 고정하고, 특정 변수(예: 나이)만 바꿨을 때 생존 확률이 어떻게 변하는지 곡선으로 나타냄<br>
▣ 용도: 특정 변수와 타겟 사이의 비선형 관계를 파악<br>
▣ 특징: "나이가 많아질수록 생존 확률이 일관되게 떨어지는가?"를 확인<br>
▣ 예시: 약물 투여량에 따른 환자의 반응률 변화 분석<br>

![](./images/image04_24.png)

(1) 나이 (Age) 분석<br>
영유아 및 어린이 구간 (0-10세)의 생존 확률 기여도가 0.7-0.8 이상으로 압도적으로 높다. 모델이 "어린이 우선 구조" 원칙을 매우 강력하게 학습했음을 시사<br>
청년 및 중년 구간 (20-50세)인 20대 초반에 급격히 하락하여 0.3-0.4 수준에서 머무르고 중간중간 미세하게 반등하는 구간이 있지만, 전반적으로 낮은 생존 확률 기여도를 유지<br>
노년 구간 (60세 이상)인 60세 이후부터 기여도가 더 떨어져 0.3 미만으로 수렴. 나이가 많을수록 생존 확률에 부정적인 영향을 주었음을 시사<br>
<br>
(2) 요금 (Fare) 분석<br>
저가 요금 구간 (0-40) : 생존 확률 기여도가 0.3-0.4 사이에서 낮게 형성. 주로 3등석 승객들이 위치한 구간으로, 낮은 요금이 생존에 불리하게 작용<br>
중고가 요금 구간 (50 이상) : 요금이 50을 넘어서는 시점에 기여도가 0.45 이상으로 급격히 상승<br>
상승 후 유지: 요금이 일정 수준(약 60-80)을 넘어가면 생존 확률 기여도가 더 이상 크게 오르지 않고 고점에 머무르거나 소폭 하락 후 유지. 이는 특정 금액 이상의 요금을 냈다면(1등석 등), 그 안에서는 금액의 차이가 생존에 주는 추가 영향이 크지 않음을 시사<br>


▣ 소스코드 (Matplotlib):<br>

	from sklearn.inspection import PartialDependenceDisplay
	PartialDependenceDisplay.from_estimator(model, X_train, ['Age', 'Fare'])
	plt.show()

<br>

# 25. 차원축소 임베딩 산점도 (PCA / t-SNE / UMAP Scatter)
성별, 나이, 등급 등 여러 개의 변수를 딱 2개(PC1, PC2)로 압축해서 2차원 평면에 점을 찍어보는 방식<br>
▣ 용도: 고차원 데이터를 시각화하여 생존자와 사망자가 데이터 공간상에서 어떻게 무리 지어 있는지(Cluster) 확인<br>
▣ 특징: 복잡한 데이터를 단순화해서 보여주지만, 압축된 축(PC1 등)이 무엇을 의미하는지 해석하기는 어려움<br>
▣ 예시: 유전자 데이터 분석, 고객 군집화 결과 시각화<br>

![](./images/image04_25.png)

(1) 축의 의미 (PC1 & PC2)<br>
PC1 (가로축): 데이터의 분산을 가장 많이 설명하는 첫 번째 주성분. 타이타닉 데이터의 특성상 주로 '요금(Fare)'과 '객실 등급(Pclass)' 정보가 이 축에 강하게 투영되었을 가능성이 있으며, 오른쪽(PC1 값이 큰 쪽)으로 갈수록 데이터가 드문드문해지는데, 이는 요금이 매우 높은 극소수의 승객들을 나타냄<br>
PC2 (세로축): 두 번째로 중요한 성분으로, PC1과 직교하며 주로 나이(Age)와 같은 다른 변수의 특성을 반영<br>
<br>
(2) 생존 및 사망 데이터의 분포<br>
좌측 밀집 구역 (PC1 < 50): 푸른색 점(사망자)이 굉장히 빽빽하게 모여 있으며, 이는 요금이 저렴하고 일반적인 조건을 가진 승객들 사이에서 사망자가 많았음을 보여줌<br>
우측 및 분산 구역: PC1 값이 커질수록(오른쪽으로 갈수록) 주황색 점(생존자)의 비율이 상대적으로 늘어나는 경향 보임<br>
중첩 현상: 주황색 점과 푸른색 점이 완전히 분리되지 않고 많이 겹쳐 있으며, 이는 단순히 요금이나 나이 같은 몇 개의 변수 조합만으로는 생존 여부를 100% 가려내기 어렵다는 뜻, 앞서 수행한 SHAP 상호작용 분석처럼 변수 간의 복합적인 관계가 중요함을 시사<br>
...
▣ 소스코드 (Seaborn):<br>
	...
	sns.scatterplot(data=pca_df, x='PC1', y='PC2', hue='Survived', alpha=0.6)
...
# 26. 결정경계 시각화 (Decision Boundary Plot)
모델이 데이터 공간을 어떻게 나누고 있는지 색상 영역으로 보여준다.<br>
▣ 용도: 분류기의 복잡도와 경계의 매끄러움을 시각적으로 확인<br>
▣ 특징: 2D(변수 2개)에서만 명확히 보이기 때문에 보통 PCA로 차원 축소 후 표시<br>
▣ 예시: SVM의 C값 변화에 따른 경계선 변화 관찰<br>

![](./images/image04_26.png)

(1) 복잡하고 각진 경계 (랜덤 포레스트의 특징) : 경계선이 매끄러운 곡선이 아니라, 마치 테트리스 블록처럼 수직과 수평으로 꺾임 → 의사결정 나무(Decision Tree) 기반 모델의 전형적인 특징으로 모델이 "나이가 10세 이하인가?", "요금이 50달러 이상인가?"와 같은 **연속적인 질문(If-Then)**을 던지며 영역을 쪼개기 때문에 이런 계단식 경계가 형성<br>
<br>
(2) 주요 생존 영역 (주황색 바탕) : <br>
고액 요금 구간: 그래프 위쪽(요금이 높은 곳)은 대부분 주황색 영역. 요금이 약 100달러를 넘어가면 나이와 상관없이 생존 확률이 높다고 판단<br>
어린이 구간: 왼쪽 하단(나이가 어린 곳)을 보면 요금이 낮더라도 생존으로 예측하는 작은 주황색 블록들이 산재. 이는 모델이 "어린이 우선 구조" 규칙을 개별적인 규칙으로 보유하고 있음을 의미<br>
<br>
(3) 사망 위험 영역 (보라색/회색 바탕) : 저가 요금 + 성인 구간: 오른쪽 하단(나이가 많고 요금이 낮은 곳)은 광범위한 사망 영역. 특히 나이가 60세를 넘어가면 요금이 100달러 미만인 경우 거의 예외 없이 사망으로 분류되는 경향<br>


▣ 소스코드 (Matplotlib):<br>

	# 'Age'와 'Fare' 두 가지만 사용하여 간략화
	from sklearn.inspection import DecisionBoundaryDisplay
	model_2d = RandomForestClassifier().fit(X_train[['Age', 'Fare']], y_train)
	DecisionBoundaryDisplay.from_estimator(model_2d, X_train[['Age', 'Fare']], response_method="predict", alpha=0.3)
	sns.scatterplot(data=X_test, x='Age', y='Fare', hue=y_test, edgecolor='k')
