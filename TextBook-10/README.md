...
| 분야                                     | 대표 연구사례                                       | 연구 목적 / 문제 정의                        | 로지스틱 회귀 적용 내용                               | 주요 결과 또는 의의                             |
| :------------------------------------- | :-------------------------------------------- | :----------------------------------- | :------------------------------------------ | :-------------------------------------- |
| **의료·보건(Medical & Health)**            | 당뇨병 발병 예측 (Diabetes Prediction)               | 혈당, BMI, 연령, 가족력 등으로 환자의 당뇨 발병 확률 예측 | 독립변수(건강지표)와 종속변수(발병 여부)를 회귀계수로 추정하여 위험요인 도출 | 단순모델로도 정확도 80% 이상 달성, 의료진의 조기 진단 지원 가능  |
|                                        | 심혈관질환 위험도 평가 (Cardiovascular Risk Assessment) | 나이, 혈압, 콜레스테롤 등으로 심근경색 위험도 산출        | 다변량 로지스틱 회귀로 변수별 Odds Ratio 계산              | 위험인자(흡연, 비만)의 유의도 확인 및 예방모델 구축          |
| **금융·경제(Finance & Economics)**         | 신용카드 부도 예측 (Credit Default Prediction)        | 고객 소득, 직업, 신용점수로 연체 가능성 분류           | 로지스틱 회귀를 통한 부도 확률 산출 및 임계값 기반 등급화           | 해석 용이성↑, 신용평가 모델의 기본 벤치마크로 활용           |
|                                        | 대출 리스크 평가 (Loan Risk Evaluation)              | 대출 신청자의 상환능력 예측                      | 이진 로지스틱 회귀를 통한 승인/거절 판단                     | 변수 중요도(Income, Debt Ratio) 분석을 통한 정책 개선 |
| **사회·심리(Social & Behavioral Science)** | 소비자 구매의사 결정요인 분석                              | 가격, 브랜드, 광고효과가 구매에 미치는 영향 평가         | 구매여부(1/0)를 종속변수로 설정, 행동요인별 회귀계수 추정          | 가격 민감도 및 충성도 분석 등 마케팅 전략 수립             |
|                                        | 투표행동 분석 (Voting Behavior Analysis)            | 연령, 소득, 정치성향이 투표참여에 미치는 영향 연구        | 로지스틱 회귀를 이용한 투표참여 확률 추정                     | 사회경제적 요인이 참여율에 미치는 상관관계 규명              |
| **산업·공학(Industrial & Engineering)**    | 품질불량 예측 (Defect Prediction in Manufacturing)  | 센서데이터로 공정 이상 여부 판정                   | 공정변수 → 불량 발생(0/1) 회귀분석                      | 실시간 공정 모니터링 및 불량률 감소                    |
|                                        | 안전사고 발생 확률 예측 (Safety Incident Prediction)    | 근로자 특성 및 근무환경에 따른 사고위험 분석            | 로지스틱 회귀로 사고발생 확률 산정                         | 작업환경 개선 및 예방정책 수립 근거                    |
| **IT·데이터(Information & Data Science)** | 스팸메일 분류 (Spam Detection)                      | 이메일 단어빈도, 발신자정보 등으로 스팸 여부 분류         | 다차원 텍스트 피처를 이용한 이진 로지스틱 분류                  | 단순모델임에도 높은 정밀도·재현율 확보                   |
|                                        | 사용자 이탈 예측 (Customer Churn Prediction)         | 서비스 이용패턴으로 해지 가능성 판단                 | 로지스틱 회귀 기반 확률모형으로 고객 유지율 분석                 | 변수 중요도 분석으로 마케팅 타깃 도출                   |


| 분야                                      | 대표 연구사례                                           | 연구 목적 / 문제 정의              | **독립변수 (X)**                | **종속변수 (y)**               | 로지스틱 회귀 적용 내용                              | 주요 결과 또는 의의                      |
| :-------------------------------------- | :------------------------------------------------ | :------------------------- | :-------------------------- | :------------------------- | :----------------------------------------- | :------------------------------- |
| **의료·보건 (Medical & Health)**            | **당뇨병 발병 예측 (Diabetes Prediction)**               | 건강지표를 이용해 환자의 당뇨 발병 확률 예측  | 혈당, BMI, 연령, 혈압, 가족력 등      | 당뇨 발병 여부 (1: 발병, 0: 정상)    | 이진 로지스틱 회귀로 변수별 Odds Ratio 계산 → 주요 위험요인 도출 | 단순모델로도 높은 정확도(≈80%) 확보, 조기진단에 활용 |
|                                         | **심혈관질환 위험도 평가 (Cardiovascular Risk Assessment)** | 생활습관·생체정보 기반 심근경색 위험 확률 산정 | 연령, 흡연여부, 혈압, 콜레스테롤, 운동량 등  | 심근경색 발생 여부 (1: 발생, 0: 미발생) | 다변량 로지스틱 회귀로 변수의 기여도 추정                    | 흡연·비만의 Odds Ratio 유의, 예방요인 규명    |
| **금융·경제 (Finance & Economics)**         | **신용카드 부도 예측 (Credit Default Prediction)**        | 고객 특성으로 부도 가능성 예측          | 소득, 신용점수, 직업, 부채비율, 거래이력 등  | 부도 여부 (1: 부도, 0: 정상)       | 각 변수에 대한 회귀계수를 통해 부도확률 산정                  | 리스크 평가의 기준모델로 활용, 변수해석 용이        |
|                                         | **대출 리스크 평가 (Loan Risk Evaluation)**              | 대출신청자 상환능력 판단              | 나이, 소득, 부양가족수, 대출금액, 신용등급   | 상환여부 (1: 연체/부도, 0: 정상상환)   | 회귀계수 기반 점수화로 승인/거절 기준 설정                   | 단순·해석 용이 / 대출심사 자동화 가능           |
| **사회·심리 (Social & Behavioral Science)** | **소비자 구매의사 분석 (Consumer Purchase Intention)**     | 마케팅 요인이 구매에 미치는 영향 분석      | 가격, 브랜드인지도, 광고노출, 리뷰점수      | 구매여부 (1: 구매, 0: 미구매)       | 로짓함수 logit(p)=β₀+β₁x₁+… 구조로 구매확률 모델링       | 브랜드인지도·가격민감도 효과 정량화              |
|                                         | **투표참여 요인 분석 (Voting Behavior Analysis)**         | 사회경제적 요인이 투표참여에 미치는 영향 연구  | 연령, 소득, 교육수준, 정치성향, 관심도     | 투표참여 여부 (1: 참여, 0: 불참)     | 사회변수별 회귀계수 해석을 통한 참여확률 예측                  | 참여율 결정요인 규명, 정책적 시사점 제공          |
| **산업·공학 (Industrial & Engineering)**    | **품질불량 예측 (Defect Prediction in Manufacturing)**  | 공정데이터로 불량품 발생 가능성 예측       | 온도, 압력, 속도, 습도, 장비모델 등      | 불량 발생 여부 (1: 불량, 0: 정상)    | 실시간 센서데이터 기반 로지스틱 분류                       | 불량률 감소, 생산효율 향상                  |
|                                         | **안전사고 발생 확률 예측 (Safety Incident Prediction)**    | 근로환경·작업조건이 사고에 미치는 영향 분석   | 근무시간, 근속연수, 보호장비 착용, 환경지수   | 사고 발생 여부 (1: 발생, 0: 무사)    | 사고발생 확률 모델로 안전관리 위험등급화                     | 사고예방정책 수립 근거 제공                  |
| **IT·데이터 (Information & Data Science)** | **스팸메일 분류 (Spam Detection)**                      | 이메일 특성으로 스팸여부 분류           | 단어빈도, 발신자, 첨부파일 존재, 제목 길이 등 | 스팸 여부 (1: 스팸, 0: 정상)       | 다차원 텍스트 피처 기반 이진 로지스틱 분류                   | 단순모델로도 높은 정확도, 학습속도 빠름           |
|                                         | **사용자 이탈 예측 (Customer Churn Prediction)**         | 서비스 이용패턴 기반 고객 해지확률 예측     | 접속빈도, 이용기간, 불만건수, 결제액, 나이   | 이탈여부 (1: 해지, 0: 유지)        | 회귀계수 해석으로 주요 이탈 요인 도출                      | 마케팅 타깃 선정 및 유지율 향상 가능            |
...
| 분야                                      | 대표 연구사례                                        | 연구 목적 / 문제 정의                       | 베이즈 로지스틱 회귀 적용 내용                                                      | 주요 결과 또는 의의                                     |
| :-------------------------------------- | :--------------------------------------------- | :---------------------------------- | :--------------------------------------------------------------------- | :---------------------------------------------- |
| **의료·보건 (Medical & Health)**            | **질병 위험도 예측 (Disease Risk Prediction)**        | 소규모 환자 데이터에서 질병 발병 확률 추정 및 불확실성 정량화 | 환자별 변수(혈압, 유전자, 생활습관 등)에 대한 사전분포(Prior) 설정 → 사후분포(Posterior)로 위험 확률 추정 | 데이터 희소성에서도 안정적인 추정 가능, 임상 리스크 분석에 활용            |
|                                         | **신경계 질환 진단 (Neurological Disease Diagnosis)** | MRI 및 생체신호로 알츠하이머 발병 확률 예측          | 베이즈 추론 기반 회귀계수 추정으로 각 피처의 불확실성 평가                                      | 해석 가능한 확률적 예측, 의사결정 보조에 적합                      |
| **금융·경제 (Finance & Economics)**         | **신용 리스크 관리 (Credit Risk Management)**         | 경기 변동과 같은 불확실 환경에서 대출 부도 확률 예측      | 사전정보(Prior)를 신용등급 분포에 반영하여 파라미터 불확실성 고려                                | 극단적 상황(소규모 데이터, 비정상 분포)에서도 강건한 추정               |
|                                         | **시장 이상탐지 (Market Anomaly Detection)**         | 비정상 거래 패턴 탐지 및 확률적 경보 시스템 구축        | 거래 지표를 입력변수로 확률 기반 분류 수행                                               | False Positive 감소, 탐지 민감도 향상                    |
| **사회·심리 (Social & Behavioral Science)** | **정책 수용도 예측 (Policy Acceptance Study)**        | 표본 수가 적은 설문에서 정책 찬성 확률 예측           | 응답자의 특성과 사전경향(Prior Belief)을 통합하여 예측                                   | 표본 불균형 문제 완화, 사전편향 반영 가능                        |
|                                         | **심리적 위험요인 분석 (Behavioral Risk Modeling)**     | 스트레스·우울 등 이진 심리 상태 분류               | 심리변수 간 불확실성 고려한 확률적 회귀 분석                                              | 모수의 신뢰구간 제공으로 해석 신뢰성 확보                         |
| **산업·공학 (Industrial & Engineering)**    | **예방정비(Preventive Maintenance)**               | 센서 데이터 부족 시 설비 고장 확률 추정             | 고장 이력 기반 사전분포 설정 후 사후분포로 위험 확률 산정                                      | 희귀 이벤트(고장) 상황에서도 예측 가능                          |
|                                         | **안전사고 확률모델링 (Safety Risk Modeling)**          | 작업환경 변화 시 사고 발생 확률 평가               | 환경 변수에 대한 사전 확률 설정 후 업데이트                                              | 실시간 안전지표 업데이트 가능, 신뢰도 높은 예측                     |
| **IT·데이터 (Information & Data Science)** | **스팸메일 및 악성코드 분류 (Spam/Malware Detection)**    | 학습데이터의 불균형(정상:비정상 비율 불균등) 문제 해결     | 베이즈 사전분포로 클래스 불균형 보정                                                   | 단순 로지스틱보다 안정적 확률예측, 과적합 감소                      |
|                                         | **사용자 행동 예측 (User Behavior Prediction)**       | 로그 데이터 기반 사용자 클릭·전환 확률 추정           | 회귀계수의 사후분포 분석으로 개별 사용자별 불확실성 계산                                        | 개별화된 확률 추정(Personalized Probabilistic Model) 가능 |
