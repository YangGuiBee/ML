...
EM 알고리즘은 잠재변수를 포함한 확률 모델에서 최대우도추정을 수행하기 위한 반복 최적화 방법으로, 잠재변수의 사후확률을 계산하는 E-step과 이를 이용해 모델 파라미터를 갱신하는 M-step을 번갈아 수행한다. 각 반복에서 데이터 우도는 단조 증가하며, 특히 가우시안 혼합모형과 같이 숨은 구성요소가 있는 생성 모델의 파라미터 추정에 널리 사용(E-step: 각 데이터가 어느 숨은 상태(가우시안)에 속했을지 확률적으로 추정하는 단계, M-step: 그 확률을 믿고, 모델 파라미터를 가장 잘 맞게 다시 계산하는 단계)<br>
