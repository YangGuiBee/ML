...
▣ 정의  
로지스틱 회귀를 다중 클래스 분류로 확장한 모델.  
각 클래스에 속할 확률을 동시에 계산한다.

▣ 목적  
3개 이상의 클래스 중 어느 하나에 속할 확률을 추정한다.
...
▣ 정의  
변수 간 조건부 의존관계를 방향성 비순환 그래프(DAG) 형태로 표현한 확률 그래픽 모델 기반 분류기이다.

▣ 목적  
인과 관계와 확률적 의존성을 명시적으로 표현하여 분류와 추론을 동시에 수행한다.
...
▣ 정의 : 각 클래스의 평균 벡터(centroid)를 계산하고, 새로운 데이터가 가장 가까운 중심에 속하는 클래스로 분류한다.
▣ 목적 : 클래스 중심과의 거리 최소화를 통해 빠르고 단순한 분류 수행.
![eq](https://latex.codecogs.com/png.latex?\hat{y}=\arg\min_c\|x-\mu_c\|^2)
