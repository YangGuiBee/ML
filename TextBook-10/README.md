...
|[1-4] 다항 로지스틱 회귀 / 소프트맥스 회귀|$ P(y=k \mid \mathbf{x}) = \frac{\exp(\mathbf{w}_k^T\mathbf{x})}{\sum_{j=1}^K \exp(\mathbf{w}_j^T\mathbf{x})} $|$ K $: 클래스 개수<br>$ \mathbf{w}_k $: 클래스 $k$의 가중치 벡터<br>$ y $: 다중 클래스 레이블<br>$ \mathbf{x} $: 입력 특성 벡터|다중 선형 경계|이미지 분류<br>자연어 처리 다중 분류|
